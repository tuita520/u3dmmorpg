...
public class BlobShadow : MonoBehaviour
{
    ...
    public void AciveSepticalRender(bool act,int layer)
    {
        BatchedBlobShadowRender bbsr;
        if (batchedBlobShadowRenders.TryGetValue(layer,out bbsr))
        {
            if (bbsr != null && bbsr.gameObject.activeSelf!=act)
            {
                bbsr.gameObject.SetActive(act);
            }
        }
    }
    void Awake()
