...
namespace EventSystem
{
    ...
    public class EventDispatcher : IEventDispatcher
    {
        ...
        public System.Text.StringBuilder StringBuilder = new System.Text.StringBuilder();
                            ...
                            while (__enumerator1.MoveNext())
                            {
                                var eventListenerData = __enumerator1.Current;
                                {
                                    if (eventListenerData.Value != null && eventListenerData.Value.Enable)
                                    {
                                        ...
                                        {
                                            Logger.Error("Event dispatch error. " + aIEvent.GetType().Name +
                                                         " exception:" +
                                                         ex);
                                            continue;
                                        }
                                        ...
                                        //REMOVE IF ONESHOT
                                        if (eventListenerData.Value.EventListeningMode ==
                                            EventDispatcherAddMode.SINGLE_SHOT)
                                        {
                                            mAppendAction.Enqueue(() =>
                                            {
                                                innerHashtable.Remove(eventListenerData.Key);
                                            });
                                        }
                                    }
