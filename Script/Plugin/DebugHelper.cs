...
public class MyProfiler
{
    ...
    private static Stack<MyProfiler> ProfilerStack = new Stack<MyProfiler>();
    private static MyProfiler Root;

    public long StartTick;
    public long TotalTick;

    public Dictionary<string, MyProfiler> Children = new Dictionary<string, MyProfiler>();
    ...
    {
        var profiler = ProfilerStack.Pop();
        profiler.TotalTick += sw.ElapsedTicks - profiler.StartTick;
    }

    public static void BeginSample(string name)
    {
        MyProfiler profiler;
        if (!ProfilerStack.Peek().Children.TryGetValue(name, out profiler))
        {
            profiler = new MyProfiler();
            ProfilerStack.Peek().Children.Add(name, profiler);
        }

        profiler.StartTick = sw.ElapsedTicks;
    ...
    public static void Clear()
    {
        ProfilerStack.Clear();
        Root = new MyProfiler();
        ProfilerStack.Push(Root);
    }

    public static void Print(System.Text.StringBuilder sb)
    {
        Print(sb, Root, 1, 1);
    }

    private static void Print(System.Text.StringBuilder sb, MyProfiler p, decimal total, int level)
    {
        if (total == 0)
            total = 1;
        decimal t = p.TotalTick/(decimal) System.Diagnostics.Stopwatch.Frequency*1000;
        sb.AppendFormat("{0:N2}----{1:P}", t, t/total).AppendLine();
        foreach (var profiler in p.Children)
        {
            for (int i = 0; i < level; i++)
            {
                sb.Append("----");
            }
            sb.Append(profiler.Key);
            for (int i = 0; i < level; i++)
            {
                sb.Append("----");
            }
            Print(sb, profiler.Value, t, level + 1);
        }
    }
}