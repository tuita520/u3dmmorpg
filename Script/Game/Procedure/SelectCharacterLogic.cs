...
catch (Exception ex)
{
	...
	IEnumerator CreateMyRoleCoroutine(Action onLoadOver)
	{
		...
		for (int i=0; i<roleList.Count; i++)
		{
		    ...
		    var elfColorId = -1;
		    var star = roleInfo.FightElfStar;
            if (star > 0)
            {
                var tbElf = Table.GetElf(elfId);
                if (tbElf != null)
                {
                    // 根据精灵表查CharacterBase表取模型ID
                    var tbCharacterBase = Table.GetCharacterBase(tbElf.ElfModel);
                    if (tbCharacterBase != null)
                    {
                        var tbModel = Table.GetCharModel(tbCharacterBase.CharModelID);
                        if (tbModel != null && star <= tbModel.MyShader.Length)
                        {
                            elfColorId = tbModel.MyShader[star - 1];
                        }
                    }
                }
            }
            ...
            }, elfId, false, 0, false, -1, elfColorId);
		}
