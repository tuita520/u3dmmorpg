...
public class LoadingWindow : MonoBehaviour
{
	...
	public float LoadingPercent = 0.3f;
...
{
#endif
		EventDispatcher.Instance.RemoveEventListener(LoadingPercentEvent.EVENT_TYPE, OnPercentChange);
...
//0级说明还没有等级数据还没有初始化玩家还没有数据
		...
		EventDispatcher.Instance.AddEventListener(LoadingPercentEvent.EVENT_TYPE, OnPercentChange);
	    bar.value = 0;
...
catch (Exception ex)
{
	...
	private void OnPercentChange(IEvent ievent)
	{
		var e = ievent as LoadingPercentEvent;
		if (null == ievent)
		{
			return;
		}
...
try
{
		...
		if (LoadingPercent >= 0.98)
	    {
			bar.value = 1;
	    }
	    else
	    {
			bar.value = Mathf.Lerp(bar.value, LoadingPercent, Time.deltaTime);    
	    }
