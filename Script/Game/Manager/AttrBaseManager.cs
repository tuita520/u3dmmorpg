...
//组织静态数据
    ...
    public class PlayerAttr
    {
                ...
                for (var i = 0; i < tbMedal.AddPropID.Length; i++)
                {
                    if (tbMedal.AddPropID[i] != -1)
                    {
                        AttrList.modifyValue(tbMedal.AddPropID[i],
                            Table.GetSkillUpgrading(tbMedal.PropValue[i]).GetSkillUpgradingValue(Medala.nLevel));
                    }
                }
            }
        }
                ...
                while (__enumerator8.MoveNext())
                {
                    var book = __enumerator8.Current;
                    {
                        ...
                        if (tbBookBase != null)
                        {
                            ...
                            {
                                AttrList.modifyValue(i + 1, tbBookBase.AttrList[i] * tbBookBase.AddAttr / 10000);
                            }
                        }
                    }
                }
        ...
        private int GetAttrFightPoint()
        {
            ...
            {
                //基础固定属性
                long nValue = AttrBaseManager.GetAttrValue(mDataId, level, type);
                var BeRefList = AttrBaseManager.GetBeRefAttrList(mDataId, type);
                if (BeRefList != null)
                {
                    {
