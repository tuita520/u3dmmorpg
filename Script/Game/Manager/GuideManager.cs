...
// 		}
            ...
            var tbGuidance = Table.GetGuidance(id);
            if (tbGuidance != null)
            {
                if (tbGuidance.GuideRule == 1)//新功能开启
                {
                    for (int i = 0; i < UIManager.Instance.mRecordStack.Count; i++)
                    {
                        if (UIManager.Instance.mRecordStack[i].Config != UIConfig.MainUI )
                        {
                            EventDispatcher.Instance.DispatchEvent(new Close_UI_Event(UIManager.Instance.mRecordStack[i].Config));
                        }
                    }    
                }
            }
