...
namespace ScriptManager
{
    ...
    {
        //战力排行榜第一对象
        public List<PlayerInfoMsg> FightLeaderMaster = new List<PlayerInfoMsg>();
        public DateTime TeamInviteClickFubenTime = new DateTime();
        public int TeamInviteClickFubenID = 0;
        public bool isInTeamInvite = false;
        public bool isTaskWildShop = false;
        private MsgSceneLode _LodeInfo;
        public bool IsNeedUpdateLode = false;
        public bool IsCheckSailingTip = false;
        public int SelectChatRoom = -1;
        public bool IsChangingPkModel = false;
        public DateTime ChangeSceneTime = new DateTime();//记录玩家通过传送点或者飞鞋切场景的时间
        public MsgSceneLode LodeInfo
        {
            get
            {
                if (_LodeInfo != null)
                    ...
                    return new MsgSceneLode();
            }
            set
            {
                IsNeedUpdateLode = false;
                _LodeInfo = value;
                if (_LodeInfo == null || _LodeInfo.LodeList.Count <= 0)
                    return;
                Dictionary<int, int> dic = new Dictionary<int, int>();
                foreach (var v in _LodeInfo.LodeList)
                {
                    var lode = v.Value;
                    if (lode.Times <= 0)
                    {
                        ...
                        if (tb == null)
                            continue;
                        var t = DataTimeExtension.EpochStart.AddSeconds(lode.UpdateTime);
                        if (global::Game.Instance.ServerTime > t)
                        {
                            dic.Add(v.Key, tb.CanCollectNum);
                        }
                        else
                        {
                            IsNeedUpdateLode = true;
                        }
                ...
                {
                    if (_LodeInfo.LodeList.ContainsKey(v.Key) == true)
                    {
                        _LodeInfo.LodeList[v.Key].Times = v.Value;
                    }
                }
                EventDispatcher.Instance.DispatchEvent(new OnSceneLodeUpdateEvent());
        ...
        /// 组队当前队伍目标
        /// </summary>
        public TeamTargetChangeItemDataModel currentTeamTarget = new TeamTargetChangeItemDataModel();
        ...
        public bool NeedPlayUnlock = false;
        private int _CurTaskId = -1;
        private bool _InputControlHPEffect = false;
        private Dictionary<int, int> timeLimitEquipList = new Dictionary<int, int>();
        private float trialTickTime = 0.0f;
        private float peaceTime = 0.0f;
        ...
        public bool InputControlHPEffect
        {
            get { return _InputControlHPEffect; }
            set { _InputControlHPEffect = value; }
        }
        ...
        private List<int> mBookGropData = new List<int>();
        public int mFightBook { get; set; }

        private Dictionary<int, int> mBountyBooks = new Dictionary<int, int>();
        public MountData mMountData { get; set; }
        ...
        public List<int> BookGropData
        {
            get { return mBookGropData; }
        }

        public Dictionary<int, int> BountyBooks
        {
                ...
                if (msg.ErrorCode == (int)ErrorCodes.OK)
                {
                    ...
                    while (__enumerator7.MoveNext())
                    {
                        var bag = __enumerator7.Current;
                        {
                            ...
                            //时装特殊处理
                            if (bagType >= eBagType.EquipShiZhuang && bagType <= eBagType.WeaponShiZhuang)
                            {
                                InitEquipData(bag.Value);
                                continue;
                            }
        ...
        private IEnumerator ApplyBooksCoroutine()
        {
            ...
            mBookGropData = new List<int>(groups);
            mBountyBooks = new Dictionary<int, int>(books);
            mFightBook = msg.Response.Fight;
        ...
        {
            NetManager.Instance.StartCoroutine(ApplyMountCoroutine());
        }

        private IEnumerator ApplyMountCoroutine()
        {
            Logger.Info(".............ApplyMount..................");
            var msg = NetManager.Instance.AskMountData(0);
            yield return msg.SendAndWaitUntilDone();
            ...
            SetLoginApplyState(eLoginApplyType.Mount);
            if (msg.ErrorCode != (int)ErrorCodes.OK)
            {
                UIManager.Instance.ShowNetError(msg.ErrorCode);
                yield break;
            }
            mMountData = msg.Response;
            var controller = UIManager.Instance.GetController(UIConfig.MountUI);
            if (null != controller)
            {
        ...
        public void ApplyPlayerInfo(ulong uuid, Action<PlayerInfoMsg> action)
        {
            ...
            {
                if (cache.Time.AddSeconds(GameUtils.PlayerInfoCacheTime) > global::Game.Instance.ServerTime)
                {
                    action(cache.Info);
                    return;
                }
        ...
        private IEnumerator ApplyPlayerInfoCoroutine(ulong characterId, Action<PlayerInfoMsg> action)
        {
            ...
            if (msg.State == MessageState.Reply)
            {
                if (msg.ErrorCode == (int)ErrorCodes.OK)
                {
                    ...
                    {
                        Info = info,
                        Time = global::Game.Instance.ServerTime
                    };
        ...
        public void ApplyAnchorRoomInfo()
        {
            NetManager.Instance.StartCoroutine(ApplyAnchorRoomInfoCoroutine());
        }

        public void ApplyAnchorIsInRoom()
        {
            NetManager.Instance.StartCoroutine(ApplyAnchorIsInRoomCoroutine());
        }

        private IEnumerator ApplyAnchorIsInRoomCoroutine()
        {
            var msg = NetManager.Instance.GetAnchorIsInRoom(0);
            yield return msg.SendAndWaitUntilDone();
            if (msg.State == MessageState.Reply)
            {
                EventDispatcher.Instance.DispatchEvent(new AnchorEnterRoomEvent(msg.Response == 1));
            }
        }

        private IEnumerator ApplyAnchorRoomInfoCoroutine()
        {
            var msg = NetManager.Instance.ApplyAnchorRoomInfo(0);
            yield return msg.SendAndWaitUntilDone();
            if (msg.State != MessageState.Reply)
            {
            ...
            GVoiceManager.Instance.Open = msg.Response.Open;
            GVoiceManager.Instance.AnchorRoom = msg.Response.AnchorRoom;
            GVoiceManager.Instance.AnchorName = msg.Response.AnchorName;
            GVoiceManager.Instance.GuildSpeekLevel = msg.Response.GuildSpeekLevel;
            GVoiceManager.Instance.AnchorBeginTime = msg.Response.BeginTime;
            GVoiceManager.Instance.AnchorEndTime = msg.Response.EndTime;
            GVoiceManager.Instance.OnlineAnchorName = msg.Response.OnlineAnchorName;
        }
        ...
        public void ApplySuperVipData(bool isDisPatch)
        {
            NetManager.Instance.StartCoroutine(ApplySuperVipDataCoroutine(isDisPatch));
        }
        private IEnumerator ApplySuperVipDataCoroutine(bool isDisPatch)
        {
            var msg = NetManager.Instance.ApplySuperVIP(1);
            yield return msg.SendAndWaitUntilDone();
            var control = UIManager.Instance.GetController(UIConfig.MainUI);
            if (msg.ErrorCode != 0)
            {

                if (control != null)
                    control.CallFromOtherClass("ChangeIsShowSuperVipIcon", new[] { (object)0 });
                UIManager.Instance.ShowNetError(msg.ErrorCode);
                yield break;
            }
            if (msg.ErrorCode == (int)ErrorCodes.OK)
            {
                if (isDisPatch)
                    EventDispatcher.Instance.DispatchEvent(new PreDownLoadImageEvent(msg.Response));
                else
                {
                    if (control != null)
                        control.CallFromOtherClass("ChangeIsShowSuperVipIcon", new[] { (object)msg.Response.State });
                }
            }
        }
        ...
        public void BagItemCountChange(BagsChangeData bag)
        {
            ...
            var changeCount = new Dictionary<int, int>();
            {
                ...
                while (__enumerator12.MoveNext())
                {
                    var changeData = __enumerator12.Current;
                    {
                        ...
                        else if (bagId >= (int)eBagType.Equip01 && bagId <= (int)eBagType.Equip12 || bagId == (int)eBagType.EquipShiZhuang)
                        {
                                    ...
                                    var itemBaseData = __enumerator23.Current;
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (changeItem.ItemId != -1)
                                            {
                                                ...
                                                if (bagId == (int)eBagType.Equip || bagId == (int)eBagType.EquipShiZhuangBag)
                                                {
            ...
            if (bag.BagsChange.ContainsKey(17))
            {
                var b = bag.BagsChange[17];
                if (b.ItemsChange.ContainsKey(0) && b.ItemsChange[0].ItemId == -1)
                {
                    ChooseEquipFromBag(17);
                }
            }
        }

        private void ChooseEquipFromBag(int pos)
        {
            var bag = GetBag(0);
            foreach (var v in bag.Items)
            {
                if (v.ItemId > 0)
                {
                    var tbEquip = Table.GetEquipBase(v.ItemId);
                    if (tbEquip == null)
                        continue;
                    if (!GameUtils.IsCanEquip(tbEquip, pos))
                        continue;

                    var jobId = GetRoleId();
                    if (tbEquip.Occupation != jobId)
                        continue;

                    BagItemDataModel dm = null;
                    switch (jobId)
                    {
                        case 0:
                            if (v.ItemId == 213001) dm = v;
                            break;
                        case 1:
                            if (v.ItemId == 310001) dm = v;
                            break;
                        case 2:
                            if (v.ItemId == 410001) dm = v;
                            break;
                    }
                    if (dm == null) continue;

                    var control = UIManager.Instance.GetController(UIConfig.EquipComPareUI);
                    control.CallFromOtherClass("ReplaceEquip", new[] { dm });
                    return;
                }
            }
        }
        ...
        public int GetFubenCount(int fubenId)
        {
            ...
            var fubenCount = GetExData(tbFuben.TotleExdata);
            return fubenCount;
        }
        ...
        public eEquipLimit CheckItemEquip(ItemBaseRecord tbItem, EquipBaseRecord tbEqup)
        {
            ...
            var rebornId = PlayerDataManager.Instance.PlayerDataModel.Attributes.Resurrection;
            if (rebornId < tbEqup.NeedRebornLevel)
            {
                return eEquipLimit.Reborn;
            }
        ...
        public void CreateAttributeSync(ulong characterId)
        {
            for (var i = (int)eSceneSyncId.SyncLevel; i < (int)eSceneSyncId.SyncMax; i++)
            {
                ...
                var i1 = i;
                if (i1 == (int)eSceneSyncId.SyncPlayerName)
                {
                    ...
                    (int attributeValue) =>
                    {
                        ...
                        else if (i1 == (int)eSceneSyncId.SyncAreaState)
                        {
                            ...
                            if (null != myPlayer && myPlayer.AreaState != (eAreaState)state)
                            {
                                ...
                                if (objMyPlayer.IsInSafeArea())
                                {
                                    GameUtils.ShowHintTip(10223);
                                }
                        ...
                        else if (i1 == (int)eSceneSyncId.SyncPkValue)
                        {
                            ...
                            if (objMyPlayer)
                            {
                                objMyPlayer.RefreshnameBarod();
                                 ...
                                 || i1 == (int)eSceneSyncId.SyncTitle4
                            )
                        {
                            ...
                            if (TitleList.ContainsKey(ii))
                            {
                                ...
                                if (GetCharacterBaseData().TitleList.Count > ii)
                                {
                                    GetCharacterBaseData().TitleList[ii] = TitleList[ii];
                                }
                            }
                            else
                            {
                                ...
                                var tmpIndex = i1 - (int)eSceneSyncId.SyncTitle0;
                                if (GetCharacterBaseData().TitleList.Count > tmpIndex)
                                {
                                    GetCharacterBaseData().TitleList[tmpIndex] = TitleList[tmpIndex];
                                }
                            }
        ...
        public List<BagItemDataModel> GetBagItemByItemIdjingJie(int bagId, int jingjieId)
        {
            var result = new List<BagItemDataModel>();
            result.Clear();
            var enumerator = EnumBagItem(bagId).GetEnumerator();
            while (enumerator.MoveNext())
            {
                ...
                var tbBase = Table.GetEquipBase(tempItem.ItemId);
                if (tbBase == null)
                {
                    continue;
                }
                if (tbBase.EquipUpdateLogic == jingjieId && tempItem.Count > 0)
                {
                    result.Add(tempItem);
                }
            }
        ...
        public void GainNewItem(int itemId, int count, int index)
        {
            ...
            if (tbItem.Type >= 10000 && tbItem.Type <= 10099)
            {
                ...
                if ((bool)control.CallFromOtherClass("CanAutoRecyleItem", new object[] { tbItem.Id }))//白绿蓝紫装备自动回收 特殊处理
                {
                    EventDispatcher.Instance.DispatchEvent(new EquipAutoRecycleEvent(index));
                    return;
                }
                //判断装备背包是否满 如果满则弹提示
                if (GetRemaindCapacity(eBagType.Equip) <= 0)
                {
                    UIManager.Instance.ShowMessage(MessageBoxType.OkCancel, GameUtils.GetDictionaryText(100001407), ""
                        , () =>
                        {
                            var ee = new Show_UI_Event(UIConfig.SettingUI, new SettingArguments(1));
            ...
            EventDispatcher.Instance.DispatchEvent(new GainNewItem_Event(itemId, count));
        ...
        public int GetBagItemFightPoint(BagItemDataModel bagItem, int level = -1, bool bCalcLevel = true)
        {
            if (bagItem.ItemId == -1)
            {
            ...
            var attributes = GetEquipAttributeFix(bagItem, level, bCalcLevel);
            var fightPoint = GetAttrFightPoint(attributes, level);
            bagItem.FightValue = fightPoint;
            return fightPoint;
        }
        public int GetBagItemFightPointNoSave(BagItemDataModel bagItem, int level = -1, bool bCalcLevel = false)
        {
            if (bagItem.ItemId == -1)
            {
                bagItem.FightValue = 0;
        ...
        public bool GetBookGropEnable(int groupId)
        {
            return mBookGropData.Contains(groupId);
        }

        public int GetBountyBookEnable(int bookId)
        {
            if (mBountyBooks.ContainsKey(bookId))
            {
                return mBountyBooks[bookId];
            }
            return -1;
        }
        ...
        public PlayerInfoMsg GetCharacterSimpleInfo(ulong id, bool isTime = false)
        {
            ...
            {
                if (cache.Time.AddSeconds(60 * 5) > global::Game.Instance.ServerTime || isTime == false)
                {
                    return cache.Info;
                }
            }
        ...
        public int GetElfSkillFightPoint(Dictionary<int, int> skill)
        {
            var fightPoint = 0;
            var enumorator = skill.GetEnumerator();
            while (enumorator.MoveNext())
            {
                fightPoint += GetOneBuffFightPoint(enumorator.Current.Key, enumorator.Current.Value);
            }

            return fightPoint;
        }

        public int GetOneBuffFightPoint(int buffId, int buffLevel)
        {
            if (buffId < 0)
                return 0;
            ...
            if (value < 0)
            {
                Logger.Error("Elf GetOneSkillFightPoint skillupgrading is null  skillID={0} buffLevel={1}", buffId, buffLevel);
                return 0;
            }
            return value;
        }
        ...
        public Dictionary<int, int> GetEquipAttributeFix(BagItemDataModel bagItem, int characterLevel = -1, bool bCalcLevel = true)
        {
            var attributes = new Dictionary<int, int>();
            var tbItem = Table.GetItemBase(bagItem.ItemId);
            var tbEquip = Table.GetEquipBase(tbItem.Exdata[0]);

            var enchanceLevel = bCalcLevel ? bagItem.Exdata[0] : 0;
            if (characterLevel == -1)
            {
            ...
            if (bagItem.Exdata.Count >= 35)
            {
                for (var i = 0; i < 2; i++)
                {//附加属性
                    var attrId = tbEquip.BaseFixedAttrId[i];
                    var baseValue = bagItem.Exdata[33 + i];
            ...
            {
                int addtional = bCalcLevel ? bagItem.Exdata[1] : 0; //bagItem.Exdata[1]
            ...
            for (var i = 0; i < 6; i++)
            {
                ...
                var baseValue = bagItem.Exdata[12 + i]/100;
                if (attrId != -1 && baseValue > 0)
                {
                    ModifyEquipAttribute(attributes, attrId, baseValue, characterLevel);
                }
        ...
        public int GetFightPoint(int[] attr, int[] attrRef, int level, int careerId)
        {
            ...
            FightPoint += RefFightPoint(BaseRef(eAttributeType.MagPowerMin), attrRef[(int)eAttributeType.MagPowerMin], tbLevel.PowerFightPoint);
            FightPoint += RefFightPoint(BaseRef(eAttributeType.PhyArmor), attrRef[(int)eAttributeType.PhyArmor], tbLevel.ArmorFightPoint);
            FightPoint += RefFightPoint(BaseRef(eAttributeType.HpMax), attrRef[(int)eAttributeType.HpMax], tbLevel.HpFightPoint);
            FightPoint += RefFightPoint(BaseRef(eAttributeType.MpMax), attrRef[(int)eAttributeType.MpMax], tbLevel.MpFightPoint);
            //FightPoint += RefFightPoint(BaseRef(eAttributeType.Hit), attrRef[(int)eAttributeType.Hit], tbLevel.HitFightPoint);
            //FightPoint += RefFightPoint(BaseRef(eAttributeType.Dodge), attrRef[(int)eAttributeType.Dodge], tbLevel.DodgeFightPoint);

            return (int)FightPoint;
        }

        public int BaseRef(eAttributeType type)
        {
            var tbState = Table.GetStats((int)type);
            if (tbState == null)
            {
            ...
            var careerId = GetRoleId();
            if (careerId >= 0)
            {
                if (CareeridToStatsPointIndex.ContainsValue(careerId))
                {
                    var value = GetAttribute(type);
                    return tbState.FightPoint[CareeridToStatsPointIndex[careerId]] * value;
                }
            }
        ...
        public int GetNormalSkill(bool weapon = false)
        {
            ...
            if (PlayerDataModel.SkillData.NormalAttack.Count > 0)
            {
                return PlayerDataModel.SkillData.NormalAttack[0].SkillId;
            }
            return -1;
        }
        ...
        public void InitBagData(BagBaseData bagBase)
        {
            ...
            for (var i = 0; i < bagBaseDataMaxCapacity1; i++)
            {
                ...
                bagItemData.UpDownStatus = i < bagBaseData.Capacity ? 0 : 1;
                ...
                for (var __i8 = 0; __i8 < __listCount8; ++__i8)
                {
                    var item = __list8[__i8];
                    {
                            ...
                            || bagId == (int)eBagType.Depot)
                        {
                            ...
                            AddTimeLimitEquip(bagItemData, true);
                        }
            ...
            {
                bagBaseData.UnlockTime = global::Game.Instance.ServerTime.AddSeconds(bagBase.NextSecond);
            }
            else
            {
                bagBaseData.UnlockTime = global::Game.Instance.ServerTime;
            }
                ...
                case 13:
                    {
                        ...
                        for (var i = 0; i < dataItemsCount4; i++)
                        {
                            ...
                            AddTimeLimitEquip(equipData, true);
                        }
                    }
                    break;
                default:
        ...
        public bool ItemOrEquipCanUse(ItemBaseRecord tbItemBase)
        {
            ...
            {
                if (-1 != tbItemBase.OccupationLimit)
                {
                    if (Instance.GetRoleId() != tbItemBase.OccupationLimit)
                    {
                        return false; 
                    }
                }
        ...
        //技能书学习技能
        public void LearnSkill(int skillid, bool equipNewSkill = true, int index = -1)
        {
            ...
            var tb = Table.GetSkill(skillid);
            if (tb == null || tb.Type == 2)
                return ;
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (skill.SkillId == skillid)
                {
                    ...
                    if (equipNewSkill)
                    {
                        ...
                        {
                            EventDispatcher.Instance.DispatchEvent(new UIEvent_SkillFrame_EquipSkill(index, skill.SkillId));
                        }
                    }
                }
            }
        ...
        public bool CheckShowSkill(int firstShowEraId = -1)
        {
            ...
            if (!IsInFubenScnen())
            {
                if (NeedPlayUnlock)
                {
                    NeedPlayUnlock = false;
                    EventDispatcher.Instance.DispatchEvent(new Event_EraNoticeFlyIcon(Vector3.zero, true, firstShowEraId));
                    return true;
                }

                var eraId = GetExData(eExdataDefine.e711);
                if (eraId < 0)
                {
                    return false;
                }

                if (GetExData(eExdataDefine.e710) < 0)
                {
                    ...
                    var role = GetRoleId();
                    if (tbMaya.SkillIds.Count >= 3)
                    {
                        LearnSkill(tbMaya.SkillIds[role]);
                    }
                    EraManager.Instance.SetAnimEra(eraId);
                }
                else
                {
                ...
                SetExData((int)eExdataDefine.e711, -1);

                NetManager.Instance.StartCoroutine(EraSkillPlayed(eraId));
                return true;
            }
        ...
        private IEnumerator EraSkillPlayed(int eraId)
        {
            using (new BlockingLayerHelper(0))
            {
                var msg = NetManager.Instance.EraPlayedSkill(eraId);
                yield return msg.SendAndWaitUntilDone();
                if (msg.State == MessageState.Reply)
                {
                    if (msg.ErrorCode == (int)ErrorCodes.OK)
        ...
        public void NotifyMatchingData(QueueInfo queueInfo)
        {
            ...
            if (queueData.QueueId == -1)
            {
                ...
                EventDispatcher.Instance.DispatchEvent(e1);
                EventDispatcher.Instance.DispatchEvent(new UIEvent_WindowShowDungeonQueue(global::Game.Instance.ServerTime, -1));
            }
            else
            {
        ...
        {
            EventDispatcher.Instance.DispatchEvent(new MainDownUINoticeFlagRefrsh());
        ...
        private void RefrehNoticeFlagCondition()
        {
            ...
            NoticeData.ElfDrawOpen = CheckCondition(GameUtils.ElfDrawConfig) == 0;
            ...
            NoticeData.TeamOpenFlag = /*CheckCondition(GameUtils.TeamFlagConfig) == 0*/true;
        ...
        private void RefrehNoticeFlagCondition(int flagId, bool flagValue)
        {
            ...
            if (flagId == GameUtils.ConditionToFlag[GameUtils.ElfDrawConfig])
            {
                NoticeData.ElfDrawOpen = flagValue;
            }
            ...
            {
                NoticeData.TeamOpenFlag = true/*flagValue*/;
            }
        ...
        public void RefreshEquipBagStatus(BagItemDataModel bagItem)
        {
            ...
            if (tbItem.Type < 10000 || tbItem.Type > 10099)
            {
                bagItem.Status = 0;//如果不是装备 状态置为0
                bagItem.UpDownStatus = 0;
                return;
            }
            ...
            if (canEquip == eEquipLimit.Occupation)
            {
                ...
                bagItem.UpDownStatus = (int)eBagItemType.No;
                return;
            }
            var fightcompare = 0;
                ...
                for (var i = eEquipType.WeaponMain; i <= eEquipTypeWeaponScend8; i++)
                {
                    ...
                    if (equipData.ItemId != -1)
                    {
                        ...
                        if (tbItemEquip.Type == 10099)
                        {
                            ...
                            {
                                fightcompare = GetBagItemFightPointNoSave(equipData); //equipData.FightValue;
                            }
                        }
                        break;
                    }
                    ...
                    case 10006: //
                        {
                            ...
                            int temp = GetBagItemFightPointNoSave(equipData); //
                            if (CheckItemEquip(equipData.ItemId) == eEquipLimit.OK || canEquip == eEquipLimit.Attribute)
                            {
                                if (fightcompare > temp)
                                {
                                    fightcompare = temp;
                                }
                            }
                    ...
                    case 100098:
                        {
                            ...
                            if (BitFlag.GetLow(tbEquip.Part, part))
                            {
                                ...
                                if (CheckItemEquip(equipData.ItemId) == eEquipLimit.OK || canEquip == eEquipLimit.Attribute)
                                {
                                    fightcompare = GetBagItemFightPointNoSave(equipData); //;
                                }
                            }
            ...
            if (GameUtils.IsTrialEnd(bagItem))
            {
                bagItem.Status = (int)eBagItemType.TrialEnd;
                bagItem.UpDownStatus = (int)eBagItemType.TrialEnd;
                return;
            }
            if (bagItem.FightValue > fightcompare)
            {
                bagItem.UpDownStatus = (int)eBagItemType.FightDown;
            }
            else if (bagItem.FightValue == fightcompare)
            {
                bagItem.UpDownStatus = (int)eBagItemType.UnLock;
            }
            else
            {
                bagItem.UpDownStatus = (int)eBagItemType.FightUp;
            }
            if (canEquip == eEquipLimit.Reborn || canEquip == eEquipLimit.Attribute || canEquip == eEquipLimit.Level)
            {
                bagItem.Status = (int)eBagItemType.No;
            }
            else
        ...
        public void RefreshEquipStatus()
        {
            for (var i = 0; i < PlayerDataModel.EquipList.Count; i++)
            {
                ...
                if (item.ItemId != -1)
                {
                    ...
                    var equipId = tbItem.Exdata[0];
                    var tbEquip = Table.GetEquipBase(equipId);
                    if (GameUtils.IsTrialEnd(item))
                        item.Status = (int)eBagItemType.TrialEnd;
                    else
                    {
                        var canEquip = CheckItemEquip(tbItem, tbEquip);
        ...
        public void SetBookGroupEnable(int groupId)
        {
            mBookGropData.Add(groupId);
        }

        public void SetBountyBookEnable(int bookId, int level = 1)
        {
            if (mBountyBooks.ContainsKey(bookId))
            {
                mBountyBooks[bookId]++;
        ...
        {
            var freeElfDraw = Instance.GetExData(14);
            var FreeTime = Extension.FromServerBinary(freeElfDraw);
            if (FreeTime > global::Game.Instance.ServerTime)
            {
        ...
        public void SetPkModel(int value)
        {
            ...
            if (SelectTargetData.CharacterBase == null)
            {
            ...
            else 
            {
                ...
                obj = ObjManager.Instance.FindCharacterById(SelectTargetData.CharacterBase.CharacterId);
                if (obj != null)
                    isMyEnemy = ObjManager.Instance.MyPlayer.IsMyEnemy(obj);                            //调末世前是敌人

                PlayerDataModel.CharacterBase.PkModel = value;//修改模式
                if (isMyEnemy && obj != null && !ObjManager.Instance.MyPlayer.IsMyEnemy(obj))   //以前是敌人现在不是敌人了，取消攻击和选择
                {
        ...
        public void SetRes(int resType, int count)
        {
            ...
            switch ((eResourcesType)resType)
            {
                case eResourcesType.LevelRes:
                    {
                        ...
                        else if (count > oldValue)
                        {
                            ...
                            //sdk统计数据
                            var ts = global::Game.Instance.ServerTime - DateTime.Parse("1970-1-1");
                            ...
                            PlayerPrefs.SetString(characterName + "timeLv", ((int)ts.TotalSeconds).ToString());
                            PlayerPrefs.Save();
                            PlatformHelper.CollectionLevelUpDataForKuaifa(count, ((int)ts.TotalSeconds).ToString(), diamond.ToString());
                        }
                ...
                case eResourcesType.HomeExp:
                    EventDispatcher.Instance.DispatchEvent(new UIEvent_SailingCheckRedPoint(1));
                    break;
                case eResourcesType.VipLevel:
                    PlayerDataModel.CharacterBase.VipLevel = GetRes((int)eResourcesType.VipLevel);
                    break;
            }
        }
        ...
        private void CheckWorldBossByRoleId(int roleId)
        {
            if (roleId >= 90000 && roleId <= 90029)
            {
                SelectTargetData.IsWorldBoss = true;
            }
            else
            {
                SelectTargetData.IsWorldBoss = false;
            }
        }
        ...
        public void SetSelectTargetData(ObjCharacter obj, int type = 0)
        {
            ...
            // 每次变换目标都重新设置
            CheckWorldBossByRoleId(obj.RoleId);
            ...
            if (SelectTargetData.CharacterBase.Reborn > 0)
            {
                SelectTargetData.CharacterBase.TargetHeadIconId = GameUtils.GetRebornSquareIconId(SelectTargetData.CharacterBase.RoleId,
                                                                                                    SelectTargetData.CharacterBase.Reborn);
            }
            else
            {
                var tbCharBase = Table.GetCharacterBase(SelectTargetData.CharacterBase.RoleId);
                if (null != tbCharBase)
                {
                    SelectTargetData.CharacterBase.TargetHeadIconId = tbCharBase.HeadIcon;
                }
            }
                ...
                while (__enumerator9.MoveNext())
                {
                    var itemBaseData = __enumerator9.Current;
                    {
                        ...
                        if (bagId == 0)
                        {
                            ...
                            if (addTimeLimit != -1)
                            {
                                AddTimeLimitEquip(itemBase, addTimeLimit == 1);
                            }
                        }
        ...
        public FriendInfoDataModel GetFriend(ulong guid)
        {
            IControllerBase conCtrler = UIManager.GetInstance().GetController(UIConfig.FriendUI);
            if (conCtrler == null)
                return null;
            ObservableCollection<FriendInfoDataModel> dataList = conCtrler.CallFromOtherClass("GetFriendsInfo", null) as ObservableCollection<FriendInfoDataModel>;
        ...
        public void GetFriend(List<FriendInfoDataModel> list, int online = 0, int level = 1)
        {
            IControllerBase conCtrler = UIManager.GetInstance().GetController(UIConfig.FriendUI);
            if (conCtrler == null)
                return;
            ...
            foreach (var v in dataList)
            {
                if (v.IsOnline >= online && v.Level >= level)
                {
                    list.Add(v);
                }
            }
        ...
        public void CollectLode(int Id)
        {//挖矿
            LodeRecord tbLode = null;
            ...
            foreach (var item in LodeInfo.LodeList)
            {
                if (item.Key == tbLode.Id)
                {
                    if (item.Value.Times <= 0)
                    {
                        GameUtils.ShowHintTip(290004);
                        return;
                    }
                }
            }
            var robNum = PlayerDataManager.Instance.GetExData(eExdataDefine.e680);
            if (robNum <= 0)
            {
                GameUtils.ShowHintTip(290012);
                return;
            }
            var command1 = new FuncCommand(() =>
            {
                ObjManager.Instance.MyPlayer.OnDisMount();
                var e = new ShowMissionProgressEvent(GameUtils.GetDictionaryText(290013));
                EventDispatcher.Instance.DispatchEvent(e);
                var endCallback = new Action(() =>
                {
                    ...
                    NetManager.Instance.StartCoroutine(CollectLodeCoroutine(tbLode.Id));
                });
                var interruptCallback = new Action(() =>
                {
                    EventDispatcher.Instance.DispatchEvent(new HideMissionProgressEvent(0));
                });
                GameLogic.Instance.StartCoroutine(GameUtils.CanBreakLaunchAction(17, tbLode.CollectTime, endCallback, interruptCallback));
            });
            GameControl.Executer.PushCommand(command1);
        }

        //祭拜
        public void WorshipMonument(int Id)
        {
            LodeRecord tbLode = null;
            Table.ForeachLode((tb) =>
            {
            ...
            //消耗判断
            if (PlayerDataManager.Instance.GetItemCount(tbLode.WorshipConsumeItemId) < tbLode.WorshipConsumeItemCount)
            {
                var itemName = Table.GetItemBase(tbLode.WorshipConsumeItemId).Name;
                var dicStr = GameUtils.GetDictionaryText(274005);//所需{0}不足
                var content = string.Format(dicStr, itemName);
            ...
            {
                ObjManager.Instance.MyPlayer.OnDisMount();
                var e = new ShowMissionProgressEvent(GameUtils.GetDictionaryText(224709));//祭拜英灵中...
                EventDispatcher.Instance.DispatchEvent(e);
                var endCallback = new Action(() =>
                {
                    ObjManager.Instance.MyPlayer.PlayAnimation(OBJ.CHARACTER_ANI.STAND);
                    EventDispatcher.Instance.DispatchEvent(new HideMissionProgressEvent(0));
                    NetManager.Instance.StartCoroutine(WorshipMonumentCoroutine(tbLode.Id));
                });
                var interruptCallback = new Action(() =>
                {
                    EventDispatcher.Instance.DispatchEvent(new HideMissionProgressEvent(0));
        ...
        public IEnumerator CollectLodeCoroutine(int LodeId)
        {
            var msg = NetManager.Instance.CollectLode(LodeId);
            yield return msg.SendAndWaitUntilDone();
            if (msg.State == MessageState.Reply)
            {
                if (msg.ErrorCode == (int)ErrorCodes.OK)
                ...
                else if (msg.ErrorCode == (int)ErrorCodes.Error_Lode_Collect_ErrorTimes)
                {
                    GameUtils.ShowHintTip(290004);
                    yield break;
                }
                else if (msg.ErrorCode == (int)ErrorCodes.Error_Lode_ErrorId)
                {
                    GameUtils.ShowHintTip(200007008);
                    yield break;
                }
            }
        }

        public IEnumerator WorshipMonumentCoroutine(int MonumentId)
        {
            var msg = NetManager.Instance.WorshipMonument(MonumentId);
            yield return msg.SendAndWaitUntilDone();
            if (msg.State == MessageState.Reply)
            {
                if (msg.ErrorCode == (int)ErrorCodes.OK)
                {
                    if (msg.Response.Data.Count > 0)
                    {
                        var e = new ShowItemsArguments
                        {
                            Items = msg.Response.Data,
                            Tip = GameUtils.GetDictionaryText(224710)//玛雅女神感受到了您的诚意，特此奖励您以下物品
                        };
        ...
        public void HoldLode(int Id)
        {//战旗
            MainUIController MainCtr = UIManager.GetInstance().GetController(UIConfig.MainUI) as MainUIController;
            if (2 != (MainCtr.GetDataModel("MainUI") as MainUIDataModel).MainActivity)
            {
                var UnionName = SceneManager.Instance.UnionName;
                if (string.IsNullOrEmpty(UnionName))
                {
                    GameUtils.ShowHintTip(290005);
                    return;
                }
                else
                {
                    var str = string.Format(GameUtils.GetDictionaryText(290000), UnionName);
                    GameUtils.ShowHintTip(str);
                    return;
                }
            }
            WarFlagRecord tbWarFlag = null;
            ...
            if (!GameUtils.CheckIsOpen(tbWarFlag.BelongToTime))
            {
                var UnionName = SceneManager.Instance.UnionName;
                if (string.IsNullOrEmpty(UnionName))
                {
            ...
            {
                GameUtils.ShowHintTip(290001);
                return;
            }
            else if (LodeInfo.TeamId == UnionId)
            {
                GameUtils.ShowHintTip(GameUtils.GetDictionaryText(290016));
                return;
            }
            var command1 = new FuncCommand(() =>
            {
                ...
                var endCallback = new Action(() =>
                {
                    ObjManager.Instance.MyPlayer.PlayAnimation(OBJ.CHARACTER_ANI.STAND);
                    EventDispatcher.Instance.DispatchEvent(new HideMissionProgressEvent(0));
                    NetManager.Instance.StartCoroutine(HoldLodeCoroutine(tbWarFlag.Id));
                });
                var interruptCallback = new Action(() =>
                {
                    EventDispatcher.Instance.DispatchEvent(new HideMissionProgressEvent(0));
                });
                GameLogic.Instance.StartCoroutine(GameUtils.CanBreakLaunchAction(17, tbWarFlag.CollectTime, endCallback, interruptCallback));
            });
            GameControl.Executer.PushCommand(command1);
        }

        public IEnumerator HoldLodeCoroutine(int LodeId)
        {
            var msg = NetManager.Instance.HoldLode(LodeId);
            yield return msg.SendAndWaitUntilDone();
            if (msg.State == MessageState.Reply)
            {
                if (msg.ErrorCode == (int)ErrorCodes.OK)
                ...
                else if (msg.ErrorCode == (int)ErrorCodes.Error_Lode_IsOwnerAlliance)
                {
                    GameUtils.ShowHintTip(GameUtils.GetDictionaryText(290016));
                }
            }
        }
        public List<FriendInfoDataModel> GetFriendTop15(int online = 0)
        {
            List<FriendInfoDataModel> list = new List<FriendInfoDataModel>();
            PartnerFrameCtrler conCtrler = UIManager.GetInstance().GetController(UIConfig.FriendUI) as PartnerFrameCtrler;
            if (conCtrler == null)
                return null;
            var tmpList = new List<FriendInfoDataModel>();
            var tmp = new List<FriendInfoDataModel>();
            ObservableCollection<FriendInfoDataModel> dataList = conCtrler.CallFromOtherClass("GetFriendsInfo", null) as ObservableCollection<FriendInfoDataModel>;
            foreach (var v in dataList)
            {
                if (v.IsOnline >= online && v.Guid != PlayerDataManager.Instance.GetGuid())
                    tmpList.Add(v);
            }
            tmpList.Sort();
        ...
        public void Tick(float delta)
        {
            ...
            trialTickTime += Time.deltaTime;
            if (trialTickTime >= 1.0f)
            {
                trialTickTime = 0.0f;
                var enumorator = timeLimitEquipList.GetEnumerator();
                while (enumorator.MoveNext())
                {
                    var equip = enumorator.Current.Key;
                    if (global::Game.Instance.ServerTime.GetTimeStampSeconds() >= (equip + 2))
                    {
                        timeLimitEquipList.Remove(equip);

                        var equipData = GetEquipData(eEquipType.WeaponMain);
                        if (equipData.ItemId == enumorator.Current.Value)
                            NetManager.Instance.StartCoroutine(rereshTiralTimeCoroutine());
                        break;
                    }
                }
            }

            if (GetPkValue() == 0 && GetPkModel() == (int)ePkModel.GoodEvil && !IsChangingPkModel)
            {
                peaceTime += Time.deltaTime;
                if (peaceTime >= GameUtils.ChangePeaceModeInterval)
                {
                    ChangePkModel((int)ePkModel.Peace);
                    peaceTime = 0.0f;
                }
            }
            else
            ...
            {
                if (tmpTime.AddSeconds(10).Date != Game.Instance.ServerTime.Date)
                {
                    tmpTime = Game.Instance.ServerTime;
                    EventDispatcher.Instance.DispatchEvent(new ChangeDayEvent());
                    Game.Instance.LoginTime = Game.Instance.ServerTime;
                }
            }
        }

        public DateTime tmpTime = DateTime.MinValue;
        private IEnumerator rereshTiralTimeCoroutine()
        {
            using (new BlockingLayerHelper(0))
            {
                var msg = NetManager.Instance.RereshTiralTime(1);
                yield return msg.SendAndWaitUntilDone();
                if (msg.State == MessageState.Reply)
                {
                    if (msg.ErrorCode == (int)ErrorCodes.OK)
                    {
                        var str = GameUtils.GetDictionaryText(10225);
                        UIManager.Instance.ShowMessage(MessageBoxType.Ok, str, string.Empty);
                    }
                }
            }
        }

        private void AddTimeLimitEquip(BagItemDataModel equip, bool add)
        {
            if (add)
            {
                if (equip.Exdata.LeftTime >= 0)
                {
                    if (equip.Exdata.LeftTime > 0)
                    {
                        timeLimitEquipList[equip.Exdata.TrialEndTimeSeconds] = equip.ItemId;
                    }
                    else
                    {
                        equip.Status = (int)eBagItemType.TrialEnd;
                    }
                }
            }

            if (!add && equip.Exdata.LeftTime >= 0)
            {
                timeLimitEquipList.Remove(equip.Exdata.LeftTime);
            }
        }
        ...
        public void SetExData(int index, int value)
        {
            ...
            switch (index)
            {
                ...
                case (int)eExdataDefine.e288:
                    {
                        ...
                        EventDispatcher.Instance.DispatchEvent(e);
                        if ((int)eExdataDefine.e282 == index)
                        {
                            PlayerDataManager.Instance.GetCharacterBaseData().GuildId = value;
                        }
                        return;
                    }
                    break;
                ...
                case (int)eExdataDefine.e71:
                    ...
                    break;
                case (int)eExdataDefine.e622:
                    {
                        EventDispatcher.Instance.DispatchEvent(new TowerBtnClickEvent(4));
                        return;
                    }
                    break;
                case (int)eExdataDefine.e623:
                    {
                        EventDispatcher.Instance.DispatchEvent(new TowerRefreshEvent());
                        return;
                    }
                    break;
                case (int)eExdataDefine.e688:
                    {
                        EventDispatcher.Instance.DispatchEvent(new StarRefreshEvent());
                        return;
                    }
                    break;
                default:
        ...
        public int CheckCondition(int nConditionId)
        {
            ...
            if (tbcon.Role != -1)
            {
                if (!BitFlag.GetLow(tbcon.Role, GetRoleId()))
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition() tbcon.RoleDict = {0}", tbcon.RoleDict);
                    }
            ...
            for (var i = 0; i != tbcon.TrueFlag.Length; ++i)
            {
                ...
                if (!GetFlag(tbcon.TrueFlag[i]))
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition() tbcon.FlagTrueDict = {0}", tbcon.FlagTrueDict);
                    }
            ...
            for (var i = 0; i != tbcon.FalseFlag.Length; ++i)
            {
                ...
                if (GetFlag(tbcon.FalseFlag[i]))
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition() tbcon.FlagFalseDict = {0}", tbcon.FlagFalseDict);
                    }
            ...
            for (var i = 0; i != tbcon.ExdataId.Length; ++i)
            {
                ...
                if (tbcon.ExdataMin[i] != -1 && nExdataValue < tbcon.ExdataMin[i])
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition() Min tbcon.ExdataDict = {0}  [nExdataValue]= {1}", i, nExdataValue);
                    }
                ...
                if (tbcon.ExdataMax[i] != -1 && nExdataValue > tbcon.ExdataMax[i])
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition() Max tbcon.ExdataDict = {0}  [nExdataValue]= {1}", i, nExdataValue);
                    }
                    return tbcon.ExdataDict[i];
                }
            }
            ...
            for (var i = 0; i != tbcon.ItemId.Length; ++i)
            {
                ...
                if (tbcon.ItemCountMin[i] != -1 && nCount < tbcon.ItemCountMin[i])
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition()Min tbcon.ItemDict[i] = {0}  [Count]= {1}", tbcon.ItemDict[i], nCount);
                    }
                ...
                if (tbcon.ItemCountMax[i] != -1 && nCount > tbcon.ItemCountMax[i])
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition()Max tbcon.ItemDict[i] = {0}  [Count]= {1}", tbcon.ItemDict[i], nCount);
                    }
            ...
            {
                var endTime = global::Game.Instance.ServerTime;
                ...
                if (diffMonth < tbcon.OpenTime[0])
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition()tbcon.OpenTimeDict = {0} diffMonth={1}", tbcon.OpenTimeDict[0], diffMonth);
                    }
            ...
            if (tbcon.OpenTime[1] > 0)
            {
                ...
                if (diffYear < tbcon.OpenTime[1])
                {
                    if (nConditionId == 119)
                    {
                        Logger.Debug("In CheckCondition()tbcon.OpenTimeDict = {0} diffYear={1}", tbcon.OpenTimeDict[1], diffYear);
                    }
        ...
        public void RefleshRewardInfo()
        {
            ...
            foreach (var item in NociceFlag)
            {
                ...
                var tbClient = Table.GetClientConfig(125);
                if (tbClient != null)
                {
                    var config = tbClient.Value.ToInt();
                    var warnDict = PlayerDataManager.Instance.CheckCondition(config);
                    if (warnDict == 0)
                    {
                    ...
                    {
                        RewardNotice.ShowButtonState = 0;
                    }
                }
        ...
        public void RewardNoticeReflesh(string str)
        {
            if (NoticeStrToSort.ContainsKey(str))
            {
                ...
                switch (str)
                {
                    ...
                    case "Attributes":
                        {
                            SetNoticeFlag(index, NoticeData.Attributes);
                            break;
                        }
                    case "EndlessFairyland":
                        {
                            SetNoticeFlag(index, NoticeData.EndlessFairyland);
                            break;
                        }
        ...
        private bool SetOnlineTime()
        {
            ...
            {
                var timeSeconds = global::Game.Instance.OnLineSeconds > 0 ? global::Game.Instance.OnLineSeconds : 0;
        ...
        //获得成就状态
        public eRewardState GetAccomplishmentStatus(int id)
        {
            var tbAchv = Table.GetAchievement(id);
            if (0 != FlagData.GetFlag(tbAchv.RewardFlagId))
            {
                return eRewardState.HasGot;
            }
            if (0 != FlagData.GetFlag(tbAchv.FinishFlagId))
            {
        ...
        //获得成就进度
        public int GetAccomplishmentProgress(int id)
        {
            var _table = Table.GetAchievement(id);
            if (-1 != _table.Exdata)
            {
                return GetExData(_table.Exdata);
            }
            return 0;
        }
        ...
        {
            var finishTime = global::Game.Instance.ServerTime.AddSeconds(seconds);
            while (finishTime > global::Game.Instance.ServerTime)
            {
