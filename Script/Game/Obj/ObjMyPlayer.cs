...
public sealed class ObjMyPlayer : ObjCharacter
{
    ...
    private void AutoCombatToggle(bool isAuto, List<int> list = null)
    {
        ...
        {
            if (isAuto == true)
                AutoCombat.SetMissionList(new List<int>());
            else
                AutoCombat.ClearMissionList();
        }
        else
        {
    ...
    protected override void Awake()
    {
        ...
        GameSetting.Instance.MainPlayerSkillPosErrorDistance = Table_Tamplet.Convert_Int(Table.GetClientConfig(1222).Value);
#if !UNITY_EDITOR
        }
        catch (Exception ex)
        {
    ...
    public override void DoDie()
    {
        ...
        if (IsAutoFight() && GameLogic.Instance.Scene.TableScene.Type != (int)eSceneType.Pvp)
        {
            LeaveAutoCombat();
        }

        EventDispatcher.Instance.DispatchEvent(new UIEvent_RemoveBuffsOnDead());
        PlayerDataManager.Instance.PlayerDataModel.Attributes.HpNow = 0;
    }

    public void EnterAutoCombatEx()
    {//根据上一个自动状态切换自动进入日常任务战盟任务或者是单纯挂机
        if (AutoCombat == null)
        {
            Logger.Error("AutoCombat is null");
            return;
        }
        if (AutoCombat.lastMissionId > 0)
        {

            var tbMission = Table.GetMissionBase(AutoCombat.lastMissionId);
            if (AutoCombat.lastMissionId > 40000 && AutoCombat.lastMissionId < 50000)
            {//自动日常
                ...
                if (conllor != null)
                {
                    ...
                    if (data != null)
                    {
                        var missionId = data.List[2].MissionId;
                        if (missionId > 0)
                        {
            ...
            else if (AutoCombat.lastMissionId > 50000 && AutoCombat.lastMissionId < 60000)
            {//自动战盟
                var conllor = UIManager.GetInstance().GetController(UIConfig.MissionTrackList);
                if (conllor != null)
                {
                    ...
                    if (data != null)
                    {
                        var missionId = data.List[3].MissionId;
                        if (missionId > 0)
                        {
                            MissionManager.Instance.GoToMissionPlace(missionId);
                            return;
            ...
            else if (tbMission.ViewType==(int)eMissionMainType.Farm)
            {//自动狩猎
                var conllor = UIManager.GetInstance().GetController(UIConfig.MissionTrackList);
                if (conllor != null)
                {
                    ...
                    if (data != null)
                    {
                        var missionId = data.List2[0].MissionId;
                        if (missionId > 0)
                        {
                            MissionManager.Instance.GoToMissionPlace(missionId);
                            return;
        ...
        var mainCon = UIManager.Instance.GetController(UIConfig.MainUI) as MainUIController;
        if (mainCon != null)
        {
            if (mainCon.CallFromOtherClass("OnClickDungeonAutoFight", null) == (object)false)
            {//如果不在副本就原地挂机
                EnterAutoCombat();
            }
        }
    ...
    private bool mReLoadResources;
    public void InitImpl(InitBaseData initData, bool reLoad, Action callback = null)
    {
    ...
    public override bool Init(InitBaseData initData, Action callback = null)
    {
        ...
        DeleteObjTime = 0;
        SetCamp(data.Camp);
        mActorAvatar.ObjType = GetObjType();
        ...
        if (PlayerDataManager.Instance.mMountData.Ride != MountId)
        {
            MountId = PlayerDataManager.Instance.mMountData.Ride;
        }
        ...
        {
            InitTableData();
            InitAnimation(true, true);
            LoadModel();
            InitShadow(true);
            InitEquip(false, true);
            if (MountId > 0)
                Mount(MountId);
        }
        if (mDead)
        ...
        InitElfFollow();
        {
            //主角不销毁了,所以切换场景重造时候要清干净上一个场景的buff
            mBuff.Reset();
    ...
    protected override void InitNavMeshAgent()
    {
        if (mNavMeshAgent)
        {
            GameObject.DestroyImmediate(mNavMeshAgent);
            mNavMeshAgent = null;
        }
    ...
    private void InitElfFollow()
    {
        ...
        var colorId = (int)UIManager.Instance.GetController(UIConfig.ElfUI).CallFromOtherClass("GetFightColorId", null);
        RefresElfFollow(ret, colorId);
    }
    ...
    public void LeaveAutoCombat(bool bClearDialyId = true)
    {
        var mainCon = UIManager.Instance.GetController(UIConfig.MainUI);
        if (mainCon != null)
        {
        ...
        //if (bClearDialyId == true)
        //{
        //    PlayerDataManager.Instance.CurTaskId = -1;
        //}
    }
    ...
    private void LoadModel()
    {
        ...
        SetModel(model);
        var frequency = mModel.GetComponent<AnimationUpdateFrequencyController>();
        if (frequency)
        {
        ...
        mAnimationController.Animation.enabled = true;
    ...
    protected override void OnDestroy()
    {
        ...
        EventDispatcher.Instance.RemoveEventListener(Event_ChickenPickUp.EVENT_TYPE, OnChickenPickUp);
    ...
    /// </summary>
    /// <param name="iEvent"></param>
    private void OnChickenPickUp(IEvent iEvent)
    {
        Event_ChickenPickUp pick = iEvent as Event_ChickenPickUp;
        if (pick != null)
        {
            var effect = Table.GetEffect(pick.effectId);
            EffectManager.Instance.CreateEffect(effect, this);
        }
            ...
            mModel.GetComponentsInChildren(OptList<Renderer>.List);
            {
                ...
                for (var __i6 = 0; __i6 < __arrayLength6; ++__i6)
                {
                    var render = __array6[__i6];
                    {
                        ...
                        Material oldMat = oreginalMaterial;
    ...
    public void RefresElfFollow(int dataId, int colorId)
    {
        ElfFollow.CreateObj(dataId, colorId);
    }

    [ContextMenu("RefreshnameBarod")]
    public void RefreshnameBarod()
    {
        ...
        if (null != GameLogic.Instance)
        {
            ...
            if (null != cam)
            {
                var per = (cam.Length - cam.MinLength) / (cam.MaxLength - cam.MinLength);
                offset = Mathf.Lerp(160, 0, per);
            }
        }
        ...
        if (HasMount())
        {
            ...
            if (tbMount != null)
            {
                NameBoard.transform.localPosition = new Vector3(0, tbMount.offsetY + offset, 0);
            }
        }
        else if (HasWing())
        {
            ...
            if (tableIndex > -1)
            {
                var tbCharModel = Table.GetClientConfig(tableIndex);
                if (tbCharModel != null)
                {
                    var offsite = int.Parse(tbCharModel.Value);
                    var height = (float)offsite;
                    NameBoard.transform.localPosition = new Vector3(0, height + offset, 0);
                }
                else
                {
                    NameBoard.transform.localPosition = new Vector3(0, 83 + offset, 0);
                }
        ...
        {
            NameBoard.transform.localPosition = new Vector3(0, 65 + offset, 0);
    ...
    protected override void Start()
    {
        ...
        EventDispatcher.Instance.AddEventListener(Event_ChickenPickUp.EVENT_TYPE, OnChickenPickUp);
        EventDispatcher.Instance.AddEventListener(Event_InviteChallenge.EVENT_TYPE, OnInviteChallenge);
    ...
    private void OnInviteChallenge(IEvent evt)
    {
        var e = evt as Event_InviteChallenge;
        StartCoroutine(SendInviteChallengeToServer(e.CharacterId));
    }

    private IEnumerator SendInviteChallengeToServer(ulong characterId)
    {
        var tip = string.Format(GameUtils.GetDictionaryText(100003321), 100);
        UIManager.Instance.ShowMessage(MessageBoxType.OkCancel, tip, "",
            () =>
            {//OK
                NetManager.Instance.StartCoroutine(OnConfirmToInviteChallenge(characterId));
            });
        yield break;
    }

    private IEnumerator OnConfirmToInviteChallenge(ulong characterId)
    {
        var msg = NetManager.Instance.InviteChallenge(characterId);
    ...
    protected override void Tick(float delta)
    {
        ...
        if (!GameLogic.Instance.LoadOver)
        {
            return;
        }
        mTickCount++;
        ...
        if (GetCurrentStateName() == OBJ.CHARACTER_STATE.IDLE || GetCurrentStateName() == OBJ.CHARACTER_STATE.HURT)
        {
            freeTime += Time.deltaTime;
            if (freeTime > 10000.0f)
            {
                freeTime = 10.0f;
            }
        }
        ...
        if (HasMount() == false && PlayerDataManager.Instance.mMountData.Ride > 0 && GameLogic.Instance.Scene.TableScene.IsCanRide > 0)
        {
            if (GetCurrentStateName() == OBJ.CHARACTER_STATE.RUN)
            {
                fMount += delta;
                if (fMount > 3.0f)
                {
                    OnRideMount(PlayerDataManager.Instance.mMountData.Ride);
                    //                    Mount(PlayerDataManager.Instance.mMountData.Ride);
                    fMount = 0;
                }
            }
            else if (GetCurrentStateName() != OBJ.CHARACTER_STATE.IDLE)
            {
                fMount = 0;
            }
        }
    ...
    public void OnRideMount(int mountId)
    {
        NetManager.Instance.StartCoroutine(SendRideMount(mountId));
    }

    public void OnDisMount()
    {
        NetManager.Instance.StartCoroutine(SendRideMount(0));
        Dismount();

    }
    private IEnumerator SendRideMount(int MountId = 0)
    {
        var msg = NetManager.Instance.Mount(MountId);
        yield return msg.SendAndWaitUntilDone();
    }
    ...
    private IEnumerator SendMoveToServerCoroutine(List<Vector3> targetList)
    {
        ...
        var msg = NetManager.Instance.MoveTo(GameLogic.Instance.Scene.SceneTypeId, v, 0.1f, game.ServerTime.ToServerBinary());
        watch.Reset();
        watch.Start();
        yield return msg.SendAndWaitUntilDone();
        watch.Stop();
                ...
                case (int)ErrorCodes.Error_CannotMove:
                    {
                        ...
                        var msg1 = NetManager.Instance.MoveTo(GameLogic.Instance.Scene.SceneTypeId, v, 0.1f, game.ServerTime.ToServerBinary());
                        watch.Reset();
                        watch.Start();
                        ...
                        {
                            game.ServerTimeDiff = (DateTime.Now - Extension.FromServerBinary(msg1.Response.Time)) -
                                                  TimeSpan.FromMilliseconds(watch.ElapsedMilliseconds / 2);
    ...
    private IEnumerator SendStopMoveToServerCoroutine(Vector3 p, Vector3 dir)
    {
        ...
        var msg = NetManager.Instance.StopMove(GameLogic.Instance.Scene.SceneTypeId, Pos);
        yield return msg.SendAndWaitUntilDone();

        if (msg.State == MessageState.Reply && msg.ErrorCode == (int)ErrorCodes.Error_DistanceTooMuch)
        {//客户端和服务器产生较大差异,按照服务器位置进行修正
            NetManager.Instance.SyncCharacterPostion(PlayerDataManager.Instance.CharacterGuid, msg.Response);
        }
