...
public class ObjNPC : ObjCharacter
{
    ...
    public bool TowardPlayer
    {
        ...
        set
        {
            ...
            else
            {
                ...
                if (null != controller && null != controller.Animation)
                {
                    if (!controller.Animation.IsPlaying(table.AinmName))
                    {
                        controller.PlayWithoutLoad(OBJ.CHARACTER_ANI.STAND);
                    }
                }
            }
        }
    ...
    public bool CanFaceToTarget()
    {
        ...
        return 0 != TableNPC.IsForwardYou;
    }
...
// 		}
    ...
    public override void StartAttributeSync()
	{
		...
		NetManager.Instance.SyncCenter.RequestSyncData(ServiceType.Scene, characterId, (uint)eSceneSyncId.SyncTargetCharacterId,
					i =>
					{
						var temp = i.Split('|');
                        //if (2 == temp.Length)
                        //{
                        //    int dataId = -1;
                        //    if (!string.IsNullOrEmpty(temp[0]) && int.TryParse(temp[1], out dataId))
                        //    {
                        //        CharacterBaseData.TargetPlayerName = temp[0];
                        //        CharacterBaseData.TargetPlayerDataId = dataId;
                        //    }
                        //}
                        if (3 == temp.Length)
                        {
	...
	public override void StopAttributeSync()
	{
		...
		var characterId = GetObjId();
		NetManager.Instance.SyncCenter.StopSyncData(ServiceType.Scene, characterId, (uint)eSceneSyncId.SyncTargetCharacterId);
	}
}