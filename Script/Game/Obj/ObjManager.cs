...
public class ObjManager : Singleton<ObjManager>, IManager
{
    ...
    private Dictionary<int,int>WarFlagDictionary = new Dictionary<int, int>();
    private bool IsInit = false;
    public Dictionary<ulong, ObjBase> ObjPool
    {
        get { return mObjDict; }
    }
    public ulong ChickenKingId { get; set; }
    ...
    public ObjMyPlayer CreateMainPlayer(InitMyPlayerData initData)
    {
        ...
        obj.name = initData.ObjId.ToString();
            ...
            MyPlayer.InitImpl(initData, true);
            obj.SetActive(true);

            mObjDict.Add(initData.ObjId, MyPlayer);
            Logger.Debug("SUCCESS: Create Character[{0}]", MyPlayer.GetObjId());
        ...
        else
        {
            ...
            MyPlayer.transform.position = p;
            MyPlayer.transform.rotation = q;
            try
            {
            ...
            {
                Logger.Error("CreateMainPlayer MyPlayer.gameObject.SetActive(true); throw exception:" + e);
            }

            MyPlayer.InitImpl(initData, false);
    ...
    {
        //技能指示器也有可能造成第一次释放技能卡顿
        ResourceManager.PrepareResource<GameObject>(Resource.PrefabPath.SkillIndicatorCaster, (res) =>
        {
            if (res == null)
            {
                Logger.Error("load res error : " + Resource.PrefabPath.SkillIndicatorCaster);
            }
        });
        ResourceManager.PrepareResource<GameObject>(Resource.PrefabPath.SkillIndicatorReceiver, (res) =>
        {
            if (res == null)
            {
                Logger.Error("load res error : " + Resource.PrefabPath.SkillIndicatorReceiver);
            }
        });
        ResourceManager.PrepareResource<Material>(Resource.Dir.Material + "Circle.mat", (res) =>
        {
            if (res == null)
            {
                Logger.Error("load res error : " + Resource.Dir.Material + "Circle.mat");
            }
        });
