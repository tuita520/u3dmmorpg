...
{
    public static readonly string[] MountPointName = { "LeftWeapen", "RightWeapen", "Center", "Top", "Bottom", "Model" };
    ...
    public void UpDateHpBar()
    {
        if (NameBoard != null)
        {
            NameBoard.UpdateHpBar(CharacterBaseData.Hp, CharacterBaseData.MaxHp);
        }
    }
    ...
    {
        {
            ...
            while (__enumerator6.MoveNext())
            {
                ...
                {
                    if ((int)eBagType.Wing == pair.Key && -1 != pair.Value && (pair.Value % 100 == 1))
                    {
                        return true;
                    }
                }
    ...
    {
       return mActorAvatar&&mActorAvatar.MountGameObject != null;
    }
    public virtual void InitShadow(bool isDynamicShadow = false, int dynamicCullingMask = -1)
    {
        if (GameSetting.Instance.GameQualityLevel > 1)
        {
            ...
            {
                mShadow = GameLogic.Instance.Scene.CreateBlobShadow(gameObject, CharModelRecord.ShadowSize, mDestroyObjectsWhenDie);

                if (GetObjType() == OBJ.TYPE.NPC || GetObjType() == OBJ.TYPE.RETINUE)
                {
                    if(mShadow)
            ...
            {
                var bs = gameObject.GetComponent<BlobShadow>();
                if (bs != null)
                {
                    bs.AciveSepticalRender(!isDynamicShadow, gameObject.layer);
                }
            }
        }
    }
    ...
    public virtual void RefreshAnimation()
    {
        ...
        switch (currentState)
        {
            case OBJ.CHARACTER_STATE.RUN:
            {
                if (HasMount())
                {
                    var anim = mActorAvatar.MountGameObject.GetComponentInChildren<Animation>();
                    if (anim != null)
                    {
                        anim.Play("Run");
                    }
                    ...
                    PlayAnimation(OBJ.CHARACTER_ANI.Ride_Run + MountStep);
                }
                else if (HasWing())
                {
            ...
            default:
            {
                if (HasMount())
                {
                    ...
                    PlayAnimation(OBJ.CHARACTER_ANI.Ride_Stand + MountStep);
                }
                else if (HasWing())
                {
//                      if (IsInSafeArea())
...
//翅膀播加速
    ...
    void ShowArrow()
    {
       // var effect = Table.GetEffect(304);
       // EffectManager.Instance.CreateEffect(effect, this, null, (e, id) =>
       // {
      //      ArrowTrans = e.transform;
      //  });
    }
    ...
    public ulong ChickenKingEffectId;
...
//     public virtual void PlayActorPlayerAnimation(int id, Action<string> action = null)
//     {
...
//     }
    ...
    public virtual void PlayAnimation(int id, Action<string> action = null)
    {
        ...
        if (GetModel() != null) 
        {
            ...
            {
                mAnimationController.PlayWithoutLoad(id, action);
            }
        }
    }
    ...
    protected override void Awake()
    {
        ...
        if(mActorAvatar) mActorAvatar.ObjType = GetObjType();
    ...
    public virtual void CreateNameBoard(string str = "", Dictionary<int, string> titleList = null)
    {
        ...
        ComplexObjectPool.NewObject(GetNameBoardRes(), go =>
        {
			...
			if (false == HeadBoardManager.Instance.IsWorking)
			{
				ComplexObjectPool.Release(go);
				return;
			}
            ...
            {
                bool isShow = character.mObjId == ObjManager.Instance.MyPlayer.mObjId;
                NameBoard.Reset(isShow);
                ShowArrow();
            }
            else
            {
            ...
            NameBoard.SetOwner(owner.gameObject,
				HeadBoardManager.Instance.gameObject,
    ...
    public void UpdateChickenExp(int lv, int exp)
    {
        if (NameBoard != null)
            NameBoard.UpdatePlayerChickenExp(lv, exp);
    }
    private EffectController effContoller;
    private bool isLoading;
    /// <summary>
    /// 是否显示鸡王标致
    /// </summary>
    /// <param name="isShow"></param>
    public void IsShowChickenKing(bool isShow,float posX = 0,float posZ = 0)
    {
        if (NameBoard != null)
            NameBoard.ShowChickenKing(isShow);
        if (effContoller != null)
        {
            effContoller.gameObject.SetActive(isShow);
        }
        else
        {
            if (isShow)
            {
                isLoading = true;
                var effect = Table.GetEffect(115);
                EffectManager.Instance.CreateEffect(effect, this, null, (e, id) =>
                {
                    effContoller = e;
                    isLoading = false;
                    ChickenKingEffectId = id;
                });
            }
        }
    }
    public virtual void OnNameBoardRefresh()
    {
        ...
        //战旗显示所属战盟
        var nameboard = string.Empty;
        if (WarFlagDic.ContainsValue(TableCharacter.Id))
        {
            var UnionName = SceneManager.Instance.UnionName;
            if (string.IsNullOrEmpty(UnionName))
            {
                nameboard = GameUtils.GetDictionaryText(290007);
            }
            else
            {
                var tbNPC = Table.GetNpcBase(TableCharacter.Id);
                if (null == tbNPC)
                {
                    return;
                }
                nameboard = string.Format(tbNPC.WarFlagShow, UnionName);
            }
            NameBoard.SetDescriptionText(nameboard, CalculateNameColor(this));
        }
        else if (GameUtils.GetNpcLodeID(TableCharacter.Id) > 0)
        {
            IControllerBase contrl = UIManager.GetInstance().GetController(UIConfig.FieldMineUI);
            if (contrl == null)
                return;

            NameBoard.SetDescriptionText(contrl.CallFromOtherClass("GetLodeTimer", new[] { (object)TableCharacter.Id }).ToString(), CalculateNameColor(this));
        }
        else
        {
            NameBoard.SetDescriptionText(nameboard, CalculateNameColor(this));
    ...
    public virtual void SetLodeName(string str)
    {
        if (NameBoard != null)
            NameBoard.SetDescriptionText(str, CalculateNameColor(this));
    }
    // 设置头顶面板名字
    public virtual void SetNameBoardName(string name, string camp, int reborn, string nameColor)
    {
        ...
        if (reborn > 0)
        {
            ...
            var blank = " ";
            nameStr = string.Format("[{0}]{1}{2}{3}{4}[-]", nameColor, Name, blank, rebornStr, camp);
        }
        else
        {
            nameStr = string.Format("[{0}]{1}{2}{3}[-]", nameColor, Name, rebornStr, camp);
        }
    ...
    public override bool Init(InitBaseData initData, Action callback = null)
    {
        ...
        if (SceneManager.Instance != null && SceneManager.Instance.isInChickenFuben() && GetObjId() == ObjManager.Instance.ChickenKingId)
        {
            IsShowChickenKing(true);
        }
        return true;
    }
    ...
    protected virtual void LoadModelAsync(Action callback = null)
    {
        ...
        ComplexObjectPool.NewObject(modelPath, model =>
        {
            ...
            if (null != mAnimationController.Animation)
            {
    ...
    {
        return mDataId < 10;  //小于10就认为是玩家模型PS:东斐说的
    }
    ...
    public virtual void InitEquip(bool sync = false, bool firstPriority = false)
    {
...

        {
            ...
            {
                if (EquipList.TryGetValue((int) eBagType.EquipShiZhuang, out fashionId) && fashionId%100 == 1)
                {
                    mActorAvatar.FashionMode = true;
                }
                else
                {
                    mActorAvatar.FashionMode = false;
                }
            }
    ...
    protected override void Tick(float delta)
    {
        ...
        // 优化npc和怪物，如果距离远，就降低刷新率，再远就不可见
        var objType = GetObjType();
        if (objType == OBJ.TYPE.NPC || objType == OBJ.TYPE.OTHERPLAYER || objType == OBJ.TYPE.RETINUE)
        {
            if (ObjManager.Instance.MyPlayer != null)
            {
                DistSqr = (int) Vector3.SqrMagnitude(
                    Position - (ObjManager.Instance.MyPlayer.Position + new Vector3(-1f, 0, 1f)));
            }

            // 优化动画更新帧数和可见性
            if (mAnimationUpdateFrequencyController != null)
            {
                // 7.75米以外的Idle的，可以开始降低动画的刷新速度
                if (DistSqr >= 60)
                {
                    if (GetCurrentStateName() == OBJ.CHARACTER_STATE.IDLE)
                    {
                        mAnimationUpdateFrequencyController.UpdateFrequency = DistSqr / 20;
                    }
                    else
                    {
                        mAnimationUpdateFrequencyController.UpdateFrequency = DistSqr / 30;
                    }
                }
                else
                {
                    mAnimationUpdateFrequencyController.UpdateFrequency =
                        GameSetting.Instance.SlowdownAnimationFrameRate;
                }
            }


            // 其他玩家的显隐由别的地方控制，这里只控制怪物，魔物等的显隐
            if (objType == OBJ.TYPE.NPC || objType == OBJ.TYPE.RETINUE)
            {
                if (mRenderer)
                {
                    ObjVisibility.Add(this);
                }
            }
        }
    }
    ...
    public void ChangeEquip(int part, int itemTypeId)
    {
        ...
        ChangeEquipModel(part, oldItemTypeId, itemTypeId,false,false);
    }
    ...
    {
        var EnchanceLevel = 0;
        if (part == (int)eBagType.EquipShiZhuang || part == (int)eBagType.Wing)
        {
            if (itemTypeId > 0)
            {
                var isHideModel = (itemTypeId % 100) == 0 ? true : false;
                if (isHideModel)
                {
                    itemTypeId = -1;
                }
        ...
        try
        {
            ...
            if (Part2Name.TryGetValue(part, out value))
            {
                if (itemTypeId == -1)
                {
                    if (part == (int)eBagType.EquipShiZhuang)
                    {
                        mActorAvatar.RestoreNormalMode();
                    }
                    else
                    {
                ...
                if (change)
                {
                    if (part == (int)eBagType.EquipShiZhuang)
                    {
                        mActorAvatar.ChangeToFashionMode(value, mountInfoPart.Path, viewRecordPart, sync);
                    }
                    else
                    {
            ...
            // 翅膀
            else if (part == (int)eBagType.Wing)
            {
                if (itemTypeId == -1)
                {
                    mActorAvatar.MountWeapon((MountPoint) mountInfo.Mount, string.Empty, mountInfo, viewRecord, part,
                    ...
                    try
                    {
                        var current = mAnimationController.GetPlayingName();
                        if (current.Equals("FlyIdle") ||
                            current.Equals("FlyMove"))
                        {
                            PlayAnimation(OBJ.CHARACTER_ANI.STAND);
                        }
                    }
                    ...
                    {
                        Logger.Error("ChangeEquipModel Wing" + e);
                    }
                    RestoreTopMountPositon();
                    return;
                }
                ...
                mActorAvatar.MountWeapon((MountPoint) mountInfo.Mount, mountInfo.Path, mountInfo, viewRecord, part, sync,
                    o =>
                    {
                        ...
                        if (ObjManager.Instance.MyPlayer != null)
                        {
                            bFirst = mObjId == ObjManager.Instance.MyPlayer.mObjId;
                        }

                        var animation = o.animation;
                        if (animation.GetClip("FlyIdle") != null && animation.GetClip("FlyMove") != null &&
                            animation.GetClip("Dead") != null)
                        {
                            animation["FlyIdle"].wrapMode = WrapMode.Loop;
                            animation["FlyMove"].wrapMode = WrapMode.Loop;
                            animation["Dead"].wrapMode = WrapMode.Loop;

                            animation.clip = animation.GetClip("FlyIdle");
                        ...
                        else
                        {
                            ResourceManager.PrepareResource<AnimationClip, AnimationClip, AnimationClip>(
                                ...
                                (flyIdle, flyMove, flyDead) =>
                                {
                                    ...
                                    if (mActorAvatar.WingGameObject && mActorAvatar.WingGameObject != o)
                                    {
                                        ComplexObjectPool.Release(mActorAvatar.WingGameObject);
                                    }
    ...
    private void CorrenctLayer(GameObject o)
    {
        o.SetLayerRecursive(o.layer);
        if (GetObjType() == OBJ.TYPE.MYPLAYER)
        {
            o.SetTagToLayer(GAMELAYER.IgnoreShadow, LayerMask.NameToLayer(GAMELAYER.IgnoreShadow));
            o.SetTagToLayer(GAMELAYER.Collider, LayerMask.NameToLayer(GAMELAYER.Collider));
        }
    }
    ...
    public void RefreshnameBarod()
    {
        if (GetObjType() == OBJ.TYPE.MYPLAYER)
        {
            var myplayer = this as ObjMyPlayer;
            if (myplayer != null)
                myplayer.RefreshnameBarod();
        }
    }
    ...
    public void Mount(int mountId)
    {
        ...
        {
            NetManager.Instance.StartCoroutine(ChangeMountCoroutine(mountId));
        }
    }

    public IEnumerator ChangeMountCoroutine(int mountId)
    {
        Dismount();
    ...
    public void Dismount()
    {
        ...
        MountStep = 0;
        MountId = 0;
        if (mActorAvatar.MountGameObject != null)
        {
            // 应该用这种方式删除装备
            mActorAvatar.MountWeapon(MountPoint.Model, "", null, null, (int)eBagType.Mount, false);
            mActorAvatar.MountGameObject = null;
            RefreshnameBarod();
            RefreshAnimation();
        }
    }
    ...
    public void RealMount(int mountId)
    {
        var tbMount = Table.GetMount(mountId);
        if (tbMount == null)
            return;
        
        MountStep = tbMount.Step;
        MountId = mountId;
        EquipBaseRecord equipInfo = Table.GetEquipBase(tbMount.ItemId);

        var mountInfo = Table.GetWeaponMount(equipInfo.EquipModel);
        var viewRecord = Table.GetEquipModelView(mountInfo.Enchance[0]);
       
         mActorAvatar.MountWeapon(MountPoint.Model, mountInfo.Path, mountInfo, viewRecord, (int)eBagType.Mount, false,
            o =>
            {
                // 特效不需要产生影子
                CorrenctLayer(o);
                ...
                if (ObjManager.Instance.MyPlayer != null)
                {
                    bFirst = ObjManager.Instance.MyPlayer.mObjId == mObjId;
                }
                {
                    mActorAvatar.MountGameObject = o;
                    var anim = o.GetComponentInChildren<Animation>();
                    anim.wrapMode = WrapMode.Loop;
                    anim["Run"].wrapMode = WrapMode.Loop;
                    anim["Stand"].wrapMode = WrapMode.Loop;
                    if (GetCurrentStateName() == OBJ.CHARACTER_STATE.RUN)
                    {
                        anim.Play("Run");
                        PlayAnimation(OBJ.CHARACTER_ANI.Ride_Run + MountStep);
                    }
                    else if (GetCurrentStateName() == OBJ.CHARACTER_STATE.DIE)
                    {
                        Dismount();
                    }
                    else
                ...
                RefreshnameBarod();
                //ResourceManager.PrepareResource<AnimationClip, AnimationClip>(
                //    path + "Stand.anim", path + "Run.anim",
                //    (flyIdle, flyMove) =>
                //    {
                //        if (o == null || mActorAvatar == null)
                //        {
