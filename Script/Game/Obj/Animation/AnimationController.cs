...
public class AnimationController : MonoBehaviour
{
    ...
    public bool PlayWithoutLoad(int animationId, Action<string> callback = null)
    {
        return Play(animationId, callback);
        ...
        if (null == Animation[tableData.AinmName])
        {
            //如果使用的是主角的动作,在尝试加载播放
            Play(animationId, callback);
            return false;
        }
        if (Animation.IsPlaying(tableData.AinmName) && mCurrentAnimationName == aniName)
        {
        ...
        if (tableData.WrapMode == 0 && state.wrapMode != WrapMode.Default)
        {
            state.wrapMode = WrapMode.Default; //默认是once
        }
        else if (tableData.WrapMode == 1 && state.wrapMode != WrapMode.ClampForever)
        {
            //state.wrapMode = WrapMode.Once;//动画播放完就停止播放状态
            state.wrapMode = WrapMode.ClampForever; //动画播完最后一帧就一直播放最后一帧
        }
        else if (tableData.WrapMode == 2 && state.wrapMode != WrapMode.Loop)
        {
            state.wrapMode = WrapMode.Loop; //循环模式，动画播完就从头播放
        }
        else if (tableData.WrapMode == 3 && state.wrapMode != WrapMode.PingPong)
        {
            state.wrapMode = WrapMode.PingPong; //循环模式，动画播完就从尾巴往回播放
        }
        else if (tableData.WrapMode == 4 && state.wrapMode != WrapMode.ClampForever)
        {
            state.wrapMode = WrapMode.ClampForever; //动画播完最后一帧就一直播放最后一帧
        }
    ...
    public bool Play(int animationId, Action<string> callback = null)
    {
        ...
        var c = Animation.gameObject.GetComponent<AnimationUpdateFrequencyController>();
        if (c != null)
        {
            if (c.OriginalAnimations.Contains(aniName))
            {
                var state = Animation[mCurrentAnimationName];
                if (null != state)
                {
        ...
        ResourceManager.PrepareResource<AnimationClip>(animFile, clip =>
        {
            if (null == clip)
            {
                Logger.Fatal("Load [{0}]animation[{1}] failded!",animationId, animFile);
                return;
            }
            ...
            Animation.AddClip(clip, aniName);
            Animation.clip = clip;
            Animation.cullingType = AnimationCullingType.BasedOnUserBounds;//这样当显示的时候又在视锥里可以出发一次播放，解决界面性质的模型播放动画经常卡死的问题
            var state = Animation[aniName];
            ...
            // 怪物召唤物的显隐由其他逻辑控制，不需要在这里处理
            if (mCharacter && mCharacter.GetObjType() != OBJ.TYPE.NPC && mCharacter.GetObjType() != OBJ.TYPE.RETINUE)
            {
                OptList<Renderer>.List.Clear();
                gameObject.GetComponentsInChildren(true, OptList<Renderer>.List);
                {
    ...
    private void Update()
    {
        ...
        if (mNeedProcessEndEvent)
        {
            if (Time.time >= mCurrentAniEndTime)
            {
                ...
                {
                    if (bWithout == true)
                        PlayWithoutLoad(mNextAnimationId);
                    else
