...
public class ComplexObjectPool
{
    ...
    private static Transform mUITransformRoot;
    private static GameObject mCleanerGameObject;

    private static Vector3 sDefaultPosition = new Vector3(0, 10000, 0);

    private static HashSet<int> mDontDestory = new HashSet<int>();
#if UNITY_EDITOR
    private static bool Quit;
#endif

    private static GameObject Cleaner
    {
        get
        {
            if (mCleanerGameObject)
                return mCleanerGameObject;

            mCleanerGameObject = new GameObject("ComplexObjectPoolCleaner");
            mCleanerGameObject.AddComponent<Helper>();
    ...
    public static Transform UIHolder
    {
        get
        {
            if (mUITransform)
            {
                ...
                {
                    mUITransformRoot = UIManager.Instance.UIRoot.transform;
                    mUITransform.parent = mUITransformRoot;
                                 ...
                                 bool dontDestory = false)
    {
        ...
        pool.NewObject(obj =>
        {
            if (dontDestory)
            {
                mDontDestory.Add(obj.GetInstanceID());
            }
    ...
    public static void Release(GameObject go, bool forceDestory = false, bool deactive = true)
    {
        ...
        if (!forceDestory && mObjectIndex.TryGetValue(go.GetInstanceID(), out pool))
        {
            if (mObjectIndex.Remove(go.GetInstanceID()))
            {
                ...
                mDontDestory.Remove(go.GetInstanceID());
            }
        }
        else
        {
...
catch (Exception ex)
{
    ...
    private class ComplexObjectPoolImpl
    {
        ...
        public void Destroy()
        {
            mEnumerating = true;
            {
                ...
                while (__enumerator3.MoveNext())
                {
                    ...
                    if (pair)
                    {
                        if (mDontDestory.Contains(pair.GetInstanceID()))
                        {
                            continue;
                        }
            ...
            if (mFreeGameObjects.Count > 0)
            {
                ...
                if (go == null)
                {
                    ...
                    ResourceManager.PrepareResource<GameObject>(Name, res =>
                    {
                        ...
                        var goRes = Object.Instantiate(res, sDefaultPosition, Quaternion.identity);
                        ...
                        if (!active)
                        {
                            SetTransformActiveRecursively(go.transform, false);
                        }
        ...
        public GameObject NewObjectSync(Action<GameObject> callbackIfReuse, Action<GameObject> callbackIfNew)
        {
            ...
            if (mFreeGameObjects.Count > 0)
            {
                ...
                if (go == null)
                {
                    ...
                    go = Object.Instantiate(res, sDefaultPosition, Quaternion.identity) as GameObject;
            ...
            {
                var res = ResourceManager.PrepareResourceSync<GameObject>(Name);
                var go = Object.Instantiate(res, sDefaultPosition, Quaternion.identity) as GameObject;
        ...
        private static void LayerToTag(GameObject go)
        {
            go.SetLayerToTag(LayerMask.NameToLayer(GAMELAYER.IgnoreShadow), GAMELAYER.IgnoreShadow);
            go.SetLayerToTag(LayerMask.NameToLayer(GAMELAYER.Collider), GAMELAYER.Collider);
            go.SetLayerToTag(LayerMask.NameToLayer(GAMELAYER.Distortion), GAMELAYER.Distortion);
        }
