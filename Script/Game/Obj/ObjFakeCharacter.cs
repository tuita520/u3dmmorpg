...
public class ObjFakeCharacter : ObjCharacter
{
    ...
    public static void ChangeColor(GameObject model, int colorId)
    {
        if (colorId == -1 || model == null)
            return;

        var tbShaderColor = Table.GetElfStarShader(colorId);
        if (tbShaderColor == null)
        {
            return;
        }

        var renderList = model.GetComponentsInChildren<SkinnedMeshRenderer>();
        for (var i = 0; i < renderList.Length; ++i)
        {
            ...
            if (null != mat)
            {
                mat.SetColor("_BColor",
                    new Color(tbShaderColor.FlowRed / 255.0f, tbShaderColor.FlowGreen / 255.0f, tbShaderColor.FlowBlue / 255.0f,
                        tbShaderColor.FlowAlpha / 255.0f));
                mat.SetColor("_TexColor",
                    new Color(tbShaderColor.SepcularRed / 255.0f, tbShaderColor.SepcularGreen / 255.0f, tbShaderColor.SepcularBlue / 255.0f,
                    tbShaderColor.SepcularAlpha / 255.0f));
                ResourceManager.ChangeShader(model.transform);
            }
        }
    }
