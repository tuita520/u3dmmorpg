...
public class PlayerAutoCombat : MonoBehaviour
{
    ...
    public int lastMissionId { get; set; }
    ...
    private void BeginAutoFight()
    {
        ...
        else if (tbScene != null && tbScene.Type == (int)eSceneType.Pvp)
        {
            mIsFuben = false;
            mIsPVP = true;
        }
        ...
        mGotoMissionId = mMissionList[0];
        {
            ...
            for (var __i2 = 0; __i2 < __listCount2; ++__i2)
            {
                var id = __list2[__i2];
                {
                    ...
                    else if (eMissionMainType.SubStoryLine != (eMissionMainType)table.ViewType)
                    {
                        mGotoMissionId = id;
                    }
                }
...
//距离足够
    ...
    private bool  IsFarFromBegin(Vector3 pos)
    {
        ...
        if (mIsFuben||mIsPVP)
        {
            var tbFuben = Table.GetFuben(GameLogic.Instance.Scene.TableScene.FubenId);
            if (tbFuben == null)
                return false;
            if (0 == tbFuben.FarmLogic)
            {
                //副本场景内挂机
                return false;
            }
            else if (1 == tbFuben.FarmLogic)
            {
                //副本原地挂机
                type = 0;                
            }
        ...
        if (targetObj == null)
        {
            targetObj = ObjManager.Instance.SelectNearestObj(MainPlayer.Position, obj =>
            {
                if (obj.IsCharacter())
                {
                    ...
                    if (character == null || character.Dead || character.CharacterBaseData.Hp == 0 || (character.GetObjType() == OBJ.TYPE.RETINUE))
                    {
                        return false;
                    }
...
//是否可拾取这种类型物品
    ...
    /// <summary>
    /// 爱丽丝相位副本特殊处理，未结束之前置于非自动战斗状态
    /// </summary>
    //public  void AliceAbout()
    //{
    //    mIsCheckFubenLogicId = false;
    ...
    {        
        ...
        if (eDungeonAssistType.PhaseDungeon == (eDungeonAssistType)fubenType.AssistType && PlayCG.Instance.IsPlayCG)
        {
            return true;
        }
        return false;
