...
namespace ClientService
{
    ...
    public static class Rank9xServiceInterfaceExtension
    {
        ...
        public static GetFightRankListOutMessage GetFightRankList(this IRank9xServiceInterface agent, int serverId, int rankType)
        {
            return new GetFightRankListOutMessage(agent, serverId, rankType);
        }
    ...
    public class GetFightRankListOutMessage : OutMessage
    {
        public GetFightRankListOutMessage(IAgentBase sender, int serverId, int rankType)
            : base(sender, ServiceType.Rank, 6504)
        {
            Request = new __RPC_Rank_GetFightRankList_ARG_int32_serverId_int32_rankType__();
            Request.ServerId=serverId;
            Request.RankType=rankType;
        ...
        public override void SetResponse(uint error, byte[] data)
        {
            ...
            {
                var ms = new MemoryStream(data, false);
                mResponse = Serializer.Deserialize<__RPC_Rank_GetFightRankList_RET_RankList__>(ms);
            }
            State = MessageState.Reply;
            ErrorCode = (int) error;
        }
