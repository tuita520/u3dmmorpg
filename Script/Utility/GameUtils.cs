...
//         }
//     }
    public static void SetActiveRecursive(this GameObject go, bool act)
    {
    ...
    public static void SetLayerToTag(this GameObject go, int layer, string tag)
    {
        OptList<Transform>.List.Clear();
        go.GetComponentsInChildren(true, OptList<Transform>.List);
        var s = OptList<Transform>.List.Count;
    ...
    public static void SetTagToLayer(this GameObject go, string tag, int layer)
    {
...
public static class Extension
{
    ...
    public static void ConvertToRenderTexturePostion(ref Vector3 vec3)
    {
        var radio = GameUtils.GetResolutionRadio();
        vec3 = vec3* radio;
    }

    public static Vector3 GetRenderTexturePostion(this Vector3 vec3)
    {
        var radio = GameUtils.GetResolutionRadio();
        return  vec3 * radio;
    }
...
public static class SkillExtension
{
    public static int GetSkillUpgradingValue(this SkillUpgradingRecord tbUpgrade, int nLevel)
    {
        if (tbUpgrade == null)
        {
            Logger.Error("GetSkillUpgradingValue Error TableId");
            return 0;
        }
        ...
        var dataId = tablePet.CharacterID;
        {
            ...
            while (__enumerator5.MoveNext())
            {
                var pair = __enumerator5.Current;
                {
                    ...
                    long nValue = pair.Value; //随从的直接用算好的属性
...
public static class GameUtils
{
    ...
    public static string LastSaveCommand = string.Empty;
    ...
    public static float AutoPickUpBuffDistance = 1.0f;
    //挂机拾取检索距离最大值
    ...
    public static int ElfDrawConfig = 58;
    ...
    public static int ChangePeaceModeInterval; //杀戮切换和平倒计时（秒）
...
//     }
    ...
    public static int FastReachCouponId;
    ...
    public static string AttributeValue(int attrId, int attrValue, bool fix = true)
    {
        ...
        switch (attrId)
        {
            ...
            case 18:
            {
                ret = string.Format("{0:N1}%", attrValue/10000.0f);
            }
                break;
            case 15:
            case 17:
            ...
            case 120:
            {
                ret = string.Format("{0:N1}%", attrValue / 100.0f) ;
            }
                break;
            case 98:
            case 99:
    ...
    public static bool CheckIsQuickUsing(BagItemDataModel itemData)
    {
        var gainItemCon = UIManager.Instance.GetController(UIConfig.GainItemHintUI);
        if (gainItemCon != null)
        {
            var dataModel = gainItemCon.GetDataModel("") as GainItemHintDataModel;
            if (dataModel != null)
            {
                foreach (var entry in dataModel.Entrys)
                {
                    if (entry.BagItemData.ItemId == itemData.ItemId)
                    {
                        itemData.Index = entry.BagItemData.Index;
                        return true;
                    }
                }
            }
    ...
    {        
        ...
        {
            if (name.IndexOf(record.Name, 0, StringComparison.CurrentCultureIgnoreCase) != -1)
            {
                isSensitive = true;
                return false;
            }
    ...
    public static bool CheckSensitiveWordOnce = true;
    ...
    public static void CheckSensitiveWord(ChatMessageDataModel chat)
    {
        if (chat.Type != (int)eChatChannel.System
            && chat.Type != (int)eChatChannel.SystemScroll
            && chat.Type != (int)eChatChannel.WishingDraw
            && chat.Type != (int)eChatChannel.WishingGroup
            && chat.Type != (int)eChatChannel.Help)
        {
            ...
            var endString = chat.Content;
            while (!string.IsNullOrEmpty(endString))
            {
                var startIndex = endString.IndexOf(SpecialCode.ChatBegin, StringComparison.OrdinalIgnoreCase);
                var endIndex = endString.IndexOf(SpecialCode.ChatEnd, StringComparison.OrdinalIgnoreCase);
                if (startIndex >= 0 && endIndex > startIndex)
                {
                    if (startIndex > 0)
                    {
                        var text = endString.Substring(0, startIndex);
                        newString += text.CheckSensitive();
                    }

                    var str = endString.Substring(startIndex + SpecialCode.ChatBegin.Length,
                        endIndex - startIndex - SpecialCode.ChatEnd.Length);
                    newString += (SpecialCode.ChatBegin + str + SpecialCode.ChatEnd);

                    endString = endString.Substring(endIndex + SpecialCode.ChatEnd.Length);
                }
                else
                {
                    newString += endString.CheckSensitive();
                    endString = "";
                }
            }

            chat.Content = newString;
        }
    }
    ...
    public static bool IsShiZuhangPart(int part)
    {
        if (part >= (int)eBagType.EquipShiZhuang && part <= (int)eBagType.WeaponShiZhuang)
        {
            return true;
        }
        return false;
    ...
    public static void ChangeMap(int sceneid)
    {
        var tbScene = Table.GetScene(sceneid);
        if (null == GameLogic.Instance.Scene || null == tbScene)
        {
            return;
        }
        else if (sceneid == GameLogic.Instance.Scene.SceneTypeId)
        {
            return;
        }
        else if (tbScene.IsPublic != 1)
        {
            ShowHintTip(200005011);
            return;
        }
        ...
        if (VipLevel < 4)
        {
            Action callback = () => 
            {
                var GoldCount = PlayerDataManager.Instance.GetRes((int)eResourcesType.GoldRes);
                if (GoldCount < tbScene.ConsumeMoney)
                {
                    ShowHintTip(210203);
                    return;
                }
                NetManager.Instance.StartCoroutine(ChangeMapCoroutine(sceneid)); ;
            };
            var str = string.Format(GetDictionaryText(270243), tbScene.ConsumeMoney, tbScene.Name);
            UIManager.Instance.ShowMessage(MessageBoxType.OkCancel, str,string.Empty ,callback);
        }
        else
        {
            NetManager.Instance.StartCoroutine(ChangeMapCoroutine(sceneid)); 
    ...
    private static IEnumerator ChangeMapCoroutine(int sceneid)
    {
        using (new BlockingLayerHelper(0))
        {
    ...
    public static int GetElfStarColorId(int modelId, int star)
    {
        var colorId = -1;
        var tbCharacterBase = Table.GetCharacterBase(modelId);
        if (tbCharacterBase != null)
        {
            var tbModel = Table.GetCharModel(tbCharacterBase.CharModelID);
            if (tbModel != null && star >= 1 && star <= tbModel.MyShader.Length)
            {
                colorId = tbModel.MyShader[star - 1];
            }
        }
    ...
    public static int GetFightElfId(ref int elfId, ref int elfColorId)
    {
        elfId = -1;
        elfColorId = -1;

        var elfDataModel = UIManager.Instance.GetController(UIConfig.ElfUI).GetDataModel("") as ElfDataModel;
        if (elfDataModel == null)
        {
            return -1;
        }
        var fightElfData = elfDataModel.Formations[0].ElfData;
        elfId = fightElfData.ItemId;
        if (elfId == -1)
        {
            return -1;
        }
        ...
        elfColorId = GetElfStarColorId(tbElf.ElfModel, fightElfData.Exdata.StarLevel);
        return 0;        
    }
    public static void BuildShowElfExData(ElfItemDataModel bagItem, int itemId)
    {
        ...
        list.Add(0);
        // 随机技能
        var tbBuffGroup = Table.GetBuffGroup(tbElf.BuffGroupId);
        if (tbBuffGroup != null && tbBuffGroup.BuffID.Count == 1)
        {
            list.Add(tbBuffGroup.BuffID[0]);
            list.Add(1);
        }
    ...
    public static int GetElfBuffId(ElfItemDataModel item, int index)
    {
        if (item != null && index >= 0)
        {
            var exId = (int)ElfExdataDefine.BuffId1 + index * 2;
            if (exId < item.Exdata.Count)
            {
                return item.Exdata[exId];
            }
        }
    ...
    public static int GetElfBuffLevel(ElfItemDataModel item, int index)
    {
        if (item != null)
        {
            var exId = (int)ElfExdataDefine.BuffLevel1 + index * 2;
            if (exId < item.Exdata.Count)
            {
                return item.Exdata[exId];
            }
    ...
    public static int GetElfSkillFightPoint(ElfItemDataModel item)
    {
        var skill = new Dictionary<int, int>();
        for (int i = 0; i < 3; i++)
        {
            var buffId = GetElfBuffId(item, i);
            var buffLevel = GetElfBuffLevel(item, i);
        ...
        return PlayerDataManager.Instance.GetElfSkillFightPoint(skill);        
    }

    private static IEnumerator ExitLoginCoroutine(Action act)
    {
        var msg = NetManager.Instance.ExitLogin(0);
        yield return msg.SendAndWaitUntilDone(TimeSpan.FromSeconds(1));
        if (act != null)
        {
            act();
        }
...
//        Logger.Error("GetAttrIndex attrId={0}", attrId);
    ...
    public static string GetTimeString(int seconds)
    {
        ...
        if (seconds > 3600)
        {
            ret = string.Format(GetDictionaryText(100002288), seconds / 3600, (seconds / 60) % 60);
        }
        else if (seconds > 60)
        {
            ret = string.Format(GetDictionaryText(100002282), seconds / 60, seconds % 60);
        }
        else
        {
            ret = string.Format(GetDictionaryText(100002283), seconds);
        }
        return ret;
    }
    ...
    public static int GetRebornSquareIconId(int roleId,int rebornId)
    {
        ...
        switch (roleId)
        {
            case 0://剑士
                {
                    iconId = tbReborn.zsRebornIconSquare;
                    break;
                }
            case 1://法师
                {
                    iconId = tbReborn.fsRebornIconSquare;
                    break;
                }
            case 2://弓手
                {
                    iconId = tbReborn.gsRebornIconSquare;
                    break;
                }
        }
        return iconId;
    ...
    public static int GetRebornCircleIconId(int roleId, int rebornId)
    {
        var tbReborn = Table.GetTransmigration(rebornId);
        if (null == tbReborn)
        {
        ...
        switch (roleId)
        {
            case 0://剑士
                {
                    iconId = tbReborn.zsRebornIcon;
                    break;
                }
            case 1://法师
                {
                    iconId = tbReborn.fsRebornIcon;
                    break;
                }
            case 2://弓手
                {
                    iconId = tbReborn.gsRebornIcon;
                    break;
                }
        }
        return iconId;
    ...
    public static string GetLastTimeDiffString(TimeSpan span)
    {
        ...
        {
            return string.Format(GetDictionaryText(220960), Math.Floor(span.TotalDays / 365)); //n年前
        }
        if (span.TotalDays > 30)
        {
            return string.Format(GetDictionaryText(220959), Math.Floor(span.TotalDays / 30)); //n月前
        }
        if (span.Days > 0)
        {
        ...
        if (span.Hours > 0)
        {
            return string.Format(GetDictionaryText(220957), span.Hours); //n小时前
        }
        if (span.Minutes > 0)
        {
...
//         return ip;
	...
	public static string MakeItemString(int itemId, int count = 0)
	{
		var tbItem = Table.GetItemBase(itemId);
		if (tbItem == null)
		{
		...
		if (count > 1)
		{
			name = string.Format("{0}X{1}",name, count);
		}
		return name;
	}

    public static string GetItemNameColorString(int itemId)
    {
        var tbItem = Table.GetItemBase(itemId);
        if (tbItem == null)
        {
    ...
    /// <summary>
    /// 获取给定时间值能分成多少个5分钟(不足5分钟，按5分钟计算)
    /// </summary>
    /// <returns></returns>
    public static int GetTimeRounddingPeerFiveMinute(TimeSpan span)
    {
        double peer = span.TotalMinutes * 0.2f;//*0.2相当于/5，,计算机乘法比除法速度快
        double d = Math.Truncate(peer);
        int ret = Convert.ToInt32(d);
        if (d < peer)
        {
            ret++;
        }
    ...
    public static string GetAllTimeDiffString(TimeSpan span, bool trim = false)
    {
        if (span.Days > 0)
        {
            if (trim && span.Hours == 0)
        ...
        if (span.Hours > 0)
        {
            ...
            return string.Format(GetDictionaryText(210409), span.Hours, span.Minutes, span.Seconds);
        }
        if (span.Minutes > 0)
        {
            if (trim && span.Seconds == 0)
    ...
    //时分秒倒计时  
    public static string GetAllTimeDiffString(DateTime serverTime, bool trim = false)
    {
        return GetAllTimeDiffString(serverTime - Game.Instance.ServerTime, trim);
    }
    ...
    public static void GotoUiTab(int uiId, int arg0, int arg1 = -1, int arg2 = -1)
    {
        ...
        if (c.UiRecord == UIConfig.PlayFrame.UiRecord)
        {
            var ee = new Show_UI_Event(UIConfig.PlayFrame);
            EventDispatcher.Instance.DispatchEvent(ee);
            return;
        } 
        if (c.UiRecord == UIConfig.OperationActivityFrame.UiRecord)
        {
    ...
    private static IEnumerator DelayDisFuBenScrollOffset(int tabIndex)
    {
        yield return new WaitForSeconds(0.01f);
        EventDispatcher.Instance.DispatchEvent(new FuBenScrollOffestEvent(tabIndex));
    }
    ...
    public static void RecycleConfirm(int itemId,int count, Action okAction)
    {
        var tbItem = Table.GetItemBase(itemId);
        if (tbItem == null)
            return;
        ...
        {
            var notice = GetDictionaryText(240167);
            UIManager.Instance.ShowMessage(MessageBoxType.OkCancel, notice);
            return;
        }

        var tbCallbackItem = Table.GetItemBase(tbItem.CallBackType);
        if (tbCallbackItem == null)
        {
            return;
        }

        var color = GetTableColorString(tbItem.Quality);
        var name = String.Format("[{0}][{1}][-]", color, tbItem.Name);
        var callbackName = string.Format("[{0}]{1}[-]",
            GetTableColorString(tbCallbackItem.Quality), tbCallbackItem.Name);

        var str = string.Format(GetDictionaryText(710), name, tbItem.CallBackPrice * count, callbackName);
        UIManager.Instance.ShowMessage(MessageBoxType.OkCancel, str, "", okAction);
    }
    ...
    public static void Init()
    {
        ...
        ConditionToFlag.Add(ElfDrawConfig, Table.GetConditionTable(ElfDrawConfig).TrueFlag[0]);
        ...
        //飞鞋道具券
        FastReachCouponId = Table.GetClientConfig(375).ToInt();
        //开启背包钥匙Id
        OpenBagKeyId = Table.GetClientConfig(509).ToInt();
        MaxMailCount = Table.GetClientConfig(284).ToInt();
        ChangePeaceModeInterval = Table.GetClientConfig(1221).ToInt();
        MColor.Init();
    }
    ...
    public static bool IsShiZhuangCanEquip(EquipBaseRecord tbEquip, int PartBag)
    {
    ...
    public static IEnumerator LaunchAction(int animationId, float time, Action endCallback, Action interruptCallback)
    {
        ...
        {
            //需求改为除玩家主动移动外，都不被打断
            if (player.GetCurrentStateName() == OBJ.CHARACTER_STATE.RUN)
            {
                interruptCallback();
                yield break;
            }
    ...
    //战旗读条动作
    public static IEnumerator CanBreakLaunchAction(int animationId, float time, Action endCallback, Action interruptCallback)
    {
        var player = ObjManager.Instance.MyPlayer;
        player.PlayAnimation(animationId);
        float elapse = 0;
    ...
    public static int CheckFuctionOnConditionByMission(int MissionId)
    {
        ...
        {
            var state = (int) MissionManager.Instance.GetMissionState(iCurrentMissionId);
            if (state != (int)eMissionState.Finished)
            {
                return -1;
            }
        }
    ...
    public static MissionBaseModel GetCurMainMission()
    {
    ...
    public static eMissionState GetCurMainMissionState()
    {
        var mission = GetCurMainMission();
        return MissionManager.Instance.GetMissionState(mission.MissionId);
    }
    ...
    public static int GetCurMainMissionId()
    {
        ...
        if (mission != null)
        {
            iCurrentMissionId = mission.MissionId;
        }
...
public static int PkValueToColorId(int pkValue)
    ...
    //获取不可回收装备
    public static List<int> GetNotRecoveryEquipList()
    {
        List<int> lst = new List<int>();
        var array = Table.GetClientConfig(306).Value.Split('|');
        for (int i = 0; i < array.Length; i++)
        {
            lst.Add(int.Parse(array[i]));
        }
        return lst;
    }
    ...
    public static void SetSpriteIcon(UISprite sprite, string atlas, string name)
    {
        if (sprite.atlas != null)
        {
            ...
            if (0 == string.Compare(stringname, atlas))
            {
    ...
    public static void ShowEquuipExchangeTip(BagItemDataModel data ,eEquipBtnShow showType = eEquipBtnShow.None,
                                       int charLv = -1)
    {
        if (data == null)
        {
        ...
        itemData.BagId = data.BagId;
        itemData.Exdata.InstallData(data.Exdata);
        if (data.Exdata.Enchance != tbEquip.MaxLevel)
        {
            itemData.Exdata.Enchance += 1;
        }
        var arg = new EquipCompareArguments
        {
            Data = itemData,
    ...
    public static void ShowItemIdTip(int itemId, int type = -1, object exdata = null)
    {
        if (itemId == -1)
        {
            return;
        ...
        switch (itemType)
        {
            ...
            case eItemInfoType.Equip:
            {
                var e = new Show_UI_Event(UIConfig.EquipInfoUI, new EquipInfoArguments {ItemId = itemId, Type = type, Exdata = exdata});
                EventDispatcher.Instance.DispatchEvent(e);
            }
                break;
            case eItemInfoType.Wing:
    ...
    private static int lastKeyPoint = PlayerPrefs.GetInt("RecordKeyPoint", 0);
    private static bool recordKeyPoint = (lastKeyPoint == 0);
    public static void RecordKeyPoint(string point, int step)
    {
#if !UNITY_EDITOR
        if (step > lastKeyPoint)
        { // 比上次靠后
            if (recordKeyPoint)
            { // 第一次
                PlatformHelper.UMEvent(point + "_1");
            }

            PlayerPrefs.SetInt("RecordKeyPoint", step);
            PlayerPrefs.Save();
            lastKeyPoint = step;

            PlatformHelper.UMEvent(point);            
        }
#endif
    }
    ...
    //称号title属性赋值
    public static ObservableCollection<AttributeStringDataModel> StarAddAttr(NameTitleRecord record)
    {
        if (record == null)
            return null;
        ...
        return new ObservableCollection<AttributeStringDataModel>(list);
    }
...
//技能书
        ...
        if (tbItem.Type == 10500)//装备时装
        {
            var fashionCon = UIManager.Instance.GetController(UIConfig.ShiZhuangUI);
            if (fashionCon != null)
            {
                fashionCon.CallFromOtherClass("QuickUseFashion", new[] { bagItem });
            }
            return;
        }
    ...
    public static bool IsQuickBuyGift(int itemId)
    {
        ...
        var _tbStore = Table.GetStore(item.StoreID);
        if (_tbStore != null)
        {
            if (_tbStore.GoodsType != 3)
            {
                return false;
            }
        }
    ...
    public static bool IsTrialEnd(BagItemDataModel item)
    {
        return (item.Exdata.LeftTime >= 0) && (Game.Instance.ServerTime.GetTimeStampSeconds() >= item.Exdata.TrialEndTimeSeconds);
    }
    ...
    public static void ShowQuickBuy(int itemId, int count)
    {
        var items = new Dictionary<int, int>();
        items[itemId] = count;
        ShowQuickBuy(items);
    ...
    public static void ShowQuickBuy(Dictionary<int, int> items)
    {
    ...
    public static void GotoMaterialUI(int itemId)
    {
        var tbItemBase = Table.GetItemBase(itemId);
        if (tbItemBase == null)
            return;

        if (tbItemBase.Type != 22000)
            return;
        ...
        if (tbItemBase.UseUI.Count > 0 && tbItemBase.UseUI[0] != -1)
        {
            if (tbItemBase.UseUI.Count > 2)
            {
                GotoUiTab(tbItemBase.UseUI[0], tbItemBase.UseUI[1], tbItemBase.UseUI[2]);
            }
            else if (tbItemBase.UseUI.Count > 1)
            {
                GotoUiTab(tbItemBase.UseUI[0], tbItemBase.UseUI[1]);
            }
            else if (tbItemBase.UseUI.Count > 0)
            {
                GotoUiTab(tbItemBase.UseUI[0], 0);
            }
        }
        else
        {
    ...
    //    if (0 <= roleId && roleId < 3)
    //    {
    //        var tbChar = Table.GetCharacterBase(roleId);
    //        if (tbChar == null) return false;
    //        if (!string.IsNullOrEmpty(tbChar.PassiveSkillGroup))
    //        {
    //            var skillGrop = tbChar.PassiveSkillGroup.Split('|');
    //            for (int i = 0; i < skillGrop.Length; i++)
    //            {
    //                int tempSkillId;
    //                if (int.TryParse(skillGrop[i], out tempSkillId))
    //                {
    //                    if (tempSkillId == skillId)
    //                    {
    //                        return tempSkillId == skillId;
    //                    }
    //                }
    //            }
    //        }
...
//如果当前要获得buff优先级高于已有的buff优先级，那就弹出确认
            ...
            case 26300: //藏宝图
            {
                ...
                if (tbScene != null)
                {
                    if (tbScene.Type == (int)eSceneType.Fuben || tbScene.Type == (int)eSceneType.Pvp)
                    {
                        EventDispatcher.Instance.DispatchEvent(new ShowUIHintBoard(210123));
                        return;
                    }                    
...
//判断距离，如果还远，就寻路
            ...
            case 19000: //永久加成道具
            {
                ...
                {
                    var e = new ShowUIHintBoard(275005);
                    EventDispatcher.Instance.DispatchEvent(e);
                    return;
                }
                break;
            ...
            case 22201: //祭拜消耗
            {
                UseWorshipMonumentItem(dataModel);
                return;
                break;
            }
        }
...
//物品还在冷却中
    ...
    private static void UseWorshipMonumentItem(ItemInfoDataModel dataModel)
    {
        var tbItemBase = Table.GetItemBase(dataModel.ItemData.ItemId);
        if (null == tbItemBase)
            return;
        var lodeId = tbItemBase.Exdata[0];//策划指定此列配置墓碑ID
        ...
        GameControl.Executer.Stop();
        ObjManager.Instance.MyPlayer.LeaveAutoCombat();
        var e = new MapSceneDrawPath(new Vector3((int)tbSceneNpc.PosX, 0, (int)tbSceneNpc.PosZ), 1.0f);
        EventDispatcher.Instance.DispatchEvent(e);
        ...
        {
            PlayerDataManager.Instance.WorshipMonument(tbLode.NpcId);
        });
    ...
    private static void UseSkillBook(ItemInfoDataModel dataModel)
    {
        ...
        if (skillLv == 0)
        {
            ...
            //没有学会该技能，无法增加技能天赋点！
            var e = new ShowUIHintBoard(270308);
            EventDispatcher.Instance.DispatchEvent(e);
        }
        else
        {
    ...
    public static List<int> FirstChargeJudge()
    {
        int FirstCharge1 = -1;
        int FirstCharge2 = -1;
        int FirstCharge3 = -1;
        ...
        //if (PlayerDataManager.Instance.GetExData((int)eExdataDefine.e69) == 1)
        {
            if (PlayerDataManager.Instance.FirstChargeData != null)
            {
                var RechargeMoney = PlayerDataManager.Instance.GetExData((int)eExdataDefine.e652);
                for (int i = 0; i < PlayerDataManager.Instance.FirstChargeData.FirstChagreItemList.Count; i++)
                {
                    var _charge = PlayerDataManager.Instance.FirstChargeData.FirstChagreItemList[i];
                    switch (i)
                    {
                        case 0:
                            {
                                if (PlayerDataManager.Instance.GetFlag(_charge.flag))
                                {
                                    FirstCharge1 = 3;
                                    break;
                                }
                                ...
                                {
                                    if (RechargeMoney >= _charge.diamond)
                                    {
                                        FirstCharge1 = 1;
                                    }
                                    else
        ...
        result.Add(FirstCharge1);
        result.Add(FirstCharge2);
        result.Add(FirstCharge3);
        return result;
    }



    public static void SummonMonster(int monsterId)
    {
        NetManager.Instance.StartCoroutine(SummonMonsterCorotion(monsterId));
    }

    private static IEnumerator SummonMonsterCorotion(int monsterId)
    {
        var player = ObjManager.Instance.MyPlayer;
        if (null == player)
        {
        ...
        var msg = NetManager.Instance.SummonMonster(monsterId);
        yield return msg.SendAndWaitUntilDone();
        ...
        {
            Logger.Error("SummonMonster--msg.State--{0}", msg.State);
        }
    }
    ...
    {
        var openDay = (int)(Game.Instance.ServerTime.Date - PlayerDataManager.Instance.OpenTime.Date).TotalDays + 1;
        return openDay;
    }

    // 开服是周几（0, 1，2，3，4，5，6）
    public static int GetOpenServerWeek()
    {
        var week = Convert.ToInt32(PlayerDataManager.Instance.OpenTime.DayOfWeek);
        return week;
    }

    private static int MaxOpenServerActCount = -1;
    public static int GetOpenServerAct()
    {
        if (MaxOpenServerActCount == -1)
        {
            MaxOpenServerActCount = 0;
            Table.ForeachKaiFu(record =>
            {
                ++MaxOpenServerActCount;
                return true;
            });
        }

        var openDay = GetOpenServerDays();
        if (openDay > MaxOpenServerActCount)
            return -1;

        var dayOfWeek = GetOpenServerWeek();

        return GetOpenGameActIdByTime(openDay, dayOfWeek);
    }

    public static int GetOpenGameActIdByTime(int openDay, int dayOfWeek)
    {
        var tbKaiFu = Table.GetKaiFu(openDay);
        if (tbKaiFu != null)
        {
            if (dayOfWeek >= 0 && dayOfWeek < tbKaiFu.Week.Length)
            {
                return tbKaiFu.Week[dayOfWeek];
            }
        }
        return -1;
    }

    // 是否是kaifu.txt里关心的 特殊开服日期
    public static bool IsCareKaifuDay(int addDays = 0)
    {
        bool isCare = false;
        var openDay = (int)(Game.Instance.ServerTime.AddDays(addDays) - PlayerDataManager.Instance.OpenTime).TotalDays + 1;
        var tbKaiFu = Table.GetKaiFu(openDay);
        if (tbKaiFu != null)
        {
            isCare = true;
    ...
    // 查找是kaifu.txt里关心的 特殊开服日期 的下一个开启的日期 相距的天数
    public static int CheckActOpenNext(int actId)
    {
        int nextOpenDay = -1;
        int next = 1;
        ...
        for (int i = next; i >= 0; )
        {
            var openDay = (int)(Game.Instance.ServerTime.Date.AddDays(next) - PlayerDataManager.Instance.OpenTime.Date).TotalDays + 1;
            //var openDay = Game.Instance.ServerTime.AddDays(next).Day - PlayerDataManager.Instance.OpenTime.Day + 1;

            var tbKaiFu = Table.GetKaiFu(openDay);
            if (tbKaiFu != null)
            {
                if (dayOfWeek >= 0 && dayOfWeek < tbKaiFu.Week.Count())
                {
                    if (tbKaiFu.Week[dayOfWeek] == actId)
                    {
                        nextOpenDay = next;
                        break;
                    }
    ...
    public static bool CheckIsOpen(List<int> list)
    {//归属刷新时间
        if (list.Count <= 0)
        {
            return true;
    ...
    public static int GetNearWeekLoop(List<int> list)
    {
        if (list.Count <= 0) // 没填则认为OK
        {
            return -1;
        ...
        foreach (var value in list)
        {
            ...
            if (DateTime.Now.DayOfWeek == DayOfWeek.Saturday) //周六要判断周一
            {
                if (value == 0)
                {
                    return 0;
    ...
    public static void ShowModelDisplay(int itemId, Action okAction = null, int okDictId = -1)
    {
        var args = new ModleDisPlayUIArguments();
        args.Args = new List<int> { itemId, 1 };
        args.OkDictId = okDictId;
        args.OkAction = okAction;
        EventDispatcher.Instance.DispatchEvent(new Show_UI_Event(UIConfig.ModelDisplayFrame, args));
    }

    public static void ChangeEquip(int equipId)
    {
        var _bagItem = PlayerDataManager.Instance.GetBagItemByItemId((int)eBagType.Equip, equipId);
        if (_bagItem != null)
        {
            int _bagId = -1;
            int _bagIndex = -1;
            ...
            control.CallFromOtherClass("GetBeReplacedEquip", new[] { _bagItem, (object)_bagId, (object)_bagIndex });

            var _equipType = PlayerDataManager.Instance.ChangeBagIdToEquipType(_bagId);
            if (_equipType != -1)
            {
                var _equipItem = PlayerDataManager.Instance.GetEquipData((eEquipType)_equipType);
                if (_equipItem != null)
                {
                    if (_bagItem.FightValue < _equipItem.FightValue)
                    {
                        return;
                    }
                }
    ...
    public static int GetElfModelDisplayFubenId()
    {
        return 212;
    }

    public static IEnumerator WaitSetIsChangeScene(float span)
    {
    ...
    public static Dictionary<int,int> dicLode2Npc = new Dictionary<int, int>();
    public static Dictionary<int, int> dicNpc2Lode = new Dictionary<int, int>();
    public static void InitDic()
    {
        Table.ForeachLode(tb =>
        {
            dicLode2Npc.Add(tb.Id,tb.NpcId);
            dicNpc2Lode.Add(tb.NpcId,tb.Id);
            return true;
        });
    }
    public static int GetLodeNpcID(int lodeId)
    {
        if(dicLode2Npc.Count ==0)
            InitDic();
        int ret = -1;
        try
        {
            dicLode2Npc.TryGetValue(lodeId, out ret);
        }
        catch (Exception)
        {
    ...
    public static int GetNpcLodeID(int NpcId)
    {
        if (dicNpc2Lode.Count == 0)
            InitDic();
        int ret = -1;
        try
        {
            dicNpc2Lode.TryGetValue(NpcId, out ret);
        }
        catch (Exception)
        {
    ...
    public static UIRoot GetUiRoot()
    {
        ...
        if (UIManager.Instance != null)
        {
            ...
            {
                root = UIManager.Instance.UIRoot.GetComponent<UIRoot>();
            }
        }

        if (root == null)
        {
            root = UnityEngine.Object.FindObjectOfType<UIRoot>();
        }
...
public class ElfItemDataModelComparer : IComparer<ElfItemDataModel>
{
    public int Compare(ElfItemDataModel x, ElfItemDataModel y)
    {
        ...
        {
            if (Table.GetItemBase(y.ItemId).Quality != Table.GetItemBase(x.ItemId).Quality)
            {
                return (Table.GetItemBase(y.ItemId).Quality - Table.GetItemBase(x.ItemId).Quality);
            }
            else
            {
                ...
                {
                    if (x.Exdata.StarLevel != y.Exdata.StarLevel)
                    {
                        return (y.Exdata.StarLevel - x.Exdata.StarLevel);
                    }
                    else
                    {
                        return 0;
        ...
        {
            if (Table.GetItemBase(x.ItemId).Quality != Table.GetItemBase(y.ItemId).Quality)
            {
                return (Table.GetItemBase(y.ItemId).Quality - Table.GetItemBase(x.ItemId).Quality);
            }
            else
...
public class EuqipInfo
{
    ...
    public bool CanBuy;
    public int BuyCost;
    public int BuyNeedItemId;
}

public class DailyActivity
{
    public DailyActivityRecord dailyActivityRecord;
    public int state;
}

public class FlyIconInfo
{
...
public enum ActivityCellState
{
    ...
    WillStart, //即将开始
    Complete, //完成了  
    NoEnoughtLV,//等级不够，未达到参加条件
    CanNotAttend,//不可再参加了    
    ActivityOver//当日限时活动结束
}
