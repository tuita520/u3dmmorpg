...
public class ExpressionHelper
{
    ...
    private static bool Compare(object param1, object param2, string condition)
    {
        ...
        {
            if (Convert.ToInt64(param1) == Convert.ToInt64(param2))
            {
                return true;
            }
        }
    ...
    private static object Calc(Expression[] args)
    {
        if (args.Length != 3)
        {
            var result = string.Format("args error !!  in Calc Expression !!");
            Logger.Error(result);
            return result;
        }
        try
        {
            var param1 = Convert.ToInt32(args[0].Evaluate());
            var condition = args[1].Evaluate() as string;
            var param2 = Convert.ToInt32(args[2].Evaluate());
            if (condition == "+")
            {
        ...
        {
            Logger.Info("Calc Not arg Evaluate {0}", ex);
        }
        return "0";
    }
    ...
    //拆解字符串，看是否有需要特殊运算的
    static System.Text.StringBuilder result_str = new System.Text.StringBuilder(128);

    static System.Text.StringBuilder midstr = new System.Text.StringBuilder(128);
    ...
    public static string GetExpressionString(object dataSource, string desc)
    {
            ...
            while (nend != -1)
            {
                ...
                if (nend == -1)
                {
                    result_str.Append(desc, nbegin, desc.Length - nbegin);
                }
                else
                {
                    //拼接井号前的字符
                    result_str.Append(desc, nbegin, nend - nbegin);
                    ...
                    while (findend)
                    {
                        ...
                        if (dianCount % 2 == 1)
                        {
                            midstr.Append(desc, nbegin, nend - nbegin);
                            midstr.Append(token2);
                        ...
                        else
                        {
                            ...
                            var EvaluateStr = Evaluate(dataSource, midstr.ToString());
                          
                            if (EvaluateStr == null)
                            {
                                return null;
                            }
                            result_str.Append(EvaluateStr);
                            findend = false;
                            nbegin = nend + token2.Length;
                        }
                    }
            ...
            if (result_str.Length == desc.Length)
            {
                for (int i = 0; i < desc.Length; i++)
                {
                    if (result_str[i] != desc[i])
                        return result_str.ToString();
                }
            }
            else
