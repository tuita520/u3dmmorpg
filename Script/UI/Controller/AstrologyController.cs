...
namespace ScriptController
{
    public class AstrologyController : IControllerBase
    {
        ...
        private BuildingData AstrBuilding;
        private Dictionary<int, string> AstrName; //星座名称
        private int AstrologyIndex = -1; //星座index
        private string attrGreenColor = "[0cff00]"; //亮绿
        private string attrGreyColor = "[C0C0C0]"; //灰色
        private Dictionary<int, string> ConditionDict; //属性加成判断条件
        private int diaIndex; //3个宝石中的index
        private bool firstRun = true; //第一次运行界面
        private int itemIndex = -1; //背包的index
        private int[] MaxExp = new int[5]; //存放五种品质最大等级经验
        private int selectedType = -1; //0：宝石，1为背包
        private int tab_Page = (int) GEM_TAB_PAGE.PageGemAll;
        private int TotalExp; //升级总的经验
        private int willLevel = 0; //升级将要达到的等级
        //属性判断条件
        private enum ATTR_CONDITION
        {
            NoCondition = 0, //无条件
            PetCount = 1, //随从个数要求
        ...
        private AstrologyDetailDataModel DetailData { get; set; }
        private AstrologyDataModel MainData { get; set; }

        #region 详细界面

        //占星屋操作
        private void AstrologyOperation(IEvent ievent)
        {
        ...
        //星座缩略图点击事件
        private void SimpleListClick(IEvent ievent)
        {
        ...
        //设置宝石选择框
        private void SetGroupDiamondSeleted()
        {
        ...
        //点击3个宝石中的一个
        private void BtnDiamonds(IEvent ievent)
        {
        ...
        //穿宝石
        private void BtnPutOn(IEvent ievent)
        {
        ...
        private IEnumerator AstrologyEquipOn(int bagIndex, int astrologyId, int Index)
        {
        ...
        private void SetSelectedDiamond(AstrologyDiamondDataModel item)
        {
        ...
        //初始化3个装备的宝石
        private void InitDetailUI(int index)
        {
            MainData.SwitchUI = 1;
            for (var i = 0; i < DetailData.DiaGroup.Count; i++)
            {
        ...
        //刷新属性列表
        private void RefleshItemAttr(AstrologyDiamondDataModel item)
        {
        ...
        //属性后面附件的字段
        private string AttrAttachString(string str, int type, int value, int param)
        {
            if (type <= 0)
            {
                return str;
        ...
        //返回单个属性字符串
        private string AttrFormat(int mprop, int value)
        {
            var str = "";
        ...
        //属性的dictionary列表
        private void AttrDictionary(Dictionary<int, int> mpropvalue, AstrologyDiamondDataModel item)
        {
            if (item.ID == -1)
            {
                return;
        ...
        //单个属性值
        private int PropValue(int level, int mLogic)
        {
        ...
        private bool AttrActiveCondition(AsyncResult<int> Count,
        ...
        //设置组合表
        private void SetWillDiaList(AstrologyDiamondDataModel item)
        {
        ...
        //计算组合属性
        private void GetGroupAttr(Dictionary<int, int> mdict, List<AstrologyDiamondDataModel> items)
        {
            var maxCount = 0;
            var maxIndex = -1;
        ...
        //星座主界面图标事件
        private void MainIconClick(IEvent ievent)
        {
        ...
        //主界面星座list设置
        private void MainListSet(int index)
        {
        ...
        //返回第一界面
        private void BtnBack(IEvent ievent)
        {
        ...
        //第一界面星座list点击事件
        private void MainListClick(IEvent ievent)
        {
        ...
        //购买宝石
        private void BtnBuyList(IEvent ievent)
        {
        ...
        private IEnumerator DrawDiamond(int type)
        {
            using (new BlockingLayerHelper(0))
            {
        ...
        //抽奖结果
        private void DrawResult(IEvent ievent)
        {
        ...
        //背包整理
        private void ArrangeClick(IEvent ievent)
        {
        ...
        //初始化背包
        private void SetBagInit(bool setDiamond = true)
        {
            for (var i = 0; i < MainData.BagBaseTemp.Count; i++)
            {
                MainData.BagBaseTemp[i].Selected = 0;
        ...
        //刷新背包
        private void RefleshBag(int tabIndex)
        {
            tab_Page = tabIndex;
        ...
        //背包Tab 点击事件
        private void BagTabClick(IEvent ievent)
        {
        ...
        //背包物品点击事件
        private void BagItemClick(IEvent ievent)
        {
        ...
        private IEnumerator AstrologyLevelUp(int bagId, int bagIndex, Int32Array needList)
        {
            using (new BlockingLayerHelper(0))
            {
        ...
        //增加一个宝石的经验
        private bool AddOneExp(AstrologyDiamondDataModel item)
        {
            if (item.ID == -1)
            {
                return true;
        ...
        //delete 一个宝石的经验
        private void DeleteOneExp(AstrologyDiamondDataModel item)
        {
            if (item.ID == -1)
            {
                return;
        ...
        //计算将要到达的等级
        private void WillLevel(AstrologyDiamondDataModel item)
        {
            if (item.ID == -1)
            {
                //SetDiamondNull();
        ...
        //获得所有经验
        private int GetTolalExp(int type, int level)
        {
            var exp = 0;
        ...
        //获取宝石类型
        private int GetDiamondType(int type)
        {
            switch (type)
            {
                case 50000:
        ...
        //初始化宝石背包数据
        private void InitAstrologyData(BagBaseData bagBase)
        {
        ...
        //更新宝石背包
        private void UpdateAstrologyData(int bagid, ItemsChangeData bag)
        {
            if (bagid == (int) eBagType.GemBag)
            {
                {
