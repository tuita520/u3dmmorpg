...
namespace ScriptController
{
    public class MyArtifactController : IControllerBase
    {
        ...
        private BagItemDataModel bagDataModel { get; set; }

        private readonly List<eEquipType> EquipOrder1 = new List<eEquipType>
        {
        ...
        private readonly List<int> EquipOrder2 = new List<int>
        {
        ...
        public void CleanUp()
        {
            ...
            bagDataModel = new BagItemDataModel();
        }
        ...
        private void RefreshAdvanceBag()
        {
        ...
        private IEnumerator RankEquipSkillCoroutine(int bagId, int bagIndex, int itemId)
        {
        ...
        private IEnumerator UseEquipSkillCoroutine(int bagId, int bagIndex, bool isUse)
        {
            using (new BlockingLayerHelper(0))
            {
                var type = isUse ? 1 : 0;
        ...
        private bool CheckSmithyEvoItem()
        {
            ...
            var evolveItems = DataModel.AdvanceDataModel.EvolveItems;
            for (var i = 0; i < evolveItems.Count; i++)
            {
                if (evolveItems[i].ItemId < 0 || evolveItems[i].BagId < 0 || evolveItems[i].Index < 0)
                    return false;
            }
            return true;
        }
        ...
        private void SelectAdvanceEquip(int index, BagItemDataModel itemData)
        {
            ...
            if (evoData.EvolvedItemId == -1 || DataModel.AdvanceSelectPack == 0 || choseItem.ItemId != itemData.ItemId)
            { // 主装备
            ...
            { // 同一物品id
                for (var i = 1; i < evoData.EvolveItems.Count; ++i)
                {
                    var item = evoData.EvolveItems[i];
                    if (item != null && item.IsGrey)
                    {
                        ++mEquipCountNow;
        ...
        private int ComparerItem(EquipItemDataModel a, EquipItemDataModel b)
        {
            //部位
