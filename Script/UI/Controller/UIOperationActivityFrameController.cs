...
namespace ScriptController
{
    public class UIOperationActivityFrameController : IControllerBase
    {
        ...
        private OperationActivityTotalDataModel OperationActivityDataModel { get; set; }
        ...
        private static readonly int MAXCount = 15;
        private bool isLotteryEnd = false;//抽奖结束
        private bool isLotteryStart = true;//开始抽奖 
        public UIOperationActivityFrameController()
        {
            ...
            EventDispatcher.Instance.AddEventListener(OnActivityLotteryEndEvent.EVENT_TYPE, OnActivityLotteryEnd);
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    ...
                    foreach (var s in array)
                    {
                        ...
                        if (LotteryRecord.Count > MAXCount)
                        {
...
// 				}
                    ...
                    var temp = GameUtils.MakeItemString(kv.Key, kv.Value);
                    model.Log += temp + "\n";
                }
            }
            catch (Exception e)
        ...
        /// 
        /// </summary>
        /// <param name="isDayUpdate">是否是每日0点刷新的</param>
        private void ApplyOperationActivity(bool isDayUpdate = false)
        {

            Dirty = Dirty || (Game.Instance.ServerTime.DayOfWeek != LastRequestTime.DayOfWeek &&
                              Game.Instance.ServerTime.Minute>=7);
            if (Dirty)
            {
...
//#endif
            ...
            if (isDayUpdate)
            {
                PlayerDataManager.Instance.ApplyOperationActivity();//部分活动在线跨天情况下 需要重新请求数据 否则数据会有问题
                //if (OperationActivityDataModel.CurrentSelectPageIdx < 0 || OperationActivityDataModel.CurrentSelectPageIdx >= OperationActivityDataModel.OpenActivityTermList.Count)
                //{
                    OperationActivityDataModel.CurrentSelectPageIdx = 0;
                    EventDispatcher.Instance.DispatchEvent(new OperationActivityPageClickEvent(OperationActivityDataModel.CurrentSelectPageIdx));
               // }
            }
        }

        private static int SortCompare(MsgOperActivtyTerm a, MsgOperActivtyTerm b)
        {
...
// 			}
        ...
        private IEnumerator OperationActivityClaimRewardCoroutine(int parentId,int id,float delay=0)
        {
            ...
            if (msg.ErrorCode != 0)
            {
                if (msg.ErrorCode == (int)ErrorCodes.Error_Operation_CannotAquire)
                {
                    var e = new ShowUIHintBoard(220705);
            ...
            SevenDayHappyListSort(id);
        }

        private void SevenDayHappyListSort(int id)
        {
            if (OperationActivityDataModel.CurrentSelectPageIdx != 5) return;
            var model = OperationActivityDataModel.ActivityTermList[OperationActivityDataModel.CurrentSelectPageIdx].TableData;

            for (int i = 0; i < model.TableList[model.SubTabIdx].ActivityItemList.Count; i++)
            {
                if (model.TableList[model.SubTabIdx].ActivityItemList[i].ItemId == id)
                {
                    model.TableList[model.SubTabIdx].ActivityItemList[i].CanGetReward = false;
                    break;                
                }
            }
        ...
        private void OnSyncOperationActivityTerm(IEvent ievent)
        {
            ...
            foreach (var term in OperationActivityDataModel.OpenActivityTermList)
            {
            ...
            foreach (var term in OperationActivityDataModel.NormalActivityTermList)
            {
                if (term.ActivityId == e.Id)
                {
                    if (OperationActivityUIType.Lottery == (OperationActivityUIType)term.UIType)
        ...
        {
            ApplyOperationActivity(true);            
        }
        private void OnActivityLotteryEnd(IEvent iEvent)
        {
        ...
        //不需要判断是否可以领取奖励直接跳转UI的情况
        private bool GotoUIDirect(int id)
        {
            foreach (var itemDataModel in mItemCache)
            {
                if (itemDataModel.ItemId == id)
                {
                    if (itemDataModel.GuideUI != -1)
                    {
                        GameUtils.GotoUiTab(itemDataModel.GuideUI, 0);
                        return true;
                    }
                }
            }
        ...
        private void ActivityLottery(int id, OperationActivityTermDataModel current, int type)
        {
            ...
            else
            {
                if (null != GameLogic.Instance)
                {
                    ...
                    if (0 != opt)
                    {
                        if (type == 1) isLotteryStart = false;
                        else isLotteryEnd = false;                    
