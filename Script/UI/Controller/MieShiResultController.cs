...
namespace ScriptController
{
    public class MieShiResultController : IControllerBase
    {
        ...
        private MieshiResultDataModel DataModel;
        ...
        private void RefreshResultData(IEvent ievent)
        {
            ...
            DataModel.ActivityId = info.msg.ActivityId;
            Table.ForeachDefendCityReward(record =>
            {
                if (record.ActivityId == DataModel.ActivityId)
                {
                    if (record.Rank[0] <= info.msg.Rank && record.Rank[1] >= info.msg.Rank)
                    {
                        var towerCount = info.msg.TowerCount;
                        if (towerCount >= 0 && towerCount < record.Rate.Count)
                        {
                            DataModel.Rate = record.Rate[towerCount];
                        }
                        else
                        {
                            DataModel.Rate = 0;
