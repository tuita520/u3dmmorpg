...
namespace ScriptController
{
    ...
    {
        private EquipUIDataModel EquipDataModel;
        ...
        private SmithyDataModel DataModel;
        private int mEquipCountNow;
        private Dictionary<int, int> dicNum_Level;
        private BuildingData mBuildingData;
        private IControllerBase mEquipPackController;
        private int mBuildingLevel;
        private Dictionary<int, bool> mLeftSideMenuStates = new Dictionary<int, bool>();
        // 当前的铸造id
        private int mCurTbForgeId = -1;
        // 当前熔炉index
        private int mCurFurnaceIndex;
        // 当前熔炉数
        private int mCurFurnaceCount;
        private Action mRefreshEvoUiAction;
        private float mSpeedUpPercent;

        private bool isShowing;

        private enum DoneBtnState
        {
            BeginForge,
            CompleteImmediately,
            FinishCasting
        ...
        private EquipPackDataModel mEquipPackDataModel
        {
...
//         }
        ...
        private Coroutine FinishTimeCor;

        private IEnumerator RefleshFinishTime()
        {
            var FurnaceProducts = DataModel.SmithyCastData.FurnaceProducts;
            while (true)
            {
        ...
        private void RefreshCastLeftSideBtns()
        {
        ...
        private void RefreshCastBtnState()
        {
            var castData = DataModel.SmithyCastData;
            // 是否正在铸造
            var isForging = BuildingData.Exdata[mCurFurnaceIndex] != -1;
        ...
        //左侧按钮点击了
        private void OnClickCastLeftSideBtn(IEvent ievent)
        {
        ...
        private void OnLeftSideBtnClicked(int idx)
        {
            SetForgeId(idx);
            RefreshCastBtnState();
        ...
        private void SetForgeId(int index)
        {
            if (index < 0)
            {
                return;
        ...
        ////左侧按钮点击了
        //private void OnFurnaceSelected(IEvent ievent)
        //{
        ...
        //其它按钮点击相应
        private void OnForgeBtnClicked(IEvent ievent)
        {
            var e = ievent as UIEvent_SmithyBtnClickedEvent;
            SmithyOperation(e.Type);
        }

        private void SmithyOperation(int type)
        {
            switch (type)
            {
                // 铸造按钮按下
        ...
        //建筑升级
        private void OnUpdateBuilding(IEvent ievent)
        {
        ...
        private void RefreshEquipPackForItemId(int itemId)
        {
            var evoData = DataModel.SmithyEvoData;
        ...
        private void OnRemoveEvoEquip(IEvent ievent)
        {
