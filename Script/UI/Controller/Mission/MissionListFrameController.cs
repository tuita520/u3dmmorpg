...
namespace ScriptController
{
    public class TaskListCtrler : IControllerBase
    {
        public TaskListCtrler()
        {
            ...
            EventDispatcher.Instance.AddEventListener(MissionTraceEvent.EVENT_TYPE, OnTraceMissionEvent);
            EventDispatcher.Instance.AddEventListener(Event_ReSetMission.EVENT_TYPE, OnClickResetMission);
        }

        private MissionListDataModel DataModel;
        private int ShowDetailMissionId = -1;
        private int tracemissionid = 0;
        private Dictionary<int, int> TraceExdataDic = new Dictionary<int, int>();
        private void OnTraceMissionEvent(IEvent ievent)
        {
            var e = ievent as MissionTraceEvent;
            var missionid = PlayerDataManager.Instance.GetExData(713);
            if (DataModel.MissionId == missionid)
            {
                GameUtils.ShowHintTip(274064);
                return;
            }
            TraceExdataDic.Clear();
            TraceExdataDic.Add(713, DataModel.MissionId);
            PlayerDataManager.Instance.SetExData(713, DataModel.MissionId);
            PlayerDataManager.Instance.SetExDataNet(TraceExdataDic);
            DataModel.TraceString = GameUtils.GetDictionaryText(274064);
            var str = string.Format(GameUtils.GetDictionaryText(274065), DataModel.MissionName);
            GameUtils.ShowHintTip(str);
            EventDispatcher.Instance.DispatchEvent(new MissionTraceUpdateEvent(DataModel.MissionId));
        }
        private void AutoNext(IEvent ievent)
        {
            ...
            {
                if (DataModel.MissionId > 50000 && PlayerDataManager.Instance.GetExData(eExdataDefine.e282) <= 0)
                {
                    //EventDispatcher.Instance.DispatchEvent(new ShowUIHintBoard(220989));
                    UIManager.Instance.ShowMessage(MessageBoxType.OkCancel, GameUtils.GetDictionaryText(220991), "", () =>
                    {
                ...
                PlayerDataManager.Instance.CurTaskId = DataModel.MissionId > 40000 && DataModel.MissionId < 60000 ? DataModel.MissionId : -1;
                ...
                {
                    EventDispatcher.Instance.DispatchEvent(new MissionTrackOpenSwitch((int)(DataModel.MissionId > 50000 ? eMissionMainType.Gang : eMissionMainType.Daily)));
            ...
            else if (eMissionMainType.Farm == type)
            {//狩猎类型
                PlayerDataManager.Instance.CurTaskId = DataModel.MissionId;
        ...
        private void LookMission(int missionId)
        {
            if (missionId == -1)
            {
                return;
            ...
            var missionid = PlayerDataManager.Instance.GetExData(713);
            if (missionid == DataModel.MissionId)
            {
                DataModel.TraceString = GameUtils.GetDictionaryText(274064);
            }
            else
            {
                DataModel.TraceString = GameUtils.GetDictionaryText(274063);
            }
		...
		*/
        ...
        private void ResetMissionDatamodel(int index)
        {
            var missions = MissionManager.Instance.MissionData.Datas;
        ...
        private void OnClickResetMission(IEvent ievent)
        {
            var evn = ievent as Event_ReSetMission;
            if (evn == null)
            {
                return;
            }
        ...
        {
            var bDiamondCount = PlayerDataManager.Instance.GetItemCount(DataModel.ResetNeedItemId);
            if (bDiamondCount < DataModel.ResetNeedItemCount)
            {
                EventDispatcher.Instance.DispatchEvent(new ShowUIHintBoard(GameUtils.GetDictionaryText(101057)));
                return;
            }
            var missionId = -1;
            var missions = MissionManager.Instance.MissionData.Datas;
            if (missions != null)
            {
                var enumerator = missions.GetEnumerator();
                while (enumerator.MoveNext())
                {
                    ...
                    if (table != null)
                    {
                        if (table.ViewType == (int)eMissionMainType.Farm)
                        {
                            missionId = id;
                        }
                    }
            ...
            NetManager.Instance.StartCoroutine(ResetMissionCor(missionId));
        }

        private IEnumerator ResetMissionCor(int missionId)
        {
            var msg = NetManager.Instance.RefreshHunterMission(1);
            yield return msg.SendAndWaitUntilDone();
            if (msg.State == MessageState.Reply)
            {
                if (msg.ErrorCode == (int) ErrorCodes.OK)
                {
                    MissionManager.Instance.MissionData.Datas.Remove(missionId);
                    //EventDispatcher.Instance.DispatchEvent(new Event_UpdateMissionData(missionId));
                }
                else
                {
        ...
        private void SelectIndex(IEvent ievent)
        {
        ...
        private void SelectTap(int index, int loolkMissionId = -1)
        {
            ...
            switch (index)
            {
                ...
                case 4:
                {
                    var tab = Table.GetClientConfig(800);
                    if (tab != null)
                    {
                        var str = tab.Value.Split('|');
                        if (str.Length == 2)
                        {
                            int resetNeedId;
                            if (!int.TryParse(str[0], out resetNeedId))
                            {
                                //解析错误或表配置错误
                                return;
                            }
                            int resetNeedCount;
                            if (!int.TryParse(str[1], out resetNeedCount))
                            {
                                //解析错误或表配置错误
                                return;
                            }
                            DataModel.ResetNeedItemCount = resetNeedCount;
                            DataModel.ResetNeedItemId = resetNeedId;
                        }
                    }
                    DataModel.MissionIndex = index;
                    ResetMissionDatamodel(index);
