...
namespace ScriptController
{
    public class MainUIController : IControllerBase
    {
        public MainUIController()
        {
            ...
            EventDispatcher.Instance.AddEventListener(UIEvent_DeviceInfo_NetWorkStateChange.EVENT_TYPE,
            ...
            EventDispatcher.Instance.AddEventListener(Event_MieShiStartCountDownData.EVENT_TYPE,
                OnEvent_UpdateMieShiStartCountDowm);
            EventDispatcher.Instance.AddEventListener(Event_CommonCountDownData.EVENT_TYPE,
                OnEvent_UpdateCommonStartCountDowm);
            EventDispatcher.Instance.AddEventListener(Event_RefreshFuctionOnState.EVENT_TYPE,
                OnEvent_RefreshFuctionOnState);
            ...
            EventDispatcher.Instance.AddEventListener(OnClickFastReachMessageBoxOKEvent.EVENT_TYPE,
                OnClickFastRecachMsgOK);
            EventDispatcher.Instance.AddEventListener(OnClickFastReachMessageBoxCancleEvent.EVENT_TYPE,
                OnClickFastRecachMsgCancle);

            EventDispatcher.Instance.AddEventListener(RefreshDungeonInfo_Event.EVENT_TYPE, OnRefreshDungeonInfo);
            EventDispatcher.Instance.AddEventListener(MonsterCountRecvEvent.EVENT_TYPE, OnRefreshMonsterCount);
            EventDispatcher.Instance.AddEventListener(NotifyStartXpSkillGuide_Event.EVENT_TYPE, OntifyStartXpkillGuide);
            EventDispatcher.Instance.AddEventListener(NotifyStartMaYaFuBenGuide_Event.EVENT_TYPE,
                OntifyStartMaYaFuBenGuide);

            EventDispatcher.Instance.AddEventListener(DungeonFightOverLater.EVENT_TYPE, OnDungeonFightOverLater);
            EventDispatcher.Instance.AddEventListener(AnchorEnterRoomEvent.EVENT_TYPE, OnAnchorEnterRoomEvent);



            EventDispatcher.Instance.AddEventListener(OnCheckenSkillUpEvent.EVENT_TYPE, OnCheckenSkillUp);
            EventDispatcher.Instance.AddEventListener(OnChooseCheckenSkillUpEvent.EVENT_TYPE, OnChooseCheckenSkillUp);
            EventDispatcher.Instance.AddEventListener(RefreshChijRankiListEvent.EVENT_TYPE, OnUpdateChijiRankList);

            EventDispatcher.Instance.AddEventListener(Event_GoToChicken.EVENT_TYPE, GotoChicken);

            
            EventDispatcher.Instance.AddEventListener(MainUI_FlyIcon2_Event.EVENT_TYPE, (iEvent) =>
            {
                var e = iEvent as MainUI_FlyIcon2_Event;
                if (State == FrameState.Open)
                    EventDispatcher.Instance.DispatchEvent(e.Event);
                else
                    flyEvent = e.Event;
            });
        }
        ...
        private MainUI_FlyIcon_Event flyEvent = null;
        private static float fastReachTimeMax = 1.0f;
        private float fastReachTime = -1.0f;
        private int _bossMaxHp;
        private float batteryLevelDuration;
        private List<int> BlacksceneNpcIds = new List<int> {8, 16, 17, 33};
        private float buffRefreshDuration;
        private float petIsLandRefreshDur;
        private string DungeonTimeFormater = "{0:D2}:{1:D2}";
        //public string MieshiTimeFormater = "{0:D2}:{1:D2}";
        private float duration;
        private bool isMvoe;
        private int IsShowButton;
        private Dictionary<uint, BuffCell> mBuffCellDictionary = new Dictionary<uint, BuffCell>();
        private BuffCell InspireBuff = new BuffCell();
        private float mConvertRate = 5.0f;
        private int mLastShowBuffInfoIndex = -1;
        private FrameState mState;
        private bool NoticeMoving = false;
        private DateTime ShowTime = DateTime.MaxValue;
        private DateTime Speed = DateTime.MaxValue;
        private Coroutine TeamTimteRefresh;
        private float timeDuration;
        private int TipDicId;
        private int MonsterCount;
        //---------------------------------------------------------------------Buff
        private int TipFubenId;
        private DateTime TipStartTime = DateTime.Now;
        private DateTime TipTargetTime = DateTime.Now;
        private BuffListDataModel BuffListDataModel { get; set; }
        //手机状态信息
        private DeviceInfoDataModel DeviceInfoData { get; set; }
        private Coroutine DungeonCoroutine { get; set; }
        private Coroutine MieshiStartCoroutine { get; set; }
        private Coroutine ConnomStartCoroutine { get; set; }
        private MsgCheckenRankList checkData { get; set; }
        private DateTime DungeonTime
        {
        ...
        {
            get { return MainDataModel.ShowBuffTime; }
            set
            {
                MainDataModel.ShowBuffTime = value;
                MainDataModel.InspireTime = GameUtils.GetTimeString(value);
            }
        }

        private MainUIDataModel MainDataModel { get; set; }
        private RadarMapDataModel RadarMapData { get; set; }
        private Dictionary<ulong, RararCharDataModel> radarDataDict = new Dictionary<ulong, RararCharDataModel>();

        private bool IsChanegPkModel = false; //从BOSSHOME副本出来后变为和平模式

        private void AddBuffToDataModel(BuffCell buffCell)
        {
            for (int i = 0; i < BuffListDataModel.ShowBuffs.Count; i++)
            {
                if (BuffListDataModel.ShowBuffs[i].BuffId == buffCell.BuffId)
                {
                    BuffListDataModel.ShowBuffs[i].BuffLastTime = buffCell.BuffLastTime;
                    BuffListDataModel.ShowBuffs[i].BuffLayer = buffCell.BuffLayer;
                    return;
                }
            }
        ...
        private void AddBuff(BuffResult buff)
        {
            BuffCell buffCell;
        ...
        private void OnEvent_UpdateCommonStartCountDowm(IEvent ievent)
        {
            var e = ievent as Event_CommonCountDownData;
            if (e != null)
            {
                int sec = e.Second;
                MainDataModel.CommonCounTime = Game.Instance.ServerTime.AddSeconds(sec);
                MainDataModel.CommonCountDownDesc = e.Desc;
                StartCommonStartCountdown();
            }
        }
            ...
            Table.ForeachFunctionOn((record) =>
            {
                ...
                if (condition != 0)
                {
                    ...
                    if (GameUtils.CheckFuctionOnConditionByMission(record.TaskID) != 0)
                    {
                        ...
                        MainDataModel.FuctionOn.ProgressValue =
                            Table.GetMissionBase(GameUtils.GetCurMainMissionId()).MissionBianHao - lastMissionOrder;
                        if (MainDataModel.FuctionOn.ProgressValue < 0)
                        {
                            MainDataModel.FuctionOn.ProgressValue = 0;
                        }
                        MainDataModel.FuctionOn.ProgressMaxVale = Table.GetMissionBase(record.TaskID).MissionBianHao -
                                                                  lastMissionOrder;
                    }
        ...
        private void OnEvent_ShowSceneMap(IEvent ievent)
        {
            ...
            if (e != null)
            {
                ...
                else if (SceneManager.Instance.isInChickenFuben())
                {
                    var ev = new Show_UI_Event(UIConfig.ChickenSceneMapUI);
                    EventDispatcher.Instance.DispatchEvent(ev);
                }
                else
                {
        ...
        private void OnShowFastRecach(IEvent ievent)
        {
        ...
        private void OnClickFastRecachMsgOK(IEvent ievent)
        {
            ...
            var fastReachItemTimes = PlayerDataManager.Instance.GetItemCount(GameUtils.FastReachCouponId);
            var leftTimes = fastReachItemTimes + freeTimes + vipAdd - vipDoneTimes;
        ...
        private void OnClickFastRecachMsgCancle(IEvent ievent)
        {
            MainDataModel.IsShowFastReachMessageBox = false;
        }

        private void OnClickFastRecach(IEvent ievent)
        {
            var myPlayer = ObjManager.Instance.MyPlayer;
            if (myPlayer == null)
            {
...
//             MainDataModel.IsShowFastReachCheckBox = true;
                // 此处目前的需求和大于5次一样  由于这个策划经常改动  所以暂时注释掉
                myPlayer.LeaveAutoCombat();
                GameUtils.FastReach(myPlayer.fastReachSceneID, myPlayer.fastReachPos.x, myPlayer.fastReachPos.z);
            }
            else if (leftTimes <= 0)
            ...
            {
                myPlayer.LeaveAutoCombat(false);
                GameUtils.FastReach(myPlayer.fastReachSceneID, myPlayer.fastReachPos.x, myPlayer.fastReachPos.z);
            }
        }

        private void OnRefreshDungeonInfo(IEvent ievent)
        {
            ...
            if (e != null && e.Info != null)
            {
                ...
                for (int i = 0; i < e.Info.FloatList.Count; i++)
                {
                    ...
                    {
                        MainDataModel.TdungeonInfo.FloatList[i] = (float) Math.Round(e.Info.FloatList[i], 1);
                    }
                }
        ...
        private void UpdateMinimapCharacter(IEvent ievent)
        {
        ...
        private void ChangeIsShowSuperVipIcon(int bol)
        {
            MainDataModel.IsShowSuperVipIcon = bol;
        }
        ...
        private void ChangeSelfPostion(Vector3 objLoction)
        {
        ...
        private void ClearBuffList(IEvent ievent)
        {
        ...
        private void CloseDungeonQueue(IEvent ievnet)
        {
        ...
        private IEnumerator DungeonCountdownCoroutine()
        {
        ...
        private IEnumerator MieShiStartCountdownCoroutine()
        {
            while (true)
            {
        ...
        private IEnumerator CommonStartCountdownCoroutine()
        {
            while (true)
            {
                if (MainDataModel.CommonCounTime < Game.Instance.ServerTime)
                {
                    MainDataModel.CommonCountDownTimeStr = "00:00";
                    ConnomStartCoroutine = null;
                    MainDataModel.IsShowCommonCountDown = false;
                    yield break;
                }
                var dif = MainDataModel.CommonCounTime - Game.Instance.ServerTime;
                MainDataModel.CommonCountDownTimeStr = string.Format("{0:D2}:{1:D2}", dif.Minutes, dif.Seconds);
                MainDataModel.IsShowCommonCountDown = true;
                yield return new WaitForSeconds(1.0f);
            }
        }


        private string GetBuffTimeString(int seconds)
        {
            string ret;
            if (seconds > 3600)
            {
        ...
        private IEnumerator InspireCoroutine()
        {
            using (new BlockingLayerHelper(0))
            {
                ...
                {
                    EventDispatcher.Instance.DispatchEvent(new PlayInspireEffectEvent());
        ...
        private void MainUIButtonShowEvent(IEvent ievent)
        {
        ...
        private void OnActivityTipClicked(IEvent ievent)
        {
        ...
        private void OnBuffListButtonClick(IEvent ievent)
        {
        ...
        private bool OnClickDungeonAutoFight()
        {
        ...
        private void OnClickInspire()
        {
            RefreshInspireInfo();
            MainDataModel.ShowInspireMessage = true;
        }

        private void RefreshInspireInfo()
        {
            var logic = GameLogic.Instance;
            if (logic == null)
            {
            ...
            if (MainDataModel.InspireGold)
            {
                ...
                InspireBuff.BuffId = buffId;
                MainDataModel.BuffIcon = tbBuff.Icon;
        ...
        private void OnClickUseMedicine()
        {
        ...
        private void OnDungeonCompleteEvent(IEvent ievent)
        {
        }

        private void OnEquipDurableChange(IEvent ievent)
        {
        ...
        private void OnExDataInit(IEvent ievent)
        {
            ...
            var Energy = PlayerDataManager.Instance.GetExData((int) eExdataDefine.e632);
            Energy = Math.Max(Energy, 0);
            MainDataModel.TdungeonInfo.Int32OneList[1] = Energy;

            //var cur = PlayerDataManager.Instance.GetExData((int)eExdataDefine.e623);
            //MainDataModel.CurTowerLevel = cur + 1;
        }
        ...
        private void OnExDataUpData(IEvent ievent)
        {
            ...
            //else if (e.Key == (int) eExdataDefine.e623)
            //{
            //    var cur = PlayerDataManager.Instance.GetExData((int)eExdataDefine.e623);
            //    MainDataModel.CurTowerLevel = cur + 1; 
            //}  
            else if (e.Key == (int) eExdataDefine.e51)
            {
                RefreshRebornLVIcon();
            }
            else if (e.Key == (int) eExdataDefine.e666)
            {
                RefreshInspireInfo();
            }
        }

        private void RefreshRebornLVIcon()
        {

            var rebornID = PlayerDataManager.Instance.ExtData[51];
            var rebornLevel = Table.GetTransmigration(rebornID);
            if (rebornLevel == null)
            {
                return;
            }
            MainDataModel.RebornLvIcon = rebornLevel.RebornIcon;
            if (MainDataModel.RebornLvIcon == -1)
            {
                MainDataModel.IsShowRebornLvIcon = 1;
            }
            else
            {
                MainDataModel.IsShowRebornLvIcon = 0;
            }

            var rold = PlayerDataManager.Instance.GetRoleId();
            if (rold == null) return;
            //剑士
            if (rold == 0)
            {
                PlayerDataManager.Instance.PlayerDataModel.PlayerRoldIcon = rebornLevel.zsRebornIconSquare;
            }
            //法师
            if (rold == 1)
            {
                PlayerDataManager.Instance.PlayerDataModel.PlayerRoldIcon = rebornLevel.fsRebornIconSquare;
            }
            //弓手
            if (rold == 2)
            {
                PlayerDataManager.Instance.PlayerDataModel.PlayerRoldIcon = rebornLevel.gsRebornIconSquare;
            }
        }

        private void OnHideMissionProgress(IEvent ievent)
        {
            MainDataModel.MissionProgress.Show = false;
            MainDataModel.MissionProgress.CurrentValue = 1.0f;
        }

        private void OnInspireCancel()
        {
            MainDataModel.ShowInspireMessage = false;
        }
        ...
        private void OnMainUiOperate(IEvent ievent)
        {
            ...
            switch (e.Type)
            {
                ...
                case 11:
                {
                    EventDispatcher.Instance.DispatchEvent(new Show_UI_Event(UIConfig.UnionWarFrame));
                }
                    break;
            }
        }

        private void OnNetWorkStateChanged(IEvent ievent)
        {
        ...
        private void OnNotifyDungeonTime(IEvent ievent)
        {
        ...
        private void OnSelectServer(IEvent ievent)
        {
        ...
        private void OnShowBuffInfo(bool bShow, int index)
        {
        ...
        private void OnShowDungeonQueue(IEvent ievent)
        {
            var e = ievent as UIEvent_ShowDungeonQueue;
            MainDataModel.Dungeon.IsShow = e.IsShow;
        }

        private void OnShowMissionProgress(IEvent ievent)
        {
        ...
        private void OnShowSenceUIEvent(IEvent ievent)
        {
            ...
            MainDataModel.ShowMapName = true;
            {
                ...
                MainDataModel.myMieshiRanking.Show = false;
                MainDataModel.myMieshiRanking.Damage = "";
                MainDataModel.myMieshiRanking.Rank = "";
            }
        }

        private void OnShowSkill(IEvent ievent)
        {
        }

        private void OnShowTargetMenu()
        {
            ...
            PlayerDataManager.Instance.ShowCharacterPopMenu(characterId, obj.Name, 1, obj.GetLevel(), obj.Reborn,
                obj.RoleId);
        }

        private void OnShowWinsDungeonQueue(IEvent ievent)
        {
        ...
        private void OnSyncBuffCell(IEvent ievent)
        {
            ...
            if (null == ObjManager.Instance.MyPlayer ||
                buffResult.TargetObjId != ObjManager.Instance.MyPlayer.GetObjId())
            {
                return;
            }
        ...
        private void OnTransmitDurable()
        {
        ...
        private void OnTransmitDurableConfirm()
        {
            ...
            {
                float delta = Time.time - ObjManager.Instance.MyPlayer.RemindNoMoveTime + 0.1f;
                GameControl.Executer.PushCommand(new StayCommand(delta));
            }
            var command1 = GameControl.GoToCommand(tbSceneNpc.SceneID, tbSceneNpc.PosX, tbSceneNpc.PosZ, 1.0f);
            GameControl.Executer.PushCommand(command1);

            var command2 = new FuncCommand(() => { MissionManager.Instance.OpenMissionNpcDialog(tbSceneNpc.NpcID); });
            GameControl.Executer.PushCommand(command2);
        }

        private void OnUpdateMissionProgress(IEvent ievent)
        {
            var e = ievent as UpdateMissionProgressEvent;
            MainDataModel.MissionProgress.CurrentValue = e.Percent;
        }

        private Vector3 RadarConvertLoction(Vector3 loc)
        {
        ...
        private void RefreshDamageList(IEvent ievent)
        {
        ...
        private void RefreshMieshiDamageList(IEvent ievent)
        {
        ...
        private IEnumerator RefreshTeamTime(int time)
        {
            while (true)
            {
                yield return new WaitForSeconds(1.0f);
        ...
        private void RemoveBuff(BuffResult buff)
        {
            BuffCell buffCell;
            if (mBuffCellDictionary.TryGetValue(buff.BuffId, out buffCell))
            {
        ...
        private void RemoveBuffsOnDead(IEvent ievent)
        {
...
//不在野外主城中//当前在副本中，无法前往仓库，请副本结束后再试
        ...
        private void SetAutoCombat()
        {
            if (MainDataModel.IsAutoFight == 0)
            {
                ...
                if (ObjManager.Instance.MyPlayer.IsInSafeArea())
                {
                    if (SceneManager.Instance.IsNeedAutoCombatByCurrSceneId())
                    {
                        ObjManager.Instance.MyPlayer.EnterAutoCombat();
                    }
                    else
                    {
                        EventDispatcher.Instance.DispatchEvent(new ShowUIHintBoard(291013));
                    }
                }
                else
                    ObjManager.Instance.MyPlayer.EnterAutoCombat();
        ...
        private void SetDungeonTimerString()
        {
            ...
            if ((type == eDungeonAssistType.DevilSquare || type == eDungeonAssistType.BloodCastle ||
                 type == eDungeonAssistType.FrozenThrone
                 || type == eDungeonAssistType.CastleCraft1 || type == eDungeonAssistType.CastleCraft2 ||
                 type == eDungeonAssistType.CastleCraft3
                 || type == eDungeonAssistType.CastleCraft4 || type == eDungeonAssistType.CastleCraft5 ||
                 type == eDungeonAssistType.CastleCraft6)
        ...
        private void SetupEntry(DamageUnit damageUnit, BossRankEntry entry)
        {
            var rank = damageUnit.Rank;
            var damageStr = GameUtils.GetBigValueStr(damageUnit.Damage);
            if (_bossMaxHp > 0)
            {
                ...
                //damageStr += "(" + percent.ToString("f1") + "%)";//去除百分比显示
            }
        ...
        private void ShowActivityTip(IEvent ievent)
        {
        ...
        private void StartCommonStartCountdown()
        {
            if (ConnomStartCoroutine == null && MainDataModel.CommonCounTime > Game.Instance.ServerTime)
            {
                ConnomStartCoroutine = NetManager.Instance.StartCoroutine(CommonStartCountdownCoroutine());
            }
        }


        private IEnumerator TeamCancelCoroutine()
        {
            using (new BlockingLayerHelper(0))
            {
        ...
        public void CleanUp()
        {
            ...
            //MainDataModel.MainActivity = 2;
        }

        private bool bInitActivity = false;
        private object activityTrigger = null;

        private void MainActivityLogic()
        {
            int week = GameUtils.GetOpenServerWeek();
            if (week == 0)
                week = 7;
            var tb = Table.GetMainActivity(week);
            if (tb == null)
                return;
            int today = (int) Game.Instance.ServerTime.DayOfWeek;
            if (today >= tb.Week.Count())
                return;
            MainDataModel.MainActivity = tb.Week[today];
            if (activityTrigger != null)
            {
                TimeManager.Instance.DeleteTrigger(activityTrigger);
                activityTrigger = null;
            }
            DateTime t = Game.Instance.ServerTime.AddDays(1).Date.AddSeconds(1);

            if (MainDataModel.MainActivity == 1)
            {
                EventDispatcher.Instance.DispatchEvent(new MieShiGetInfo_Event());
            }
            activityTrigger = TimeManager.Instance.CreateTrigger(t, MainActivityLogic);
        }

        public void OnShow()
        {
            if (MainDataModel.ShowInspireMessage)
            {
                MainDataModel.ShowInspireMessage = false;
            }
            var countEndlessFairyland = PlayerDataManager.Instance.GetExData((int) eExdataDefine.e622);
            int curFloor = PlayerDataManager.Instance.GetExData((int) eExdataDefine.e623);
            var tbTower = Table.GetClimbingTower(curFloor);
            if (tbTower != null && tbTower.SweepFloor > 0)
            {
                if (countEndlessFairyland < 1)
                {
                    PlayerDataManager.Instance.NoticeData.EndlessFairyland = false;
                }
                else
                {
                    PlayerDataManager.Instance.NoticeData.EndlessFairyland = true;
                }
            }
            else
            {
            ...
            if (flyEvent != null)
            {
                EventDispatcher.Instance.DispatchEvent(flyEvent);
                flyEvent = null;
            }
        }

        private IEnumerator DelayOpenUI(UIConfig ui, int tab = 0)
        {
            yield return new WaitForSeconds(1f);
            EventDispatcher.Instance.DispatchEvent(new Show_UI_Event(ui));
        }

        private SceneRecord lastSceneTb = null;

        private void OpenPoster()
        {
            MainDataModel.bActiveAdItem = false;
            bool b = false;
            if (PlayerDataManager.Instance.GetFlag(3499) == true)
            {
                lastSceneTb = GameLogic.Instance.Scene.TableScene;
                return;
            }
            ...
            do
            {
                if (lastSceneTb == null || GameLogic.Instance.Scene.TableScene == null)
                    break;
                if (lastSceneTb.Id == GameLogic.Instance.Scene.TableScene.Id)
                    break;
                if (lastSceneTb.Type != 0 || GameLogic.Instance.Scene.TableScene.Type != 0)
                    break;

                //下面是打开广告的逻辑
                //首冲 3000,3001,3002 
                if (PlayerDataManager.Instance.GetFlag(3000) == false ||
                    PlayerDataManager.Instance.GetFlag(3001) == false ||
                    PlayerDataManager.Instance.GetFlag(3002) == false)
                {
                    b = true;
                    NetManager.Instance.StartCoroutine(DelayOpenUI(UIConfig.FirstChargeFrame));
//                EventDispatcher.Instance.DispatchEvent(new Show_UI_Event(UIConfig.FirstChargeFrame));
                ...
                if (controller != null)
                {
                    var datamodel = controller.GetDataModel("") as RechargeActivityDataModel;
                    if (datamodel != null && datamodel.IsShowTouZiBtn != 0)
                    {
                        if (PlayerDataManager.Instance.GetFlag(2538) == false ||
                            PlayerDataManager.Instance.GetFlag(2546) == false)
                        {
                            b = true;
                            NetManager.Instance.StartCoroutine(DelayOpenUI(UIConfig.RechargeActivityUI));
//                        EventDispatcher.Instance.DispatchEvent(new Show_UI_Event(UIConfig.RechargeActivityUI));
                ...
                var vip = PlayerDataManager.Instance.TbVip;
                if (PlayerDataManager.Instance.GetFlag(vip.BuyFlag) == true)
                    break;
                MainDataModel.VipLevel = vip.Id;
                MainDataModel.ADItem.ItemId = vip.ItemId;
                MainDataModel.bActiveAdItem = true;
                b = true;
            } while (false);
...
//        if(PlayerDataManager.Instance.GetFlag())
        }
        ...
        public void Tick()
        {
            ...
            if (fastReachTime >= 0)
            {
                ...
                {
                    // 显示
                    var tbClient = Table.GetClientConfig(124);
                    if (tbClient != null)
                    {
                        var config = tbClient.Value.ToInt();
                        var warnDict = PlayerDataManager.Instance.CheckCondition(config);
                        if (MainDataModel.IsShowFastReach != true && warnDict == 0)
                        {
                            var freeTimes = PlayerDataManager.Instance.GetExData((int) eExdataDefine.e700);
                            //var doneTimes = PlayerDataManager.Instance.GetExData((int)eExdataDefine.e701);
                            var vipDoneTimes = PlayerDataManager.Instance.GetExData((int) eExdataDefine.e702);
                            var vipAdd =
                                Table.GetVIP(PlayerDataManager.Instance.GetRes((int) eResourcesType.VipLevel)).SentTimes;
                            var fastReachItemTimes = PlayerDataManager.Instance.GetItemCount(GameUtils.FastReachCouponId);
                            var leftTimes = fastReachItemTimes + freeTimes + vipAdd - vipDoneTimes;
                            if (leftTimes <= 0)
                            {
            ...
            petIsLandRefreshDur += Time.deltaTime;
            if (petIsLandRefreshDur >= 1)
            {
                petIsLandRefreshDur -= 1;
                InitPetIslandBackInfo();
            }
            if (2 == MainDataModel.MainActivity)
            {
                var tbWarFlag = Table.GetWarFlag(1);
                if (null == tbWarFlag)
                {
                    return;
                }
                var TimeList = tbWarFlag.BelongToTime;
                var ServerTime = Game.Instance.ServerTime;
                var now = ServerTime.Hour*100 + ServerTime.Minute;
                if (now < TimeList[0])
                {
                    MainDataModel.FieldLabel = GameUtils.GetDictionaryText(290049);
                    MainDataModel.FieldLabelColor = GameUtils.GetTableColor(5);
                }
                else if (now >= TimeList[0] && now < TimeList[1])
                {
                    MainDataModel.FieldLabel = GameUtils.GetDictionaryText(290047);
                    MainDataModel.FieldLabelColor = GameUtils.GetTableColor(1);
                }
                else
                {
                    MainDataModel.FieldLabel = GameUtils.GetDictionaryText(290048);
                    MainDataModel.FieldLabelColor = GameUtils.GetTableColor(5);
                }
            }
        }
...
//爬塔的自动战斗搞到了这里..不太合适...
            ...
            //BossHome默认PK模式为组队模式
            if (id == 22000 || id == 22001)
            {
                IsChanegPkModel = true;
                //ChangePkModel(1);
                PlayerDataManager.Instance.ChangePkModel(1);
            }


            //蛮荒孤岛场景为杀戮模式
            //if (id >= 20000 && id <= 20005)
            //{
            //    PlayerDataManager.Instance.ChangePkModel(3);
            //}
            MainDataModel.IsChickenDungeon = SceneManager.Instance.isInChickenFuben() ? 1 : 0;

            MainDataModel.IsMieshiDungeon = isInMieshiFuben() ? 1 : 0;
            MainDataModel.IsShowFastReachMessageBox = false;

            if (PlayerDataManager.Instance.GetRoleId() == 0)
            {
                MainDataModel.ShowMaYaWeaponTypeIcon = 633;
            }
            else if (PlayerDataManager.Instance.GetRoleId() == 1)
            {
                MainDataModel.ShowMaYaWeaponTypeIcon = 634;
            }
            else if (PlayerDataManager.Instance.GetRoleId() == 2)
            {
                MainDataModel.ShowMaYaWeaponTypeIcon = 635;
            }
            ...
            var conllor = UIManager.GetInstance().GetController(UIConfig.EquipUI);
            if (conllor != null)
            {
                var dat = conllor.GetDataModel("") as EquipUIDataModel;
                if (dat != null)
                {
                    dat.isAutoRefine = false;
                }
            }
            if (PlayerDataManager.Instance.IsInPvPScnen() || PlayerDataManager.Instance.IsInBossHomeScnen())
            {
                MainDataModel.IsShowServie = false;
            }
            else
            {
                MainDataModel.IsShowServie = true;
            }

            if (PlayerDataManager.Instance.GetName() == GVoiceManager.Instance.OnlineAnchorName)
            {
                MainDataModel.AnchorOnline = 0;
            }
        ...
        private void InitPetIslandBackInfo()
        {
            ...
            if (str1 != null)
            {
                ...
                MainDataModel.TdungeonInfo.StringList[0] =
                    GameUtils.GetTimeDiffString(Game.Instance.ServerTime.AddSeconds(temp));
                if (temp <= 0)
                {
                    var str = GameUtils.GetDictionaryText(240176);
                    NotifyBattleReminder(19, str, 1);
                }
            }
            ...
            if (str2 != null)
            {
                ...
                MainDataModel.TdungeonInfo.StringList[1] =
                    GameUtils.GetTimeDiffString(Game.Instance.ServerTime.AddSeconds(temp));

                if (temp <= 0)
                {
                    var str = GameUtils.GetDictionaryText(240177);
                    NotifyBattleReminder(19, str, 1);
                }
            }
        }

        private void NotifyBattleReminder(int type, string info, int param)
        {
...
//         }
        ...
        {
            BuffTime = 0;
            MainDataModel.IsShowExitFubenJianTou = false;

            MainDataModel.CommonCounTime = Game.Instance.ServerTime.AddMinutes(-1);
            MainDataModel.IsShowCommonCountDown = false;
...
//如果是恶魔，血色，或者世界boss，关闭活动副本tip
            ...
            {
                if (tbScene.Type == 0 && IsChanegPkModel)
                {
                    PlayerDataManager.Instance.ChangePkModel(0);
                    IsChanegPkModel = false;
                }
            }

            CheckenSkillTemp.Clear();
        }

        public object CallFromOtherClass(string name, object[] param)
        {
            if (name == "ChangeIsShowSuperVipIcon")
            {
                ChangeIsShowSuperVipIcon((int) param[0]);
            }
            return null;
        }
        ...
        private void OnRefreshMonsterCount(IEvent ievent)
        {
            MonsterCountRecvEvent e = ievent as MonsterCountRecvEvent;
            MonsterCount = e.Count;
            MainDataModel.MonterCount = MonsterCount;
        }

        private void OntifyStartXpkillGuide(IEvent ievent)
        {
            var myPlayer = ObjManager.Instance.MyPlayer;
            if (myPlayer == null)
            {
            ...
            MissionManager.Instance.StartDialog(112, () =>
            {
                var skillId = -1;
                if (myPlayer.RoleId == 0) // 战士
                {
                    skillId = 30;
                }
                else if (myPlayer.RoleId == 1) // 法师
                {
                    skillId = 133;
                }
                else if (myPlayer.RoleId == 2) // 弓手
                {
                    skillId = 231;
                }
                if (skillId == -1)
                ...
                var e = new SkillEquipMainUiAnime(skillId, 4, () =>
                {
                    GuideManager.Instance.StartGuide(10000);
                });
                EventDispatcher.Instance.DispatchEvent(e);
            });
        }

        private void OntifyStartMaYaFuBenGuide(IEvent ievent)
        {
            var e = ievent as NotifyStartMaYaFuBenGuide_Event;
            if (e == null)
            {
                return;
            }
            ...
            if (e.mtype == 0)
            {
                ...
                // 开启动画
                MissionManager.Instance.StartDialog(117, () =>
                {
                    var item = -1;
                    if (myPlayer.RoleId == 0) // 战士
                    {
                    ...
                    // 动画结束弹玛雅武器获得界面
                    GameUtils.ShowModelDisplay(item, () =>
                    {
                        // 播放玛雅武器飞动画
                        EventDispatcher.Instance.DispatchEvent(new Show_UI_Event(UIConfig.MainUI));
                        EventDispatcher.Instance.DispatchEvent(new PlayMayaWeaponFlyEvent());
                        //myPlayer.EnterAutoCombat();
                    });

                    // 播放玛雅武器飞动画
                    //var e1 = new PlayMayaWeaponFlyEvent();
                    //EventDispatcher.Instance.DispatchEvent(e1);

                    //玩家武器模型替换为玛雅武器
                    //myPlayer.ChangeEquipModel(17, -1, item * 100 + 1);
                });
            }
            else
            {
        ...
        private void OnDungeonFightOverLater(IEvent ievent)
        {
            MainDataModel.IsShowExitFubenJianTou = true;
            MainDataModel.bChooseCheckSkill = false;
        }

        private void OnAnchorEnterRoomEvent(IEvent iEvent)
        {
            var v = iEvent as AnchorEnterRoomEvent;
            if (v == null) return;
            MainDataModel.AnchorOnline = 0;
            if (v.InRoom)
            {
                var playerName = PlayerDataManager.Instance.GetName();
                MainDataModel.AnchorOnline = (playerName == GVoiceManager.Instance.OnlineAnchorName) ? 0 : 1;
            }
        }

        //缓存的吃鸡技能选择队列
        private List<Dictionary<int, int>> CheckenSkillTemp = new List<Dictionary<int, int>>();

        private void OnCheckenSkillUp(IEvent ievent)
        {
            OnCheckenSkillUpEvent e = ievent as OnCheckenSkillUpEvent;
            if (e == null)
                return;
            MainDataModel.bChooseCheckSkill = true;
            int i = 0;
            foreach (var v in e.dicSkill)
            {
                MainDataModel.BuffList[i].Lv = v.Value;
                MainDataModel.BuffList[i].SkillId = v.Key;
                i ++;
            }
            CheckenSkillTemp.Add(e.dicSkill);
        }

        private void OnChooseCheckenSkillUp(IEvent ievent)
        {
            MainDataModel.bChooseCheckSkill = false;
            if (CheckenSkillTemp.Count > 0)
                CheckenSkillTemp.RemoveAt(0);
            if (CheckenSkillTemp.Count > 0)
            {
                MainDataModel.bChooseCheckSkill = true;
                int i = 0;
                foreach (var v in CheckenSkillTemp[0])
                {
                    MainDataModel.BuffList[i].Lv = v.Value;
                    MainDataModel.BuffList[i].SkillId = v.Key;
                    i++;
        ...
        private void OnUpdateChijiRankList(IEvent ievent)
        {
            RefreshChijRankiListEvent e = ievent as RefreshChijRankiListEvent;
            if (e == null)
                return;
            MainDataModel.ChijiRank.Entrys.Clear();
            if (e.msg == null)
                return;
            if (e.msg.bosDie)
            {
                MainDataModel.ChickenRefreshTime = e.msg.LeftTime;
            }
            else
            {
                MainDataModel.ChickenRefreshTime = 0;
            }
            checkData = e.msg;
            for (int i = 0; i < e.msg.RankList.Count; i++)
            {
                if (i == 0)
                    NotifyChickenKingChange(e.msg.RankList[i].Guid, e.msg.PosX, e.msg.PosZ);
                MishiRankEntry cell = new MishiRankEntry();
                cell.Name = e.msg.RankList[i].Name;
                cell.Rank = (i + 1).ToString();
                cell.Damage = e.msg.RankList[i].Score.ToString();
                MainDataModel.ChijiRank.Entrys.Add(cell);
                if (e.msg.RankList[i].Guid == ObjManager.Instance.MyPlayer.GetObjId())
                {
                    if (MainDataModel.TdungeonInfo.Int32OneList.Count > 0)
                        MainDataModel.TdungeonInfo.Int32OneList[0] = e.msg.RankList[i].Kill;
                    if (MainDataModel.TdungeonInfo.FloatList.Count > 0)
                        MainDataModel.TdungeonInfo.FloatList[0] = e.msg.RankList[i].Die;
                }
            }

        }
        private void NotifyChickenKingChange(ulong objId, float x, float z)
        {
            if (ObjManager.Instance == null)
            {
                return;
            }
            if (ObjManager.Instance.ChickenKingId == objId)
                return;
            ObjManager.Instance.ChickenKingId = objId;
            ...
            while (__enumerator1.MoveNext())
            {
                var objBase = __enumerator1.Current;
                {
                    ...
                    {
                        obj.IsShowChickenKing(true);
                    }
                    else
                    {
                        obj.IsShowChickenKing(false,x,z);
                    }
                }
            }
        }

        private void GotoChicken(IEvent evt)
        {
            var e = new MapSceneDrawPath(new Vector3(checkData.PosX, 0, checkData.PosZ), 1.0f);
            EventDispatcher.Instance.DispatchEvent(e);
            var command = GameControl.GoToCommand(SceneManager.Instance.CurrentSceneTypeId, checkData.PosX,checkData.PosZ);
