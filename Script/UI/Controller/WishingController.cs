...
namespace ScriptController
{
    public class AspirationFrameCtrler : IControllerBase
    {
        ...
        #region 静态变量
        private const int INTERVEL_TIME = 5;
        #endregion
        ...
        public AspirationFrameCtrler()
        {
            ...
            EventDispatcher.Instance.AddEventListener(UIEvent_WishShowFreeDrawEvent.EVENT_TYPE, UIEvent_WishShowFreeDrawEventCallBack);
        }

        private void UIEvent_WishShowFreeDrawEventCallBack(IEvent iEvent)
        {
		...
		*/
        ...
        {     
            ...
            if (_args != null)
            {
                ...
                else if (_args.Tab == 1)//祈福
                {
                    WishData.ShowWitchUI = 1;
                    WishData.ShowWishingBag = 0;//关闭愿望包裹
                }
            }
            //if (_args != null)
            //{
        ...
        public void OnShow()
        {
            var exdataCount = (PlayerDataManager.Instance.GetExData((int)eExdataDefine.e479)) % 10;
            var con = 10 - exdataCount;
            WishData.ItemCount = string.Format(GameUtils.GetDictionaryText(100002303), con);
        }
        #endregion
        #region 普通函数
        //判断显示当前或者历史愿望
        ...
        private void GetPaintResult(List<ItemBaseData> draw, long time)
        {
            ...
            else if (draw.Count <= 10 && draw.Count > 1)
            {
                ...
                {
                    int number = r.Next(0, _drawCount3);
                    while (sore.Contains(number))
                    {
                        number = r.Next(0, _drawCount3);
                    }
                    sore.Add(number);

                    Draw.TenGetItem[i].ItemId = draw[number].ItemId;
                    Draw.TenGetItem[i].Count = draw[number].Count;
                    Draw.TenGetItem[i].Index = draw[number].Index;
                    Draw.TenGetItem[i].Exdata.InstallData(draw[number].Exdata);
                    var _tbItem = Table.GetItemBase(draw[number].ItemId);
        ...
        private void WishDrawReward()
        {
            ...
            var professReward = Table.GetClientConfig(professID);
            string[] rewardValue = professReward.Value.Split('|');
            for (int i = 0; i < rewardValue.Length; i++)
            {           
                var IconID = Table.GetItemBase(int.Parse(rewardValue[i]));
                Draw.WillGetItem[i].IconId = IconID.Icon;
                Draw.WillGetItem[i].ItemId = IconID.Id;
                //Draw.WillGetItem[i].Name = IconID.Name;  
                Draw.WillGetItem[i].IsShowItemUI = 1;           
            }
        }
