...
namespace ScriptController
{
    public class AttributeController : IControllerBase
    {
        ...
        private AutoPiont mAutoPiont;
        private bool mIsResetPoint;
        ...
        private AttributeUIDataModel DataModel { get; set; }
        private int Attribute = int.Parse( Table.GetClientConfig(1210).Value);
        ...
        private void OpenAdvancedProperty(IEvent ievent)
        {
            DataModel.OpenAdvancedProperty = true;
        }
        private void CloseAdvancedProperty(IEvent ievent)
        {
            DataModel.OpenAdvancedProperty = false;
        }

        private IEnumerator AttributeAutoAddCoroutine()
        {
        ...
        private IEnumerator ButtonAddOnPress(int index)
        {
            var pressCd = 0.25f;
            ...
            while (true)
            {
                ...
                {
                    ResourceManager.Instance.StopCoroutine(ButtonPress);
                    ButtonPress = null;
                    yield break;
                }
                if (pressCd > 0.01)
        ...
        private IEnumerator ButtonResOnPress(int index)
        {
            var pressCd = 0.25f;
            var maxInteration = 100;
            while (true)
        ...
        private void ExdataUpdate(IEvent ievent)
        {
            ExdataUpdate();
            ProgressUpdate();
        }

        private void OnAttrExDataUpData(IEvent ievent)
        {
            var playerData = PlayerDataManager.Instance;
            if (playerData != null)
            {
                RefresNoticeData(playerData.GetExData(eExdataDefine.e52) > 0);
                if (playerData.GetExData(eExdataDefine.e52) >= Attribute)
                {
                    PlayerDataManager.Instance.NoticeData.Attributes = true;
                }
                else
                {
                    PlayerDataManager.Instance.NoticeData.Attributes = false;
                }
            }
        }

        private int GetAttributeAdd(int index)
        {
            var ret = 0;
            switch (index)
            {
        ...
        private int GetPlayerAttribute(int i)
        {
        ...
        private void LevelUpUpdate(IEvent ievent)
        {
        ...
        private void OnAttributeDistributionChange(IEvent ievent)
        {
        ...
        private void OnBtnClickOperate(IEvent ievent)
        {
        ...
        private void OnClickBtnAutoAdd()
        {
            NetManager.Instance.StartCoroutine(AttributeAutoAddCoroutine());
        }

        private void OnClickBtnRecommond()
        {
            ResetAddPoint();
        ...
        private int OnClickPointAdd(int index)
        {
            if (DataModel.Distribution == 0)
            {
                return 0;
        ...
        private void OnClickSetAttr(int index)
        {
            var canAddNum = DataModel.Distribution;
            if (canAddNum == 0)
            {
                return;
            }
            NumPadLogic.ShowNumberPad(0, canAddNum + GetAttributeAdd(index), (num) =>
            {
                if (num > 0)
                {
                    DataModel.Distribution += GetAttributeAdd(index);
                    SetAttributeAdd(index, num);
                    SetAttributeStr(index, num);
                    RefreshRelateAttribute();
                    DataModel.Distribution -= num;
                }
            });
        }
        private int OnClickPointDel(int index)
        {
            var point = GetAttributeAdd(index);
            if (point == 0)
            {
        ...
        private void OnExDataInit(IEvent ievent)
        {
            ...
            if (playerData != null)
            {
                ...
                if (playerData.GetExData(52) >= Attribute)
                {
                    PlayerDataManager.Instance.NoticeData.Attributes = true;
                }
                else
        ...
        private void RefreshRelateAttribute()
        {
            PushPointAdd();
...
//         }
        ...
        private void SetAttributeAdd(int index, int value)
        {
            switch (index)
            {
                case 0:
        ...
        private void SetAttributeStr(int index, int value)
        {
            var playerData = PlayerDataManager.Instance.PlayerDataModel.Attributes;
            switch (index)
            {
        ...
        private void UpdateDistribution(int point)
        {
            DataModel.Distribution = point;
            DataModel.Original = DataModel.Distribution;
        }

        private void RefresNoticeData(bool isNeedAdd)
        {
