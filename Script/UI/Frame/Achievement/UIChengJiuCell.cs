...
namespace GameUI
{
	public class UIChengJiuCell : MonoBehaviour
	{
	    ...
	    public void OnEraBtnClick()
	    {
            var item = GetComponent<ListItemLogic>();
            if (null != item)
            {
                var dateModel = item.Item as EraAchvCellDataModel;
                if (null != dateModel)
                {
                    EventDispatcher.Instance.DispatchEvent(new UI_EventEraAchvItemAward(dateModel.Id));
                }
            }	        
	    }
