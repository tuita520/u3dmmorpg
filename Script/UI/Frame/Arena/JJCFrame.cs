...
namespace GameUI
{
	public class JJCFrame : MonoBehaviour, IChainRoot, IChainListener
	{
        ...
        public TimerLogic ExpectTimerLogicLeft;
        public TimerLogic StartTimerLogicLeft;
        public TimerLogic ExpectTimerLogicRight;
        public TimerLogic StartTimerLogicRight;
	...
	//打开荣誉兑换界面，并显示相应的位置
	    ...
	    public void OnClickBtnRecord()
	    {
            ...
            PlayerDataManager.Instance.NoticeData.ArenaRankChange = false;
	    }
	...
	//-------------------------------Battle--------------------------------------
        public void OnClickQueueLeft()
        {
            var e = new MultyBattleOperateEvent(0);
            EventDispatcher.Instance.DispatchEvent(e);
        }
        public void OnClickQueueRight()
        {
            var e = new MultyBattleOperateEvent(1);
            EventDispatcher.Instance.DispatchEvent(e);
        }
        public void OnClickFirstWinReward()
        {
            var e = new CheckGetFirstWinRewardEvent(0);
            EventDispatcher.Instance.DispatchEvent(e);
        }
	    private void Start()
	    {
	...
	try
	{
            ...
            if (StartTimerLogicLeft )
            {
                StartTimerLogicLeft.OnFormatTime += FormatBeginTime;
            }
            if (ExpectTimerLogicLeft)
            {
                ExpectTimerLogicLeft.OnFormatTime += FormatWaitTime;
            }
            if (StartTimerLogicRight)
            {
                StartTimerLogicRight.OnFormatTime += FormatBeginTime;
            }
            if (ExpectTimerLogicRight)
            {
                ExpectTimerLogicRight.OnFormatTime += FormatWaitTime;
            }
	#if !UNITY_EDITOR
	}
	catch (Exception ex)
