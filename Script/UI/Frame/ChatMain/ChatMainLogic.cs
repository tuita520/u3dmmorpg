...
public class ChatMainLogic : MonoBehaviour
{
    ...
    public void OnClick_CloseGift()
    {
        EventDispatcher.Instance.DispatchEvent(new ChatMainOperate(11));
    }
    ...
    {
        EventDispatcher.Instance.DispatchEvent(new ChatMainOperate(12));
    }
    ...
    private void Start()
    {
        ...
        for (var i = 0; i < CustomSetting.Count; ++i)
        {
            var uiButton = CustomSetting[i];
            if (uiButton != null)
            {
                var j = i;
                uiButton.onClick.Add(new EventDelegate(() =>
                {
                    EventDispatcher.Instance.DispatchEvent(new CustomChatEvent(j, uiButton.GetComponent<UIToggle>().value));
                }));
            }
        }
