...
namespace GameUI
{
	public class PlayerPanelFrame : MonoBehaviour
	{
        ...
        public OverheadTitleFrame NameBoard;
        ...
        {
            var e = new PlayerInfoOperation(11);
            EventDispatcher.Instance.DispatchEvent(e);
        }

	    private void OnEvent_ShowTitle(IEvent ievent)
	    {
            var e = ievent as ShowPlayerInfoTitle;
            if (e == null || e.TitleList == null || e.AlianceName == null || NameBoard == null)
	        {
	            return;
	        }
	        ...
	        int index = 0;
            foreach (var item in e.TitleList)
            {
                if (index == 1)
                {
                    var tbNameTitle = Table.GetNameTitle(item);
                    if (tbNameTitle != null)
                    {
                        var active = true;
                        NameBoard.SetTitle(tbNameTitle, e.AlianceName, ref active);
                        NameBoard.gameObject.SetActive(true);
                        break;
                    }
                }
                index++;
	    ...
	    private void OnEnable()
	    {
	        ...
	        if (isDelBinding)
	        {
                ...
                EventDispatcher.Instance.AddEventListener(ShowPlayerInfoTitle.EVENT_TYPE, OnEvent_ShowTitle);
	    ...
	    private void DeleteListener()
	    {
            ...
            EventDispatcher.Instance.RemoveEventListener(ShowPlayerInfoTitle.EVENT_TYPE, OnEvent_ShowTitle);
	    }
