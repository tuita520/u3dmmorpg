...
namespace GameUI
{
	public class EquipMentUIFrame : MonoBehaviour, IChainRoot, IChainListener
	{
        ...
        public GameObject JiLianSuccess; 
        public GameObject JingLianSuccess;
        ...
        public Animation JingLianSuccessEffect;
	...
	catch (Exception ex)
	{
	    ...
	    {
            for (var i = 0; i < OperateMenus.Count; i++)
	        {
        ...
        public void OnClickEquipAppendAuto()
        {
            //if (lockAnimation)
            //{
            //    return;
            //}
            lockAnimation = false;
            var e = new EquipOperateClick(45);
            EventDispatcher.Instance.DispatchEvent(e);
        }
        ...
        public void OnClickSuperExcellentOk()
        {
            var e = new EquipOperateClick(5);
	        EventDispatcher.Instance.DispatchEvent(e);
        }
        public void OnClickSuperExcellentCancle()
        {
            var e = new EquipOperateClick(6);
            EventDispatcher.Instance.DispatchEvent(e);
        }
        ...
        private void OnPlaySpriteAnimationEvent(IEvent ievent)
        {
            var e = ievent as PlaySpriteAnimationEvent;
            if (Effect1 == null)
            {
                return;
            }
            ...
            Effect1.GetComponentInChildren<UISpriteAnimation>().Play();
            Invoke("PlaySpriteAnimation",1.0f);
        }
	...
	catch (Exception ex)
	{
        ...
        private void OnExcellentEquipEvent(IEvent iEvent)
        {
            JiLianSuccess.gameObject.SetActive(true);
            StartCoroutine(EndAnimation(1f, JiLianSuccess.gameObject));
        }
        ...
        private void OnEquipMentUiNotify(IEvent ievent)
	    {
	        ...
	        switch (e.Type)
	        {
                ...
                case 3: // �����ɹ�
                    {
                        if (JingLianSuccessEffect && JingLianSuccessEffect.clip)
                        {
                            lockAnimation = true;
                            JingLianSuccessEffect.gameObject.SetActive(true);
                            JingLianSuccessEffect[JingLianSuccessEffect.clip.name].time = 0;
                            JingLianSuccessEffect.Sample();
                            JingLianSuccessEffect.Play(PlayMode.StopAll);
                            StartCoroutine(EndAnimation(1.1f, JingLianSuccessEffect.gameObject));

                            if (JingLianSuccess)
                            {
                                JingLianSuccess.gameObject.SetActive(true);
                                StartCoroutine(EndAnimation(1.1f, JingLianSuccess.gameObject));  
                            }
                ...
                case 4:
                    {
                        if (JingLianSuccess != null)
                        {
                            JingLianSuccess.SetActive(false);
                        }
                        if (JingLianSuccessEffect != null)
                        {
                            JingLianSuccessEffect.Stop();
                            JingLianSuccessEffect.gameObject.SetActive(false);
                        }
                    }
                    break;
	        }
	    ...
	    private void OnEquipMentUiPackStart(IEvent ievent)
	    {
	        ...
	        mStart = e.Start;
	        pack = e.Pack;
            if (pack == 0 && EquipPack != null && EquipPack.EquipScroll != null)
	        {
	            EquipPack.EquipScroll.SetLookIndex(mStart);
	        }
            else if (pack == 1 && EquipPack != null && EquipPack.PackScroll != null)
	        {
	            EquipPack.PackScroll.SetLookIndex(mStart);
	        }
	        pack = -1;
	    ...
	    private void RemoveBindEvent()
	    {
            ...
            EventDispatcher.Instance.RemoveEventListener(PlaySpriteAnimationEvent.EVENT_TYPE,OnPlaySpriteAnimationEvent);
            EventDispatcher.Instance.RemoveEventListener(ExcellentEquipEvent.EVENT_TYPE, OnExcellentEquipEvent);
        }
        ...
        public void OnButtonEquipShengJie()
        {
            EventDispatcher.Instance.DispatchEvent(new UIEvent_EquipShengJie());
        }

        public void AddGoldBless()
        {
            EventDispatcher.Instance.DispatchEvent(new AddAndSubGodBlessEvent(1));
        }

        public void SubGoldBless()
        {
            EventDispatcher.Instance.DispatchEvent(new AddAndSubGodBlessEvent(0));
        }
