...
namespace GameUI
{
	public class RewardCardFrame : MonoBehaviour
	{
	            ...
	            ("Effect/UI/UI_TuJianJiHuo.prefab", res =>
	            {
	                if (gameObject)
	                {
	                    ...
	                    iTween.MoveTo(effect,
	                        iTween.Hash("position", new Vector3(0, height/2.0f - 27, 0), "time", 0.8f, "islocal", true,
	                            "easetype",
	    ...
	    public void OnLevelupClick()
	    {
            ...
            if (listitem != null)
            {
                var e = new UIEvent_HandBookFrame_OnLevelupClick(listitem.Item as HandBookItemDataModel);
                EventDispatcher.Instance.DispatchEvent(e);
            }
	    }
