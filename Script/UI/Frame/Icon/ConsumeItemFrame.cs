...
namespace GameUI
{
	...
	catch (Exception ex)
	{
	    ...
	    public void UpdataBindingData()
	    {
	        ...
	        else if(ItemId == 22007) //神佑宝石个数大于0，绿色，小于0，红色
            {
                ColorLable = ItemTotalCount.Count <= 0 ? MColor.red : MColor.green;
                if (PlayerDataManager.Instance.NoticeData.RefGoldBlessNum)
                {
                    EventDispatcher.Instance.DispatchEvent(new RefGodBlessMaxEvent(ItemTotalCount.Count));
                }
            }
            else
	        {
