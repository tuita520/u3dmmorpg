...
namespace GameUI
{
    public class JingJieItemFrame : MonoBehaviour
	{
	    ...
	    public ItemIdDataModel ItemData
	    {
	        ...
	        set
	        {
                ...
                var tbItemBase = Table.GetItemBase(ItemIdData.ItemId);
	            ...
	            if (ItemTotalCount != null)
	            {
	                ItemTotalCount.Count = _bagItem.Count;//PlayerDataManager.Instance.GetItemTotalCount(ItemIdData.ItemId);
	                ItemTotalCount.PropertyChanged += OnUpdataBindingData;
