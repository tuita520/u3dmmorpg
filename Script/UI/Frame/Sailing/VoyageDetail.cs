	            ...
	            for (var __i1 = 0; __i1 < __arrayLength1; ++__i1)
	            {
	                var label = __array1[__i1];
	                {
	                    if (i < infoData.ItemPropUI.Count)
	                    {
                            ...
                            var tbMedal = Table.GetMedal(infoData.MedalId);
                            var MedalValue = Table.GetSkillUpgrading(tbMedal.PropValue[i]).Values;
                            attr.AttrValue = MedalValue[infoData.ItemData.MaxLevel-1].ToString();
