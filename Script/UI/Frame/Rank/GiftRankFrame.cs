...
namespace GameUI
{
	public class GiftRankFrame : MonoBehaviour
	{
		...
		{
			OnUIReset();
			var e = new Close_UI_Event(UIConfig.GiftRankUI, true);
			EventDispatcher.Instance.DispatchEvent(e);
		}

		public void OnClickViewInfo()
		{
            var e = new GiftRankOperationEvent(1);
			EventDispatcher.Instance.DispatchEvent(e);
		}

		public void OnClickWorship()
		{
            var e = new GiftRankOperationEvent(2);
			EventDispatcher.Instance.DispatchEvent(e);
		}
		...
		private void OnCloseUiBindRemove(IEvent ievent)
		{
			...
			if (e.Config != UIConfig.GiftRankUI)
			{
				return;
			}
			if (e.NeedRemove == 0)
...
//				Binding.SetBindDataSource(controller.GetDataModel(""));
		...
		private void RemoveBindEvent()
		{
			...
			EventDispatcher.Instance.RemoveEventListener(UpdateGiftModelViewEvent.EVENT_TYPE, OnUpdateGiftModelViewEvent);
