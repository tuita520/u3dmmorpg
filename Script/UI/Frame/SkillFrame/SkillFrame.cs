...
namespace GameUI
{
	public class SkillFrame : MonoBehaviour
	{
        ...
        public GameObject skillUpSpeakTip;
        public GameObject skillOperationSpeakTips;
        public GameObject skillTalentUISpeakTip;
...
// 	                new UIEvent_SkillFrame_OnSkillTalentSelected(fristId));
        ...
        public void OnClickGoToComposeBtn()
        {
            var e = new UIEvent_SkillFrame_GoToCompose();
            EventDispatcher.Instance.DispatchEvent(e);
        }
        ...
        {
            {
                if (null == skillTalentSelect)
                {
                    var res = ResourceManager.PrepareResourceSync<GameObject>
                        ("Effect/UI/SkillFrame/UI_Talent_Select_2.prefab");
                ...
                if (t.gameObject.activeSelf != activity)
                {
                    t.gameObject.SetActive(activity);
                }
            }
        }
...
// 	            Binding.SetBindDataSource(controller.GetDataModel(""));
	...
	#endif
            skillOperationSpeakTips.SetActive(false);
            skillUpSpeakTip.SetActive(false);
            skillTalentUISpeakTip.SetActive(false);
	    }
	...
	catch (Exception ex)
	{
        ...
        private void OnEvent_PropertyChange(object o, PropertyChangedEventArgs args)
        {
            if (args.PropertyName == "TalentIdSelected")
            {
                ...
                StartCoroutine(OnSkillTalentBallSelectContrient(datamodel.TalentIdSelected));
            }
        }

        public IEnumerator OnSkillTalentBallSelectContrient(int id)
        {
            yield return new WaitForFixedUpdate();
            OnSkillTalentBallSelect(id);
        }

        public void OnClickSkillUpSpeakTip()
        {
            skillUpSpeakTip.SetActive(true);
        }
        public void OnClickCloseSkillUpSpeakTip()
        {
            skillUpSpeakTip.SetActive(false);
        }
        public void OnClickskillOperationSpeakTip()
        {
            skillOperationSpeakTips.SetActive(true);
        }
        public void OnClickCloseskillOperationSpeakTip()
        {
            skillOperationSpeakTips.SetActive(false);
        }
        public void OnClickSkillTalentUISpeakTip()
        {
            skillTalentUISpeakTip.SetActive(true);
        }
        public void OnClickCloseSkillTalentUISpeakTip()
        {
            skillTalentUISpeakTip.SetActive(false);
        }
	}
