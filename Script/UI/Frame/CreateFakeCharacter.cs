...
public class CreateFakeCharacter : MonoBehaviour
{
                       ...
                       int elfColorId = -1)
    {
        ...
        if (coroutine)
        {
            ...
            mRunningCoroutine =
                ResourceManager.Instance.StartCoroutine(CreateCoroutine(dataId, bsync, dictEquip, callback, elfId, layer, charColorId, elfColorId));
        }
        else
        {
            CreateChar(dataId, bsync, dictEquip, callback, elfId, layer, charColorId, elfColorId);
        }
    }

    public void CreateWithColor(int dataId,
        int colorId,
        Dictionary<int, int> dictEquip = null,
        Action<ObjFakeCharacter> callback = null,
        int elfId = -1,
        ...
        bool bsync = false)
    {
        Create(dataId, dictEquip, callback, elfId, coroutine, layer, bsync, colorId);
    }
