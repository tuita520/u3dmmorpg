...
namespace GameUI
{
	...
	{
	    public static int[] Guide201 = {20093, 20192, 20290};
	    ...
	    private void OnBagUnlock(IEvent ievent)
	    {
            ...
            //    NetManager.Instance.StopCoroutine(unlockTimerCoroutine);
            //}
            //UnLockTime.text = GameUtils.GetTimeDiffString(Game.Instance.ServerTime);
            //if (e.IsRefresh)
            //{
        ...
        private void OnBagUnlockByPaid(IEvent ievent)
        {
            var e = ievent as PackUnlockUIEvent;
            if (e == null)
            {
            ...
            var data = PlayerDataManager.Instance.PlayerDataModel.Bags.Bags[e.BagId];
