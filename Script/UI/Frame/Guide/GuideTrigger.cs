...
namespace GameUI
{
	public class GuideTrigger : MonoBehaviour
	{
	    ...
	    public static readonly Dictionary<int, List<GuidanceRecord>> s_guidTblDict =
	        new Dictionary<int, List<GuidanceRecord>>();
		public static readonly Dictionary<string, GuidanceRecord> s_newFunctionList = new Dictionary<string, GuidanceRecord>();
	...
	//初始化表
	            ...
	            Table.ForeachGuidance(table =>
	            {
	                ...
	                {
						s_newFunctionList.Add(table.Name,table);
	                }
	    ...
	    public static bool CheckCondtion(GuidanceRecord table)
	    {
	        if (table.Level > 0)
	        {
        ...
        public bool ShowNewFunction(int flagId)
        {
            if (!GameSetting.Instance.EnableNewFunctionTip)
            {
                return false;
                ...
                var table = kv.Value;
                {
                    ...
                    else
                    {
                        ...
                        EventDispatcher.Instance.DispatchEvent(new UIEvent_OpenNewFunctionEvent(table.Name, () =>
                        {
                            //StartCoroutine(StartGuideCoroutine(table.Flag));
                            UIManager.Instance.ShowUI(UIConfig.MainUI);
							EventDispatcher.Instance.DispatchEvent(new UIEvent_ShowMainButton(false, true));
							GuideManager.Instance.StartGuide(table.StepId);
	            ...
	            for (var __i3 = 0; __i3 < __listCount3; ++__i3)
	            {
	                var table = __list3[__i3];
	                {
	                    if (CheckCondtion(table))
	                    {
	                        ...
	                        curGuidStep = table.StepId;
	                        if (CanTrigger(table.StepId))
	                        {
	                            if (224 == table.Id)
	                            {
	//许愿池
                                ...
                                if (table.UIID == 45 && e.Arg.Tab==2)//竞技场，需要判断是不是多人，多人不处理
                                {
                                    return;
                                }
	                            GuideManager.Instance.StartGuide(table.StepId); //表改了 table.Flag就是开启哪个引导    
	                        }
	                        else
	                        {
	                            Logger.Debug("!CanTrigger ({0})", table.StepId);
	                        }
	                        return;
	                    }
	                }
		...
		public void TriggerNewFunction()
		{
			...
			EventDispatcher.Instance.DispatchEvent(new UIEvent_OpenNewFunctionEvent(tb.Name, () =>
			{
		...
		public static bool IsFunctionOpen(string funcName)
		{
			GuidanceRecord tb;
			if (s_newFunctionList.TryGetValue(funcName, out tb))
			{
				if (-1 != tb.FlagPrepose)
				{
					if (!PlayerDataManager.Instance.GetFlag(tb.FlagPrepose))
					{
						return false;
					}
				}
