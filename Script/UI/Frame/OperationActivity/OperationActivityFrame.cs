...
catch (Exception ex)
{
    Logger.Error(ex.ToString());
		...
		private void ChangeSubTableEvent(IEvent ievent)
		{
		    ...
		    List<OperationActivityItemDataModel> dataModelsList = new List<OperationActivityItemDataModel>();
		    dataModelsList = data.TableList[data.SubTabIdx].ActivityItemList.ToList();
            dataModelsList.Sort(SevenDayHappyListSort);

            data.TableList[data.SubTabIdx].ActivityItemList.Clear();
		    for (int i = 0; i < dataModelsList.Count; i++)
		    {                
                data.TableList[data.SubTabIdx].ActivityItemList.Add(dataModelsList[i]);
		    }		     
		...
		IEnumerator LotteryAnimationEffectCoroutine()
		{
			...
			RefreshMask();
            EventDispatcher.Instance.DispatchEvent(new OnActivityLotteryEndEvent());
		}
