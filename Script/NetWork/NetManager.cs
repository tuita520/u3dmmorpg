                                  ...
                                  IChat9xServiceInterface, ITeam9xServiceInterface
{
    ...
    private void NetWorkStateChanged(IEvent ievent)
    {
        ...
        if (status == NetworkStatus.ReachableViaWWAN || status == NetworkStatus.ReachableViaWiFi)
        {
            PrepareForReconnect();
        }
    }
...
catch (Exception ex)
{
    ...
    private void QueueSuccess(QueueSuccessData data)
    {
        ...
        switch (type)
        {
            case QueueType.Login:
                Instance.StartCoroutine(LoginLogic.LoginSuccess());
                break;
        ...
        PlayerDataManager.Instance.BagItemCountChange(bag);
        {
            ...
            while (__enumerator4.MoveNext())
            {
                var changeData = __enumerator4.Current;
                {
                    ...
                    if (bagid >= (int)eBagType.EquipShiZhuang && bagid <= (int)eBagType.WeaponShiZhuang)
                    {
                        PlayerDataManager.Instance.UpdateEquipData(bagid, changeData.Value);
                        var fashionCon = UIManager.Instance.GetController(UIConfig.ShiZhuangUI);
                        if (fashionCon != null)
                        {
                            fashionCon.CallFromOtherClass("UpdatePartData", new[] { changeData.Value });
                        }
                        continue;
                    }
                    ...
                    if (bagid == (int) eBagType.Piece)
                    {
                        ...
                        if (null != HandbookCon)
                       {
                           EventDispatcher.Instance.DispatchEvent(new HankBookItemData_Event());
    ...
    public void NotifyCommonCountdown(int time)
    {
        EventDispatcher.Instance.DispatchEvent(new Event_CommonCountDownData(GameUtils.GetDictionaryText(300942), time));
    }
