                                  ...
                                  IChat9xServiceInterface, ITeam9xServiceInterface
{
    ...
    /// 收到角斗申请
    /// </summary>
    public void ReceiveChallenge(ulong characterId, string name, string server)
    {
        var tip = string.Format(GameUtils.GetDictionaryText(100003322), server, name);
        UIManager.Instance.ShowLimitMessage(MessageBoxType.OkCancel, tip, "",
            () =>
            {//OK
                NetManager.Instance.StartCoroutine(OnSendAcceptChallenge(characterId, true));
            },
            () =>
            {//Cancel
                NetManager.Instance.StartCoroutine(OnSendAcceptChallenge(characterId, false));
            },false,true,false,10);
    }

    private IEnumerator OnSendAcceptChallenge(ulong characterId, bool accept)
    {
        var msg = NetManager.Instance.AcceptChallenge(characterId, accept);
...
//功能未开放，不处理队伍消息
    ...
    public void MatchingSuccess(int fubenId)
    {
        ...
        //    if (fubenId == 200)
        //    {
        //        var e1 = new MessageBoxAutoChooseEvent(true, 10);
        //        EventDispatcher.Instance.DispatchEvent(e1);
        //    }
        //}    
        //if (fubenId >= 100 && fubenId <= 117)
    ...
    public void TeamServerMessage(int resultType, string args)
    {
        ...
        switch ((eLeaveMatchingType) resultType)
        {
            ...
            {
                if (queueData.QueueId < 111 || queueData.QueueId > 117)
                {
                    queueData.QueueId = -1;
                    //超时排队确认时间
            ...
            {
                queueData.QueueId = -1;
                string des = string.Format(GameUtils.GetDictionaryText(274062),args);
                //同队中有玩家拒绝
                UIManager.Instance.ShowMessage(MessageBoxType.Ok, string.Format (GameUtils.GetDictionaryText(274061), des));
            }
                break;
...
//                GameUtils.ShowHintTip(GameUtils.GetDictionaryText(270019));
//                EventDispatcher.Instance.DispatchEvent(new TeamChangeLineupEvent());
            }
                break;
    ...
    public void BroadcastAnchorOnline(string charName, int online)
    {
        GVoiceManager.Instance.OnlineAnchorName = charName;
        //EventDispatcher.Instance.DispatchEvent(new AnchorOnlineEvent(GVoiceManager.Instance.IsOnLine));
    }
    ...
    public void SyncMissions(MissionDataMessage missions)
    {
        ...
        if (null != GameLogic.Instance && null != GameLogic.Instance.GuideTrigger)
        {
            ...
            {
                var l = listCompletedMission;
                var n = l.Count;
                for (var i = 0; i < n; i++)
                {
                    var id = l[i];
                    if (id == PlayerDataManager.Instance.CurTaskId)
                    {
                        MissionManager.Instance.CommitMission(id);
                        return;
                    }
                }
            }
    ...
    public void NotifySnapShotResult(int state)
    {
        // throw new NotImplementedException();
        var eState = (eSnapShotResult) state;
        switch (eState)
        {
            case eSnapShotResult.SnapShotExist:
            {
                UIManager.Instance.ShowMessage(MessageBoxType.OkCancel, "存档已经存在是否覆盖？", "",
                    () => { NetManager.Instance.StartCoroutine(SendSnapShotCommand()); });
            }
                break;
            case eSnapShotResult.SnapShotNotExist:
            {
                UIManager.Instance.ShowMessage(MessageBoxType.Ok, "存档不存在！");
            }
                break;
            case eSnapShotResult.NeedReload:
            {
                Game.Instance.ExitToLogin();
            }
                break;
    ...
    private IEnumerator SendSnapShotCommand()
    {
        var command = GameUtils.LastSaveCommand + ",true";
        var msg = NetManager.Instance.GMLogic(command);
        yield return msg.SendAndWaitUntilDone();
