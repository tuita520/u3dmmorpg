    ...
    public void AddBuff(BuffResult buff)
    {
        ...
        switch (buff.Type)
        {
            ...
            case BuffType.HT_MISS:
            {
                ...
                if (autoCombatData == null || otherPlayer == null) return;
...
//眩晕
    ...
    public IEnumerator SendUseSkillRequestCoroutine(ulong objId, int skillId, ulong targetId)
    {
        ...
        var msg = Instance.SendUseSkillRequest(GameLogic.Instance.Scene.SceneTypeId,requset);
...
//异步逻辑可能发送接受不一致
	...
	public void ForceStopMove(PositionData msg)
	{
		...
		obj.StopMoveWithoutNotifyServer();
		var des = GameLogic.GetTerrainPosition(GameUtils.DividePrecision(msg.Pos.x),
			 GameUtils.DividePrecision(msg.Pos.y));
		var dir = new Vector3(GameUtils.DividePrecision(msg.Dir.x), 0, GameUtils.DividePrecision(msg.Dir.y));
		var diff = (des.xz() - obj.Position.xz()).magnitude;
		if (diff > 3)
		{
			obj.Position = des;
